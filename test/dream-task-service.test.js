...
describe("analyzeDreamTasks (requires provider API key from .env)", () => {
  itIfProviderKey("generates suggestions via real LLM and reads them back from cache", async () => {
    ...
    const writtenNote = app.replaceNoteContent.mock.calls[0][1];
    expect(writtenNote).toMatch(/\n---\s*\n\n>\s*Generated by .+ · .+/);
    ...
    expect(cachedResult.llmAttributionFooter).toBe(freshResult.llmAttributionFooter);

    // CRLF in the note must not break parsing the persisted attribution line (Amplenote / some clients).
    await app.replaceNoteContent({ uuid: "dream-note-uuid" }, app.replaceNoteContent.mock.calls[0][1].replace(/\n/g, "\r\n"));
