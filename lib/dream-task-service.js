...
async function _generateSuggestionsFromLlm(params) {
  ...
  const configuredProviderEm = providerEmOverride || app.settings?.[SETTING_KEYS.LLM_PROVIDER_MODEL];
  if (!configuredProviderEm || configuredProviderEm === 'none') {
    return {
  ...
  try {
    ...
    result = await llmPromptWithPluginFallback(app, prompt, _buildLlmOptions(app, providerEmOverride));
  ...
  return {
    enforcedTasks,
    goalsSummary,
    llmAttributionFooter: _llmAttributionFooterFresh(app, providerEmOverride),
  };
}
...
function _buildLlmOptions(app, providerEmOverride = null) {
  ...
  if (overrideProviderEm) {
    ...
    const overrideApiKey = overrideApiSetting ? (app.settings?.[overrideApiSetting] || "").trim() : "";
  ...
  if (devMayForceOpenAi) {
    ...
    if (apiSetting) {
      const apiKey = (app.settings?.[apiSetting] || "").trim();
...
function _dreamTaskResolvedModel(app, providerEmOverride = null) {
  const opts = _buildLlmOptions(app, providerEmOverride);
...
function _llmAttributionFooterFresh(app, providerEmOverride = null) {
  const model = _dreamTaskResolvedModel(app, providerEmOverride);
