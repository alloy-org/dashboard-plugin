...
// Date: 2026-03-14 | Model: claude-4.6-opus-high-thinking
export async function analyzeDreamTasks(app) {
  const now = new Date();
  ...
  // Date: 2026-03-14 | Model: claude-4.6-opus-high-thinking
  const topTasks = _rankAndSelectTopTasks(openTasks);
  const taskJsonObjects = topTasks.map(task => _buildTaskJsonObject(task));
  logIfEnabled(`[dream-task] Selected ${taskJsonObjects.length} top tasks from ${openTasks.length} open tasks for LLM analysis`);
  ...
  return { tasks, goalsSummary, cached: false, noteUUID: noteHandle.uuid };
}
