...
import { apiKeyBucketFromLlmProvider, apiKeyFromProvider, DASHBOARD_NOTE_TAG, IS_DEV_ENVIRONMENT, SETTING_KEYS } from "constants/settings"
...
async function _generateSuggestionsFromLlm(params) {
  ...
  try {
    ...
    // Prompt: "add adequate debugging to understand why all of these failed"
    logIfEnabled('[dream-task] llmPromptWithPluginFallback returned', {
      durationMs: Number((performance.now() - llmStart).toFixed(1)),
      resultType: result === null ? 'null' : typeof result,
      resultIsArray: Array.isArray(result),
      hasTasksField: !!(result && typeof result === 'object' && 'tasks' in result),
      tasksLength: Array.isArray(result?.tasks) ? result.tasks.length : null,
    ...
    logIfEnabled('[dream-task] llmPromptWithPluginFallback threw',
      { durationMs: Number((performance.now() - llmStart).toFixed(1)), status: error?.response?.status,
...
// [Claude claude-sonnet-4-6] Task: always resolve aiModel so llmPrompt never receives undefined
// Prompt: "dream-task-service is throwing 'Model undefined not found in any provider'"
