...
export async function analyzeDreamTasks(app, excludeUuids) {
  ...
  const rawContent = await app.getNoteContent(noteHandle.uuid);
  const existingContent = (typeof rawContent === 'string') ? rawContent : '';

  // [Claude] Task: always fetch domain tasks to build task-note mapping and find busiest note
  // Prompt: "at least 50% of suggested tasks should be existing tasks; clicking non-existing creates in busiest note"
  ...
  const taskUuidToNoteUUID = new Map();
  for (const task of openTasks) {
    if (task.uuid && task.noteUUID) {
      taskUuidToNoteUUID.set(task.uuid, task.noteUUID);
    }
  }
  ...
  if (!isReseed) {
    ...
    if (todaySection) {
      ...
      const enrichedTasks = cached.tasks.map(task => ({
        ...task,
      ...
      return { tasks: enrichedTasks, goalsSummary: cached.goalsSummary, cached: true, noteUUID: noteHandle.uuid, defaultNoteUUID };
    }
  }
  ...
  if (planNote) {
    const fullPlanContent = await app.getNoteContent(planNote.uuid);
    if (fullPlanContent) {
  ...
  // [Claude] Task: validate, enforce 50% existing task minimum, and enrich with noteUUIDs
  // Prompt: "at least 50% of suggested tasks should be existing tasks from getTaskDomainTasks"
  // Date: 2026-03-16 | Model: claude-4.6-opus-high-thinking
  const validUuids = new Set(openTasks.map(t => t.uuid).filter(Boolean));
  const tasks = _validateTasks(result.tasks, validUuids);
  const enforcedTasks = _enforceExistingTaskMinimum(tasks, topTasks, taskUuidToNoteUUID);
  const goalsSummary = _validateGoalsSummary(result.goalsSummary);
  logIfEnabled(`[dream-task] Received ${enforcedTasks.length} task suggestions (${enforcedTasks.filter(t => t.isExisting).length} existing, ${enforcedTasks.filter(t => !t.isExisting).length} invented)`);

  const noteContent = _formatTasksForNote(dateHeading, enforcedTasks, goalsSummary);
  try {
  ...
  const shownUuids = enforcedTasks.filter(t => t.uuid).map(t => t.uuid);
  return { tasks: enforcedTasks, goalsSummary, cached: false, noteUUID: noteHandle.uuid, defaultNoteUUID, shownUuids };
}
...
// Prompt: "at least 50% of suggested tasks should be existing tasks from getTaskDomainTasks"
// Date: 2026-03-16 | Model: claude-4.6-opus-high-thinking
function _validateTasks(tasks, validUuids) {
