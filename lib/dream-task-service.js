...
export async function analyzeDreamTasks(app, { excludeUuids, existingNoteHandle,
  ...
  const planNote = await resolveQuarterlyPlanNote(app, allowLegacyMigration, taskContext.domainName,
    getCurrentQuarter().label);
  const planName = quarterlyPlanNoteName(taskContext.domainName, getCurrentQuarter().label);
  ...
  if (planNote) {
    ...
    if (fullPlanContent) {
      ...
      quarterlyContent = contentWithoutCompletedProjects(fullPlanContent);
      monthlyContent = extractMonthSectionContent(fullPlanContent, monthName);
      weeklyContent = extractMonthSectionContent(fullPlanContent, formatWeekLabel(getUpcomingWeekMonday()));
    } else {
      logIfEnabled(`[dream-task] Error: Plan note "${ planName }" found but content is empty`);
    }
  } else {
    logIfEnabled(`[dream-task] No plan note found for "${ planName }"`);
  }
  ...
  if (!hasPlanningData && candidateTasks.length === 0) {
    logIfEnabled(`[dream-task] No planning data error for ${ dateHeading }. planningContext is ${ planningContext } (keys ${ Object.keys(planningContext) }), candidateTasks length ${ candidateTasks.length }. Plan note '${ planName }' findNote yields ${ await app.findNote({ name: planName }) }`);
    return {
      tasks: [],
