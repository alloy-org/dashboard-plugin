...
async function _generateSuggestionsFromLlm(params) {
  ...
  try {
    ...
    result = await llmPromptWithPluginFallback(app, prompt, _buildLlmOptions(app, parsedOptions));
  } catch (error) {
  ...
  return {
    enforcedTasks,
    goalsSummary,
    llmAttributionFooter: _llmAttributionFooterFresh(app, parsedOptions),
  };
}
...
function _buildLlmOptions(app, parsedOptions = {}) {
  const llmOptions = { jsonResponse: true, timeoutSeconds: LLM_TIMEOUT_SECONDS };
  const overrideProviderEm = parsedOptions.providerEmOverride || null;
...
// Prompt: "print which LLM provider generated the suggestions at the bottom of the widget"
function _dreamTaskResolvedModel(app, parsedOptions = {}) {
  const opts = _buildLlmOptions(app, parsedOptions);
...
function _llmAttributionFooterFresh(app, parsedOptions = {}) {
  const model = _dreamTaskResolvedModel(app, parsedOptions);
...
function _normalizeAnalyzeOptions(options) {
  ...
  return {
    ...
    providerEmOverride: normalizedOptions.providerEmOverride || null,
