...
export async function analyzeDreamTasks(app, { excludeUuids, existingNoteHandle,
  ...
  const undismissedCount = noteContext.cached.tasks.filter(t => !_isTaskDismissed(t)).length;
  logIfEnabled('[dream-task] cached-note state', { noteUUID: noteContext.noteHandle?.uuid ?? null,
    ...
    cachedTaskCount: noteContext.cached.tasks.length, undismissedCount, minimumSuggestionCount,
    willUseCache: undismissedCount >= minimumSuggestionCount });
  if (undismissedCount >= minimumSuggestionCount) {
  ...
  const requestedSuggestionCount = Math.max(MIN_SUGGESTION_REQUEST_COUNT,
    minimumSuggestionCount - noteContext.cached.tasks.length);
  const candidateTasks = _filterCandidateTasks(taskContext.openTasks, excludeSet);
  ...
  if (!hasPlanningData && candidateTasks.length === 0) {
    logIfEnabled(`[dream-task] No planning data error for ${ dateHeading }. planningContext is ${ planningContext } (keys ${ Object.keys(planningContext) }), candidateTasks length ${ candidateTasks.length }. Plan note '${ derivePlanName() }' findNote yields ${ await app.findNote({ name: derivePlanName() }) }`);
    return {
      tasks: [],
...
async function _loadTaskContext(app) {
  ...
  logIfEnabled(`[dream-task] Found ${openTasks.length} open tasks from ${ domainUuid ? `domain ${ domainUuid }` : "all notes (couldn't find a domainUuid)" }`);
  const noteNameFromNoteUuid = await _noteNameFromNoteUuidMap(app, openTasks);
  const nativeTaskFromTaskUuid = _nativeTaskFromTaskUuidMap(openTasks);
  ...
  return { allTasks, defaultNoteUUID, domainUuid, nativeTaskFromTaskUuid, noteNameFromNoteUuid,
    noteUuidFromTaskUuid, openTasks };
}
...
function _buildCachedAnalyzeResult(noteContext, taskContext) {
  ...
  const tasks = noteContext.cached.tasks
    .filter(task => !_isTaskDismissed(task))
...
async function _loadPlanningContext(app, monthName, existingContent) {
  ...
  const planNotes = await arrayFromFilterNotesResult(app.filterNotes({ query: planName }));
