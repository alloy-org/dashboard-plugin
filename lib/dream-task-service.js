...
import { AMPLE_AGENT_PRO_NOTE_NAME, preferredModels, providerFromModel, providerNameFromProviderEm } from "providers/ai-provider-settings"
...
//   defaultNoteUUID: string|null, shownUuids: string[], llmAttributionFooter: string|null}|
...
async function _generateSuggestionsFromLlm(params) {
  ...
  return {
    enforcedTasks,
    goalsSummary,
    llmAttributionFooter: _llmAttributionFooterFresh(providerEmOverride),
  };
}
...
function _dreamTaskResolvedModel(providerEmOverride = null) {
  const opts = _buildLlmOptions(providerEmOverride);
...
function _llmAttributionFooterFresh(providerEmOverride = null) {
  const model = _dreamTaskResolvedModel(providerEmOverride);
