...
export async function analyzeDreamTasks(app, options = {}) {
  ...
  const cachedResult = _buildCachedAnalyzeResult(app, parsedOptions, noteContext, taskContext);
...
// @param {object} app - App bridge (for dashboard LLM label when serving cached note content).
...
function _buildCachedAnalyzeResult(app, parsedOptions, noteContext, taskContext) {
  ...
  return {
    ...
    shownUuids: [],
    llmAttributionFooter: _llmAttributionFooterCached(app),
  };
}
...
async function _generateSuggestionsFromLlm(params) {
  ...
  if (!result || !result.tasks) {
    ...
    const snippet = result ? JSON.stringify(result).substring(0, 200) : 'empty response';
    return {
...
// ----------------------------------------------------------------------------------------------
// @desc Footer when serving cached note content (exact generation provider is not stored on the note).
function _llmAttributionFooterCached(app) {
  const providerEm = app.settings?.[SETTING_KEYS.LLM_PROVIDER_MODEL];
  const bucket = apiKeyBucketFromLlmProvider(providerEm);
  if (!bucket) {
    return 'Suggestions loaded from today\'s note';
  }
  const name = providerNameFromProviderEm(bucket);
  return `Suggestions from today\'s note · dashboard AI: ${ name }`;
}
