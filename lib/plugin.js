...
import { SETTING_KEYS, DASHBOARD_COMPONENTS, DASHBOARD_NOTE_TAG } from "constants/settings"
import {
  fetchDashboardData,
  createQuarterlyPlan,
  fetchQuotes,
...
const plugin = {
   ...
   *
   * @param {"createOrAppendMonthlyPlan"} actionType - Create a quarterly plan note or append a
   *   month section to an existing one.
   *   args[0] {object} quarterInfo  { label, year, quarter }
   *   args[1] {string} monthName   Full month name, e.g. "March"
   *
   ...
   *
   * @param {"getMonthlyPlanContent"} actionType - Check a quarterly plan note for a month
   *   section and return its content.
   *   args[0] {string} noteUUID    UUID of the quarterly plan note.
   *   args[1] {string} monthName   Full month name, e.g. "March".
   *
   ...
   *
   * @param {"getNoteSections"}      actionType - Return the sections of a note.
   *   args[0] {string} noteUUID
   *
   ...
   *   args[0] {string} url
   *
   * @param {"recordMoodRating"}     actionType - Record a mood rating via app.recordMoodRating.
   *   args[0] {number} rating  Integer in the range -2 to +2.
   *
   * @param {"saveMoodNote"}         actionType - Append a mood entry to the "Mood rating history" note.
   *   args[0] {number} rating      The integer mood value (-2 to +2).
   *   args[1] {string} label       Human-readable mood label (e.g. "Good").
   *   args[2] {string} [notes]     Optional free-text notes the user entered.
   *
    ...
    try {
      switch (actionType) {
        ...
        case "createOrAppendMonthlyPlan":
          return await createOrAppendMonthlyPlan(app, args[0], args[1]);

        case "createOrAppendWeeklyPlan":
          return await createOrAppendWeeklyPlan(app, args[0], args[1]);

        // [Claude] Task: DreamTask agentic analysis bridge — runs non-blocking on plugin side
        ...
        case "randomNote":
          return await randomNoteAction(app);

        // [Claude] Task: record mood rating and persist optional notes to a history note
        // Prompt: "submit mood via app.recordMoodRating, write to 'Mood rating history' note via app.createNote"
        // Date: 2026-03-07 | Model: claude-4.6-opus-high-thinking
        case "recordMoodRating":
          return await app.recordMoodRating(args[0]);

        case "saveMoodNote": {
          const moodValue = args[0];
          const moodLabel = args[1];
          const moodNotes = args[2] || "";
