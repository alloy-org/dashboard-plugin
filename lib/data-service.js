...
export async function createQuarterlyPlan(app, quarterInfo) {
  ...
  const existing = await arrayFromFilterNotesResult(app.filterNotes({ query: noteName }));
...
async function _findQuarterlyPlans(app) {
  ...
  // Pass the filterNotes call straight into arrayFromFilterNotesResult (it awaits internally).
  // Do NOT `.then()`-chain the bridge call: its non-spec thenable resolves .then() to undefined.
  const [currentPlans, nextPlans] = await Promise.all([
    arrayFromFilterNotesResult(app.filterNotes({ query: `${current.label} Plan` })),
    arrayFromFilterNotesResult(app.filterNotes({ query: `${next.label} Plan` }))
  ]);
