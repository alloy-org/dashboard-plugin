 ...
 */
import { getCurrentQuarter, getNextQuarter, extractMonthSectionContent, defaultMonthTemplate, FULL_MONTH_NAMES } from "constants/quarters"
import {
  DASHBOARD_NOTE_TAG,
...
export async function createQuarterlyPlan(app, quarterInfo) {
  ...
  let template = _defaultQuarterlyTemplate(label, quarter);
...
// [Claude] Task: pick a random task-domain note updated within the last month and navigate to it
...
export async function randomNoteAction(app) {
  const oneMonthAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
  const notes = await app.filterNotes({ group: "task" });
  if (!notes || notes.length === 0) return;

  const recent = notes.filter(n => {
    if (!n.updated) return true;
    return new Date(n.updated) >= oneMonthAgo;
  });

  const pool = recent.length > 0 ? recent : notes;
  const pick = pool[Math.floor(Math.random() * pool.length)];
  if (pick?.uuid) {
    await app.navigate(`https://www.amplenote.com/notes/${pick.uuid}`);
  }
}
...
async function _readDashboardSettings(app) {
  const keys = ["dashboard_victory-value_config", "dashboard_calendar_config", "dashboard_quotes_config", "dashboard_mood_config"];
  const settings = {};
  ...
  settings[SETTING_KEYS.LLM_PROVIDER]          = app.settings[SETTING_KEYS.LLM_PROVIDER] || '';
  settings[SETTING_KEYS.LLM_API_KEY]           = app.settings[SETTING_KEYS.LLM_API_KEY] || '';
