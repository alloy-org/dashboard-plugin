...
export async function createQuarterlyPlan(app, quarterInfo) {
  ...
  if (prevNotes.length > 0) {
    const prevContent = await app.getNoteContent(prevNotes[0].uuid);
...
export async function fetchQuotes(app, planContent) {
  const apiKey = app.settings[SETTING_KEYS.LLM_API_KEY];
  const provider = app.settings[SETTING_KEYS.LLM_PROVIDER] || "openai";
...
export async function getMonthlyPlanContent(app, noteUUID, monthName) {
  ...
  const sections = await app.getNoteSections(noteUUID);
  ...
  const fullContent = await app.getNoteContent(noteUUID);
