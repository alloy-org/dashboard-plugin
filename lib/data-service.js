...
export async function fetchDashboardData(app) {
  ...
  // Chain tasks and quarterly plans off domain resolution so both are scoped to the active Task Domain.
  ...
  const plansPromise = domainPromise.then(info => _findQuarterlyPlans(app, info));
