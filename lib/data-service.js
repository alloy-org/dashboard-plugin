...
export async function fetchDashboardData(app) {
  ...
  // Chain tasks off domain resolution so they start as soon as the domain UUID is known
  const tasksPromise = domainPromise.then(info => {
    ...
    return _fetchTasksForDomain(app, info.selectedDomainUuid);
  });
...
// --------------------------------------------------------------------------------------
// [Claude claude-sonnet-4-6] Task: detect whether the dashboard plugin was installed recently
// Prompt: "build a method to detect if the dashboard plugin was first installed recently"
// Records the first-load unix timestamp in settings on first call; subsequent calls compare
// the stored value against the threshold. Returns true when first-load is within daysThreshold days.
export async function isPluginRecentlyInstalled(app, daysThreshold = 14) {
  const raw = app.settings?.[SETTING_KEYS.FIRST_INSTALLED_AT];
  if (!raw) {
    const nowSeconds = Math.floor(Date.now() / 1000);
    await app.setSetting(SETTING_KEYS.FIRST_INSTALLED_AT, String(nowSeconds)).catch(() => {});
    app.settings = app.settings || {};
    app.settings[SETTING_KEYS.FIRST_INSTALLED_AT] = String(nowSeconds);
    return true;
  }
  const firstLoadedAt = Number(raw);
  if (!Number.isFinite(firstLoadedAt) || firstLoadedAt <= 0) return false;
  const thresholdMilliseconds = daysThreshold * 24 * 60 * 60 * 1000;
  return (Date.now() - firstLoadedAt * 1000) < thresholdMilliseconds;
}
...
async function _resolveTaskDomains(app) {
  ...
  if (!hasDomains || isStale) {
    ...
    logIfEnabled(`[_resolveTaskDomains] getTaskDomains() returned ${freshDomains?.length ?? 0} domains in ${Date.now() - t0}ms`);
...
async function _fetchTasksForDomain(app, domainUuid) {
  ...
  logIfEnabled(`[_fetchTasksForDomain] getTaskDomainTasks returned ${list.length} tasks in ${Date.now() - t0}ms`);
  return list;
}
