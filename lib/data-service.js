 ...
 */
import { FULL_MONTH_NAMES, defaultMonthTemplate, defaultQuarterlyTemplate, defaultWeekTemplate,
  extractMonthSectionContent, getCurrentQuarter, getNextQuarter } from "constants/quarters"
import {
  DASHBOARD_NOTE_TAG,
...
export async function createQuarterlyPlan(app, quarterInfo) {
  ...
  const template = defaultQuarterlyTemplate(label, quarter);
...
//   - {Promise<Object>} domainPromise - Resolves to { domains, selectedDomainUuid }
//   - {Promise<Object>} plansPromise - Resolves to the { current, next } quarterly plans
...
async function _readDashboardSettings(app) {
  ...
  settings[SETTING_KEYS.DEBUG_CONSOLE]         = app.settings[SETTING_KEYS.DEBUG_CONSOLE] || '';
