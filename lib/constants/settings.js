...
// The date esbuild produced the loaded bundle, as YYYY-MM-DD. Both builds define it; a bundle compiled by anything
// that does not (a bare jest run, for example) leaves the define unreplaced, so fall back to an explicit "unknown".
...
export const SETTING_KEYS = {
  ...
  RENDER_BREADCRUMB: "dashboard_render_breadcrumb",
  ...
  QUARTERLY_PLAN_TOGGLES: "dashboard_quarterly_plan_toggles",
};
...
// ---------------------------------------------------------------------------------------
// @desc The registry icon for a widget. Returns null for an id absent from the registry (a popup or
//   test-only widget) so callers such as WidgetWrapper can fall back rather than throw.
// [Claude claude-opus-5 (1M context)] Task: tolerate an unregistered widgetId now that WidgetWrapper
//   resolves its own icon from the registry
export function widgetIconFromId(widgetId) {
  return widgetDataFromId(widgetId)?.icon || null;
}
...
// ---------------------------------------------------------------------------------------
// @desc The fixed tagline a widget shows beneath its title, for the widgets whose subtitle never
//   changes. Widgets with a subtitle that varies (a date, a chosen note name) pass their own.
// @returns {string|null} The registry's visibleSubtitle, or null when the widget has none.
// [Claude claude-opus-5 (1M context)] Task: registry source of truth for static widget subtitles
export function widgetSubtitleFromId(widgetId) {
  return widgetDataFromId(widgetId)?.visibleSubtitle || null;
}
...
// backwards-compatible initialisation in dashboard-init-service and renderActiveComponents.
