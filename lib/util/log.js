...
// ----------------------------------------------------------------------------------------------
// @desc Log and buffer a message regardless of the Console Logging setting, for output the operator asked
//   for directly (the Debug Console's expression evaluator) rather than incidental instrumentation. Without
//   this, a requested result would vanish whenever logging happened to be off.
// @param {...*} args - Values to log, same shape as logIfEnabled's.
...
// ----------------------------------------------------------------------------------------------
// @desc Append one entry to the rolling buffer and notify listeners.
// @param {Array<*>} args - The logged argument list, stored on the entry for the DebugConsole to format.
function _recordEntry(args) {
