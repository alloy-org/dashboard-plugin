...
//   - `shareAccess`            -> legacy collaborator names (fallback when getPeople is unavailable)
// Notes left without these fields stay private so the "shared" filter is observably selective.
//
// The collaborator identities themselves now come from the dev getPeople() shim (SAMPLE_PEOPLE
// below), whose `sharing.notes` reference these same note UUIDs; `shareAccess` is retained only as
// the no-getPeople fallback exercised by collaboratorNamesFromNoteHandle.
...
// Shared Notes widget; the names match each note's legacy `shareAccess` entries.
