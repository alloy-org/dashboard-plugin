...
export async function fetchDomainOrAllNotesTasks(app, domainUuid, { includeDone = false, maxNotes = MAX_ALL_NOTES_TO_SCAN } = {}) {
  if (domainUuid) {
...
async function _allNotesTasks(app, includeDone, maxNotes) {
  ...
  const settled = await Promise.allSettled(noteHandles.map(handle =>
    app.getNoteTasks({ uuid: handle.uuid }, { includeDone }).then(tasks => ({ handle, tasks: tasks || [] }))));
  const allTasks = [];
  for (const outcome of settled) {
    ...
    const { handle, tasks } = outcome.value;
