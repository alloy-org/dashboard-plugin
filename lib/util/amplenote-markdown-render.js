...
import { renderBlockMarkdown, renderMarkdown } from "util/utility";
...
export function amplenoteMarkdownRender(markdown) {
  return renderWithRichFootnotes(markdown, renderMarkdown);
}

// ------------------------------------------------------------------------
// @desc Block-level counterpart to amplenoteMarkdownRender, for a whole note rather than the single
//   line of a task or tooltip: headings, lists, tables, and code fences become their own elements, and
//   Rich Footnotes are handled identically. As with the inline version, call attachFootnotePopups on
//   the container after inserting the HTML to give the footnote links their popups.
// @param {string} markdown - Raw markdown, potentially with `[^N]:` footnote definitions.
// @returns {string} HTML string ready for `dangerouslySetInnerHTML`.
// [Claude claude-opus-5 (1M context)] Task: render a full note's markdown, footnotes included
// Prompt: "Ensure that there is a note available to display that includes Rich Footnote content"
export function amplenoteMarkdownRenderBlock(markdown) {
  return renderWithRichFootnotes(markdown, renderBlockMarkdown);
}

// ------------------------------------------------------------------------
// @desc Render markdown with the given marked entry point, replacing every `[text][^N]` reference
//   whose definition exists with a footnote link. References become HTML comments before the markdown
//   is rendered, so neither renderer can mangle a half-built link, and are swapped for the finished
//   links afterwards.
// @param {string} markdown - Raw markdown, potentially with `[^N]:` footnote definitions.
// @param {Function} renderer - renderMarkdown (inline) or renderBlockMarkdown (block-level).
// @returns {string} HTML string.
// [Claude claude-opus-5 (1M context)] Task: share footnote handling between the two render depths
// Prompt: "Ensure that there is a note available to display that includes Rich Footnote content"
function renderWithRichFootnotes(markdown, renderer) {
  if (!markdown) return "";
  ...
  let html = renderer(withPlaceholders);
