...
// ------------------------------------------------------------------------------------------
// @desc Request a garbage-collection cycle when the host explicitly exposes window.gc, which
//   Chromium only provides when launched with --js-flags=--expose-gc. Production embeds normally
//   do not expose this function, so callers must still treat memory samples as approximate.
// @returns {boolean} True when a collection was requested, otherwise false.
export function collectGarbageIfAvailable() {
  if (typeof window === "undefined" || typeof window.gc !== "function") return false;
  window.gc();
  return true;
}
