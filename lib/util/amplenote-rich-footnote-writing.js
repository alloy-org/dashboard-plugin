...
import { parsedRichFootnotes, referencedFootnoteIdentifiers } from "util/amplenote-rich-footnotes";
...
//   A task's `content` also carries the `[^n]:` definitions its references point at, often spanning several
//   lines with captions and images. Those bodies are dropped before flattening, and every remaining line break
//   is collapsed, because a line break inside `[...]` ends the label before the link can close.
// @param {string} sourceText - Markdown as stored, possibly containing links, footnote references, footnote
...
export function linkLabelFromMarkdown(sourceText, numbering = null, emptyFallback = "Untitled") {
  ...
  const textWithoutDefinitions = parsedRichFootnotes(rawText).body;
  const withFootnoteMarkers = _renumberedFootnoteReferences(textWithoutDefinitions, numbering);
  const withoutImages = withFootnoteMarkers.replace(/!\[([^\]]*)\]\([^)]*\)/g, "$1");
  const withoutInlineLinks = withoutImages.replace(/\[([^\]]*)\]\([^)]*\)/g, "$1");
  const singleLineText = _flattenedStrayBrackets(withoutInlineLinks).replace(/\s+/g, " ").trim();
...
// @param {object|null} numbering - Tracks which footnote numbers have been handed out so far across one block of
//   content being written (e.g. one project section), so every label rendered into that block shares a single
//   `[^1]`, `[^2]`, ... sequence. Create it once per block with `footnoteNumbering()`, pass the same object to each
//   label, then emit `footnoteDefinitionsMarkdown(numbering)` at the end of the block. It is mutated in place:
//   - {Array<object>} definitions - One entry per footnote assigned so far, in first-cited order, each shaped
//     { identifier, number, text }: `identifier` is the source text's footnote id (the "7" in `[^7]`), `number`
//     is the number it was given in this write, and `text` is the definition body to emit for it
//   - {Map<string, number>} numberByIdentifier - Source footnote id to its assigned number, so a footnote
//     cited by two labels reuses one number and one definition
//   Pass null to drop footnote references from the label entirely, for content that will not carry definitions.
