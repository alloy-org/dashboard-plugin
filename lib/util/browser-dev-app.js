  ...
  _instance = {
    ...
    async filterNotes(options = {}, sortOrder = "") {
      if (options.taskDomainUUID) {
        return _sortFilteredNoteHandles(_SAMPLE_NOTE_HANDLES[options.taskDomainUUID] || [], options.query, sortOrder);
      }
      if (options.query) {
        try {
          const res = await fetch(`/api/note-find?name=${encodeURIComponent(options.query)}`);
          const data = await res.json();
          return _sortFilteredNoteHandles(data?.uuid ? [data] : [], options.query, sortOrder);
        } catch {
          return [];
        }
      }
      // No domain or query — return all sample notes across all domains (mirrors real Amplenote behaviour).
      const allHandles = Object.values(_SAMPLE_NOTE_HANDLES).flat();
      return _sortFilteredNoteHandles(allHandles, null, sortOrder);
    },
