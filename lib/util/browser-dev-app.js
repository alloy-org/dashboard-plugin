...
import { SAMPLE_NOTE_HANDLES, withAsyncIterator } from "./dev-sample-notes.js";
...
function _sortFilteredNoteHandles(noteHandles, query = "", sortOrder = "") {
  ...
  if (!["relevance", "title"].includes(normalizedSortOrder)) return [...noteHandles];
  ...
  _instance = {
    ...
    filterNotes(options = {}, sortOrder = "") {
      ...
      if (options.taskDomainUUID) {
        noteHandles = _sortFilteredNoteHandles(SAMPLE_NOTE_HANDLES[options.taskDomainUUID] || [], options.query, sortOrder);
      } else {
