...
import { readPlanGoals, refreshPlanIntentPossibilities, savePlanGoals } from "plan-wizard/plan-wizard-service";
import { useCallback, useEffect, useRef, useState } from "react";
import { logIfEnabled } from "util/log";

const EMPTY_CONTEXT = { generatedAt: { personal: null, work: null }, goalRecords: [], goals: [], noteUuid: null,
  possibilities: { personal: [], work: [] }, scope: null };
...
export default function usePlanWizard({ app, domainName = null, domainUuid = null, quarter = null, year = null }) {
  ...
  const saveGoals = useCallback(async goals => {
    ...
    setSaveError(null);
    try {
      const context = await savePlanGoals(app, { ...scopeOptions, goals });
      if (!isCurrentRequest(token)) return false;
      setPlanningContext(context);
      return true;
    } catch (writeError) {
      logIfEnabled("[use-plan-wizard] failed to save plan goals", writeError?.message);
