...
import { readPlanGoals, refreshPlanActionProspects, refreshPlanIntentPossibilities, savePlanGoals, savePlanProspects, savePlanQuarterAnswer } from "plan-wizard/plan-wizard-service";
...
export default function usePlanWizard({ app, domainName = null, domainUuid = null, quarter = null, year = null }) {
  ...
  // @returns {Promise<object|null>} The refreshed planning context, or null when superseded or failed.
  const discoverProspects = useCallback(async () => {
    const token = claimRequest();
    setIsDiscovering(true);
    setDiscoveryFailureReason(null);
    try {
      const context = await refreshPlanActionProspects(app, scopeOptions);
      if (!isCurrentRequest(token)) return null;
      setError(null);
  ...
  return { discoverProspects, discoveryFailureReason, error, isDiscovering, isLoading, isRefreshing, isSaving,
    planningContext, refreshPossibilities, reload, saveError, saveGoals, saveProspectDecision, saveProspects,
    saveQuarterAnswer };
}
