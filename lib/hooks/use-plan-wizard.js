...
import { consolidatePlanActionProspects, readPlanGoals, readPlanProjectSources, refreshPlanActionProspects,
  refreshPlanIntentPossibilities, savePlanGoals, savePlanProspects, savePlanQuarterAnswer,
  savePlanThemeJudgement } from "plan-wizard/plan-wizard-service";
...
//   - {object|null} projectSources - What project discovery reads, from projectSourcesFromEvidence: set as soon as a
//     discovery pass has collected its evidence, or by loadProjectSources; null until either has happened.
//   - {boolean} isLoadingProjectSources - True while loadProjectSources is collecting evidence.
...
//   - {Function} loadProjectSources - Collect discovery's sources without a provider call, when none are held yet.
...
//   - {Function} saveProspectDecision - Persist one card's priority or completion without toggling page-wide saving
//     state.
...
export default function usePlanWizard({ app, domainName = null, domainUuid = null, quarter = null, year = null }) {
  ...
  const [projectSources, setProjectSources] = useState(null);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      logIfEnabled(`[use-plan-wizard] loaded ${ scopeKey }`, { noteUuid: context.noteUuid,
        storedReadItemCount: context.intentReading.readItems.length, storedThemeCount: context.intentReading.themes.length,
        workGeneratedAt: context.generatedAt.work, workPossibilityCount: context.possibilities.work.length });
      setError(null);
  ...
  const discoverProspects = useCallback(async requestedTrigger =>
    runExclusivePass("project-discovery", async () => {
      ...
      // ----------------------------------------------------------------------------------------------
      // @desc Adopt the sources this pass collected, so the sources page can show them while the provider works.
      // @param {object} progress - { projectSources } reported by refreshPlanActionProspects.
      const handleProgress = progress => {
        if (isCurrentRequest(token)) setProjectSources(progress.projectSources);
      };
      try {
        const context = await refreshPlanActionProspects(app, { ...scopeOptions, onProgress: handleProgress, triggerAction });
        if (!isCurrentRequest(token)) return null;
        setError(null);
  ...
  // ----------------------------------------------------------------------------------------------
  // @desc Collect the sources discovery reads for the current scope, for a sources page opened before any discovery
  //   pass ran in this session. Sources already held, or a pass already collecting them, are left alone.
  // @returns {Promise<void>} Resolves once the sources are held or the attempt has failed.
  // Scope is checked by key rather than through the request token, since claiming one would supersede a save or
  //   discovery pass the user started, and this read has no result worth discarding one for.
  const loadProjectSources = useCallback(async () => {
    if (projectSources || isDiscovering) return;
    await runExclusivePass("project-sources", async () => {
      const requestedScopeKey = scopeKey;
      setIsLoadingProjectSources(true);
      try {
        const sources = await readPlanProjectSources(app, scopeOptions);
        if (scopeKeyRef.current === requestedScopeKey) setProjectSources(sources);
      } catch (sourcesError) {
        logIfEnabled("[use-plan-wizard] could not collect project sources", sourcesError?.message);
      } finally {
        if (scopeKeyRef.current === requestedScopeKey) setIsLoadingProjectSources(false);
      }
    });
  }, [app, isDiscovering, projectSources, runExclusivePass, scopeKey]);
  ...
  // ----------------------------------------------------------------------------------------------
  // @desc Persist one or more card-local decisions without entering the page-wide isSaving state.
  // @param {Array<object>} prospects - Decision records shaped for savePlanProspects.
  // @param {object} [decisionOptions] - { shouldPublish } republishes the plan note after the save, without
  //   waiting on it, for a decision such as Complete that suggestion surfaces read from the note.
  ...
  const saveProspectDecision = useCallback(async (prospects, decisionOptions = {}) => {
    const requestedScopeKey = scopeKey;
    setSaveError(null);
    try {
      ...
      setPlanningContext(context);
      if (decisionOptions.shouldPublish) publishPlanNote(context);
      return true;
    } catch (writeError) {
  ...
  return { consolidateProspects, discoverProspects, discoveryFailureReason, error, intentReading, isConsolidating,
    ...
    saveQuarterAnswer, saveThemeJudgement, viewQuarterlyPlan };
}
