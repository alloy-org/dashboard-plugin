...
//     stored reading is current; readItems arrive at inferring, themes only at ready.
...
export default function usePlanWizard({ app, domainName = null, domainUuid = null, quarter = null, year = null }) {
  ...
  const refreshPossibilities = useCallback(async () => runExclusivePass("intent-inference", async () => {
    ...
    setReadingProgress({ phase: "reading", readItems: [] });
    // ----------------------------------------------------------------------------------------------
    // @desc Adopt the notes the service has finished reading, so the reading page can list them while the provider
    //   is still working. A superseded pass's notes are ignored like its result.
    // @param {object} progress - { phase, readItems } reported by refreshPlanIntentPossibilities.
    const handleProgress = ({ readItems }) => {
      if (isCurrentRequest(token)) setReadingProgress({ phase: "inferring", readItems });
    };
    try {
  ...
  const intentReading = readingProgress
    ? { ...planningContext.intentReading, phase: readingProgress.phase, readItems: readingProgress.readItems, themes: [] }
