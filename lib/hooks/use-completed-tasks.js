...
// ------------------------------------------------------------------------------------------
// Fetches completed tasks for each day of the selected week in parallel via
...
export default function useCompletedTasks(app) {
    ...
    try {
      const results = await Promise.all(
        boundaries.map(async ({ from, to, dateKey }) => {
          const tasks = await app.getCompletedTasks(from, to);
