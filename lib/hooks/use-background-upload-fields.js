...
export default function useBackgroundUploadFields({ app, initialImageUrl = '', initialMode = 'cover' } = {}) {
  ...
  const handleFileUpload = useCallback(async (file) => {
    if (!file || !file.type.startsWith('image/')) {
      logIfEnabled('[bg-upload] rejected file (not an image or missing):', file?.type);
      return;
    }
    logIfEnabled('[bg-upload] starting upload for:', file.name, file.type, file.size, 'bytes');
    setUploading(true);
    try {
      const dataURL = await fileToDataURL(file);
      logIfEnabled('[bg-upload] file converted to data URL, length:', dataURL.length);
      const pluginNoteUUID = app.context?.pluginUUID || 'dev-dashboard-plugin-uuid';
      const fileURL = await app.attachNoteMedia({ uuid: pluginNoteUUID }, dataURL);
      logIfEnabled('[bg-upload] uploadBackgroundImage returned URL:', fileURL);
      if (fileURL) {
        setBackgroundImageUrl(fileURL);
      } else {
        logIfEnabled('[bg-upload] uploadBackgroundImage returned falsy URL — upload may have failed silently');
      }
    } catch (err) {
      logIfEnabled('[bg-upload] upload FAILED with exception:', err);
    } finally {
      setUploading(false);
      logIfEnabled('[bg-upload] upload flow complete');
    }
  }, []);
