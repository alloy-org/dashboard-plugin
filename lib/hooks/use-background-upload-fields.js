...
export default function useBackgroundUploadFields({ initialImageUrl = '', initialMode = 'cover' } = {}) {
  ...
  const handleFileUpload = useCallback(async (file) => {
    if (!file || !file.type.startsWith('image/')) {
      console.warn('[bg-upload] rejected file (not an image or missing):', file?.type);
      return;
    }
    console.log('[bg-upload] starting upload for:', file.name, file.type, file.size, 'bytes');
    setUploading(true);
    try {
      const dataURL = await fileToDataURL(file);
      console.log('[bg-upload] file converted to data URL, length:', dataURL.length);
      const fileURL = await callPlugin('uploadBackgroundImage', dataURL);
      console.log('[bg-upload] callPlugin returned URL:', fileURL);
      if (fileURL) {
        setBackgroundImageUrl(fileURL);
      } else {
        console.warn('[bg-upload] callPlugin returned falsy URL — upload may have failed silently');
      }
    } catch (err) {
      console.error('[bg-upload] upload FAILED with exception:', err);
    } finally {
      setUploading(false);
      console.log('[bg-upload] upload flow complete');
    }
  }, []);
  ...
  const handleRemoveImage = useCallback(async () => {
    ...
    console.log('[bg-upload] removing background image');
    try {
      await callPlugin('removeBackgroundImage');
      setBackgroundImageUrl('');
      console.log('[bg-upload] background image removed');
    } catch (err) {
      console.error('[bg-upload] remove FAILED:', err);
    }
  }, []);
