...
//
// [Claude] Task: format date values to ISO date keys using local timezone
// Date: 2026-02-23 | Model: claude-sonnet-4-6
// Modified: 2026-03-08 | Model: claude-4.6-opus-high-thinking
function formatDateKey(dateValue) {
  let date;
  if (dateValue instanceof Date) {
    date = dateValue;
  } else {
    const ms = typeof dateValue === 'number' && dateValue < 1e10 ? dateValue * 1000 : dateValue;
    date = new Date(ms);
  }
...
function groupTasksByDate(tasks, getDateFn, filterFn = () => true) {
  ...
  logIfEnabled(`Analyzed ${ tasksAnalyzed } tasks, grouped into ${ Object.keys(grouped).length } date keys in ${ new Date() - startAt }ms`);
  return grouped;
}
...
export default function useDomainTasks() {
  ...
  const buildAgendaTasksByDate = useCallback((currentDateIso) => {
    ...
    logIfEnabled(`Agenda will reviewed ${ allOpenTasks.length } open tasks`, allOpenTasks.map(t => ({deadlineText: t.deadline ? formatDateKey(t.deadline) : "N/A",
      startText: t.startAt ? formatDateKey(t.startAt) : "None", ...t })));
    logIfEnabled(`${ pendingTasks.length } relevant (with startAt)`, pendingTasks);
  ...
  logIfEnabled(`useDomainTasks initialized with active domain ${ activeTaskDomain } in ${ new Date() - startAt }ms`);
  return {
    taskDomains,
    activeTaskDomain,
    tasksFetchedAt,
