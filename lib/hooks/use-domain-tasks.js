...
export default function useDomainTasks() {
  ...
  const buildAgendaTasksByDate = useCallback((currentDateIso) => {
    ...
    const pendingTasks = allOpenTasks
      .filter(task => task.startAt || task.deadline)
      .sort((a, b) => (a.startAt || a.deadline) - (b.startAt || b.deadline));

    console.log(`Agenda: reviewed ${ allOpenTasks.length } open tasks`,
      allOpenTasks.map(t => ({ deadlineText: t.deadline ? formatDateKey(t.deadline) : "N/A", startText: t.startAt ? formatDateKey(t.startAt) : "None", ...t })));
    console.log(`${ pendingTasks.length } relevant (with startAt)`, pendingTasks);

    const groupedByDate = {};
    pendingTasks.forEach(task => {
      let dateKey = task.startAt ? formatDateKey(task.startAt) : null;
      dateKey = dateKey || (task.deadline ? formatDateKey(task.deadline) : null);
