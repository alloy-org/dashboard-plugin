...
function groupTasksByDate(tasks, getDateFn, filterFn = () => true) {
  ...
  console.debug(`Analyzed ${ tasksAnalyzed } tasks, grouped into ${ Object.keys(grouped).length } date keys in ${ new Date() - startAt }ms`);
  return grouped;
}
...
export default function useDomainTasks() {
  ...
  const buildAgendaTasksByDate = useCallback((currentDateIso) => {
    ...
    console.log(`Agenda will reviewed ${ allOpenTasks.length } open tasks`, allOpenTasks.map(t => ({deadlineText: t.deadline ? formatDateKey(t.deadline) : "N/A",
