...
async function fetchAllDomainNotes(app, taskDomainUUID, excludedUuids = new Set()) {
  ...
  for await (const noteHandle of (await app.filterNotes({ group: "taskLists", taskDomainUUID }, "changed"))) {
    if (excludedUuids.has(noteHandle?.uuid)) continue;
    logIfEnabled(`[RecentNotes] Scanning note ${ noteHandle.uuid } in domain ${ taskDomainUUID } (updated ${ noteHandle.updated }, changed ${ noteHandle.changed })`);
