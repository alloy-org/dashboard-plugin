...
async function fetchAllDomainNotes(app, excludedUuids = new Set()) {
  ...
  for (const domain of taskDomains) {
    ...
    for await (const noteHandle of (await app.filterNotes({ taskDomainUUID: domain.id }, "changed"))) {
...
async function staleCandidatesFromScan(app, state) {
  ...
  let notes = await fetchAllDomainNotes(app, recentlyVisited);
  let visitFilter = recentlyVisited;
  if (notes.length === 0 && state.staleCandidates.length === 0) {
    visitFilter = new Set();
    notes = await fetchAllDomainNotes(app, visitFilter);
  }
