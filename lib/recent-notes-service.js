...
export async function findStaleTaskNotes(app, seed, options = {}) {
  ...
  if (!options.forceRefresh && noteContext.state.selectedNotes.length >= maxNotes) {
    return hydratedSelectedNoteEntriesFromState(app, noteContext.state.selectedNotes.slice(0, maxNotes));
  }
  ...
  state = normalizedRecentNotesState({
    ...
    selectedNotes,
    staleCandidates,
    visitedByDay: mapWithUuidsForDate(state.visitedByDay, state.dateKey, scanResult.visitedUuids,
      MAX_VISITED_UUIDS_PER_DAY),
  }, state.dateKey);
