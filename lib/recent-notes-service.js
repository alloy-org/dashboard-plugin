...
// ----------------------------------------------------------------------------------------------
// @desc Fetch task-list note handles for the active task domain, capped to protect mobile clients.
// @param {object} app - Amplenote app bridge.
// @param {string|null} taskDomainUUID - Active task domain UUID from the dashboard; when absent, all
//   task-list notes are scanned (the "All Notes" fallback) rather than returning nothing.
// @param {Set<string>} excludedUuids - Note UUIDs that do not need task fetching.
// [Claude claude-opus-4-8 (1M context)] Task: scan all task-list notes when no task domain can be found
// Prompt: "fall back to not specifying any task domain UUID when no domain can be found"
async function fetchAllDomainNotes(app, taskDomainUUID, excludedUuids = new Set()) {
  const seen = new Set();
  const notes = [];

  const filterOptions = taskDomainUUID ? { group: "taskLists", taskDomainUUID } : { group: "taskLists" };
  for await (const noteHandle of (await app.filterNotes(filterOptions, "changed"))) {
...
// @param {number} params.maxNotes - Max notes to return.
// @param {number} params.seed - Deterministic selection seed.
// @param {string|null} params.taskDomainUUID - Active task domain UUID.
...
// [OpenAI gpt-5.4] Task: replace positional Recent Notes parameters with named options and seed-aware cache reuse
...
export async function findStaleTaskNotes({ app, maxNotes = MAX_NOTES, seed, taskDomainUUID = null }) {
  const noteContext = await resolveRecentNotesNoteContext(app);
  let state = stateForTaskDomain(noteContext.state, taskDomainUUID, noteContext.state.dateKey);
  const savedSelectedNotes = state.selectedNotes;
  if (state.selectionSeed === seed && savedSelectedNotes.length >= Math.min(MIN_NOTES, maxNotes)) {
    return hydratedSelectedNoteEntriesFromState(app, savedSelectedNotes.slice(0, maxNotes));
  }

  const scanResult = await staleCandidatesFromScan(app, state, taskDomainUUID);
...
function normalizedRecentNotesState(value, dateKey) {
  ...
  return {
    ...
    selectionSeed: Number.isFinite(source.selectionSeed) ? source.selectionSeed : null,
    ...
    taskDomainUUID: typeof source.taskDomainUUID === 'string' && source.taskDomainUUID.length > 0 ? source.taskDomainUUID : null,
    visitedByDay: normalizedDateUuidMap(source.visitedByDay, dateKey),
  };
}

// ----------------------------------------------------------------------------------------------
// @desc Reset persisted Recent Notes state when the active dashboard task domain changes.
// @param {object} state - Current normalized state from the daily archive note.
// @param {string|null} taskDomainUUID - Active task domain UUID for the current render.
// @param {string} dateKey - Current local date key.
// [OpenAI gpt-5.4] Task: prevent same-day Recent Notes cache reuse across domain switches
...
function stateForTaskDomain(state, taskDomainUUID, dateKey) {
  if ((state?.taskDomainUUID || null) === (taskDomainUUID || null)) return state;
  return normalizedRecentNotesState({ selectionSeed: null, taskDomainUUID }, dateKey);
}
...
async function resolveRecentNotesNoteContext(app) {
  ...
  if (noteHandle?.uuid) {
    ...
    return { noteHandle: { uuid: noteHandle.uuid }, state: recentNotesStateFromNoteContent(content, dateKey) };
  }
...
// @param {string|null} taskDomainUUID - Active task domain UUID from the dashboard.
...
// [Claude claude-sonnet-4-6] Task: break deadlock when visitedByDay covers all notes and staleCandidates is empty
// Prompt: "component no longer finds notes in dev environment"
async function staleCandidatesFromScan(app, state, taskDomainUUID) {
  const recentlyVisited = uuidSetFromDateMap(state.visitedByDay);
  let notes = await fetchAllDomainNotes(app, taskDomainUUID, recentlyVisited);
  let visitFilter = recentlyVisited;
  if (notes.length === 0 && state.staleCandidates.length === 0) {
    visitFilter = new Set();
    notes = await fetchAllDomainNotes(app, taskDomainUUID, visitFilter);
  }
  const staleCandidates = [];
  const visitedUuids = [];
  const knownCandidates = new Set(state.staleCandidates.map(entry => entry.uuid));
  for (const noteHandle of notes) {
    if (!noteHandle?.uuid || visitFilter.has(noteHandle.uuid)) continue;
    try {
