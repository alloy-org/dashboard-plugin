...
const TABLE_HEADER = "| DateTime | Approved/rejected | Task suggested | Proposed agenda theme or prompt |\n"
  + "| --- | --- | --- | --- |";
...
// ----------------------------------------------------------------------------------------------
// @desc Normalize a value for a table cell: newlines would break the row apart, so they collapse to spaces.
//   Pipes are left intact here — they are escaped only when the row is serialized, so a value that round-trips
//   through the note is never escaped twice.
// @param {string} text - Raw cell value.
// @returns {string}
function _cleanCellValue(text) {
  return String(text ?? "").replace(/\r?\n/g, " ").trim();
}

// ----------------------------------------------------------------------------------------------
// @desc Escape a stored value for writing into a markdown table cell.
// @param {string} text - Stored (unescaped) cell value.
// @returns {string}
function _escapedCell(text) {
  return _cleanCellValue(text).replace(/\|/g, "\\|");
}

// ----------------------------------------------------------------------------------------------
// @desc Render one month's heading plus its decision table.
...
function _monthSectionMarkdown(monthKey, rows) {
  const rowLines = rows.map(row => `| ${ _escapedCell(row.decidedAtLabel) } | ${ _escapedCell(row.decisionLabel) } `
    + `| ${ _escapedCell(row.taskTitle) } | ${ _escapedCell(row.themeLabel) } |`);
  return `## ${ monthLabelFromMonthKey(monthKey) }\n\n${ [TABLE_HEADER, ...rowLines].join("\n") }\n`;
}
...
async function _resolveDecisionNote(app, domainName) {
  ...
  if (existingHandle?.uuid) {
    ...
    return { monthsByKey: _monthsFromNoteContent(content), noteHandle: existingHandle };
  }
...
function _rowFromDecision(decision) {
  const taskTitle = _cleanCellValue(decision?.taskTitle);
  ...
  return { decidedAtLabel: `${ formatDateKey(decidedAt) } ${ clockLabel }`,
    ...
    taskTitle, themeLabel: _cleanCellValue(decision.themeLabel) };
}
...
function _rowsFromSectionBody(sectionBody, monthKey) {
  ...
  for (const line of sectionBody.split("\n")) {
    ...
    // Split on unescaped pipes only, then unescape, so titles containing "\|" survive the round trip.
    const cells = trimmed.replace(/^\|/, "").replace(/\|$/, "").split(/(?<!\\)\|/)
      .map(cell => cell.trim().replace(/\\\|/g, "|"));
    if (cells.length < TABLE_COLUMN_COUNT) continue;
    const isSeparatorRow = /^-+$/.test(cells[0].replace(/\s/g, ""));
    if (isSeparatorRow || cells[0].toLowerCase() === "datetime") continue;
    if (!cells[2]) continue;
    rows.push({ decidedAtLabel: cells[0], decisionLabel: cells[1], monthKey, taskTitle: cells[2], themeLabel: cells[3] });
  }
  return rows;
}
