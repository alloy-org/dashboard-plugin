...
function useDreamTaskActions(app, defaultNoteUUID, noteUUID, setTasks) {
  ...
  const onScheduleTask = useCallback(async (event, task) => {
    ...
    const result = await scheduledDreamTaskResultFromStartAt(app, defaultNoteUUID, startAt, task);
    if (!result.taskUuid) {
    ...
    await patchTaskMetadata(task, { taskUuid: result.taskUuid });
    const key = _taskKey(task);
    setTasks(previous => (previous || []).map(candidate => (
    ...
    if (typeof window !== "undefined") {
      const taskUpdateEvent = new CustomEvent(DASHBOARD_TASKS_UPDATED_EVENT, {
        content: task.title, noteUUID: result.noteUUID || task.noteUUID || defaultNoteUUID, startAt, taskUuid: result.taskUuid,
      });
