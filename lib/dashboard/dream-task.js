...
const NO_CONFIG_ERROR_CODES = new Set(['llm_error', 'parse_error', 'invalid_api_key']);
...
function renderTaskList(h, dreamTasks, maxTasks, headerActions, onTaskClick, listRef, llmAttributionFooter,
  ...
  const cards = displayDreamTasks.map((dreamTask, i) => {
    ...
    logIfEnabled("Rendering dreamTask", { ...dreamTask, isClickable, isDismissing, task });
...
export default function DreamTaskWidget({ app, gridHeightSize, gridWidthSize, onOpenSettings, providerApiKey,
  ...
  }, [app, recordTaskUuids, taskGenerateCount, proposedTasksNoteName, providerName, hasLlmConfig]);
  ...
  }, [app, hasLlmConfig, providerEm, providerApiKey, envApiKey, providerName]);
  ...
  if (!hasLlmConfig && !loading && (!tasks || tasks.length === 0) && (isInitialPreFetch || isLlmFallbackFailure)) {
