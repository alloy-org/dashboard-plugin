...
import { _loadSeenUuidsMap, _maxTasksFromGrid, _recordSeenUuids, _taskGenerateCount, _todayProposedTasksNoteName,
  applyDreamTaskAnalysisResult, completeDreamTask, fetchDreamTaskSuggestions, handleOpenSettings, handleTaskClick,
...
import { buildAvailableTimeSlots, fetchSchedulingOccupancy, startAtSecondsFromDateAndMinutes } from "dream-task-schedule";
        ...
        return h('div', { key: taskKey || i, className: `dream-task-card${ task.isExisting ? '' : ' dream-task-card--invented' }${ isDismissing ? ' dream-task-card--dismissing' : '' }` },
          ...
          h('div', { className: 'dream-task-card-actions' },
            ...
            h('a', {
              href: '#',
              className: 'dream-task-card-action',
              onClick: (event) => onSchedule(event, task),
...
function useDreamTaskActions(app, defaultNoteUUID, noteUUID, setTasks) {
  ...
  // [Claude GPT-5.5] Task: route Complete action through Amplenote task completion helper
  // Prompt: "when the user clicks Complete link on a task that already exists, use app.updateTask"
  const onCompleteTask = useCallback(async (event, task) => {
    event.preventDefault();
    fireConfettiForTask(event);
    await completeDreamTask(app, noteUUID, task);
    removeTaskAfterFade(task);
  }, [app, fireConfettiForTask, noteUUID, removeTaskAfterFade]);
  ...
  // Prompt: "add a Schedule link that pops up a dialog to pick a time not occupied"
  const onScheduleTask = useCallback(async (event, task) => {
    event.preventDefault();
    ...
    if (!task.isExisting || !taskUuid) {
      const targetNoteUuid = task.noteUUID || defaultNoteUUID;
      if (!targetNoteUuid) {
        logIfEnabled('[DreamTask] Schedule: no note available to create invented task');
        await app.alert('Could not schedule: no target note is available for this suggestion.');
        return;
      }
      taskUuid = await app.insertTask({ uuid: targetNoteUuid }, { content: task.title });
    }
    if (taskUuid) await app.updateTask(taskUuid, { startAt });
    removeTaskAfterFade(task);
  }, [app, defaultNoteUUID, removeTaskAfterFade]);
