...
//   can reach paths 1 (cached note), 2 (Ample Agent Pro callPlugin), or 3 (direct llmPrompt). Logged
//   once per significant change so the user can read it in DebugConsole without scrolling through
//   per-render duplicates.
// @param {object} app - Amplenote app bridge.
// @param {string|null} providerEm - Dashboard-selected provider bucket id (e.g. 'openai').
// @param {string|undefined} providerApiKey - Per-provider API key forwarded from dashboard config.
// @param {string} envApiKey - Dev-only OPEN_AI_ACCESS_TOKEN value (length only logged).
// @returns {object} Structured snapshot for `logIfEnabled` consumption.
// [Claude claude-opus-4.7] Task: surface all DreamTask fallback inputs in a single log line
// Prompt: "add adequate debugging to understand why all of these failed" (cache + callPlugin + llmPrompt)
...
// Prompt: "if no AI API key is present, suggest visiting amplenote.com/plugins/ample_agent_pro"
  ...
  return h(WidgetWrapper, { title: widgetTitleFromId(WIDGET_ID), icon: widgetIcon(), widgetId: WIDGET_ID },
    h('div', { className: 'dream-task-no-config' },
      ...
      h('p', { className: 'dream-task-no-config-text' },
        'No API key? Install ',
        h(CopyLink, { url: 'https://www.amplenote.com/plugins/ample_agent_pro', className: 'dream-task-settings-link' },
          "Amplenote\u2019s AI integration"
        ),
        ' to enable suggested tasks and 10+ other AI features \u2014 no API key required.'
      ),
      h('p', { className: 'dream-task-no-config-text' },
        h('a', { href: '#', className: 'dream-task-settings-link', onClick: onSettingsClick }, 'Configure AI settings \u2192'),
        h('br'),
        h('a', { href: "https://www.amplenote.com/subscriptions/new", className: 'dream-task-settings-link' }, "View Founder Subscription \u2192"),
        h('br'),
        h('a', { href: "https://www.amplenote.com/plugins/ample_agent_pro", className: 'dream-task-settings-link' }, "View Ample Agent Pro plugin \u2192"),
      )
    )
  );
}
...
// @param {Function} h - React createElement.
// @param {Array<object>} dreamTasks - DreamTask suggestion objects from the service.
...
// @param {object} actionHandlers - onPreserve, onComplete, onRemove, onSchedule, dismissingTaskKeys, expandedExplanationKeys, onToggleExplanation.
...
// [Claude claude-sonnet-4-6] Task: add per-card and total render timing via logIfEnabled
// Prompt: "Break down in logIfEnabled what is consuming so much time rendering dream-task.js"
function renderTaskList(h, dreamTasks, maxTasks, headerActions, onTaskClick, listRef, llmAttributionFooter,
    actionHandlers) {
  ...
  const cards = displayDreamTasks.map((dreamTask, i) => {
    ...
    logIfEnabled("Rendering dreamTask", { title: dreamTask.title, isClickable, isDismissing, nativeTask: !!task });
    ...
    logIfEnabled(`[DreamTask] card ${i} markdown render: ${(performance.now() - mdStart).toFixed(1)}ms`);

    return h('div', { key: taskKey || i, className: `dream-task-card${ dreamTask.isExisting ? '' : ' dream-task-card--invented' }${ isDismissing ? ' dream-task-card--dismissing' : '' }` },
      h('div', { className: 'dream-task-card-header' },
        ...
        renderDreamTaskKindBadge(h, isClickable, dreamTask),
        h('span', {
      ...
      h('div', { className: 'dream-task-card-actions' },
        ...
        }, dreamTask.preserveThroughTomorrow ? '🚫 Remove preserve status' : '📎 Preserve a day'),
        h('a', {
          href: '#',
          className: 'dream-task-card-action',
  ...
  logIfEnabled(`[DreamTask] card map total (${displayDreamTasks.length} cards): ${(performance.now() - mapStart).toFixed(1)}ms`);
...
// ----------------------------------------------------------------------------------------------
// @desc Encapsulate preserve/complete/remove/schedule/toggle-explanation action state and handlers.
// @param {object} app - Amplenote app bridge.
// @param {string|null} defaultNoteUUID - Fallback note for invented tasks (used by schedule).
...
// [Claude claude-opus-4.7] Task: add onScheduleTask and accept defaultNoteUUID for invented-task creation
...
function useDreamTaskActions(app, defaultNoteUUID, noteUUID, setTasks) {
  ...
  // [Claude GPT-5.5] Task: route Complete action through shared task completion utility
  ...
  // [Claude claude-opus-4.7] Task: prompt for date/time, write startAt, creating invented tasks first
  // Prompt: "add a Schedule link that pops up a dialog to pick a time not occupied"
  const onScheduleTask = useCallback(async (event, dreamTask) => {
    ...
    // Inform other components that there is a new task in town
...
export default function DreamTaskWidget({ app, gridHeightSize, gridWidthSize, onOpenSettings, providerApiKey,
  ...
  // [Claude claude-opus-4.7] Task: drop hasLlmConfig early-return so cache + Ample-Agent-Pro paths can run
  ...
  // [Claude claude-opus-4.7] Task: replace prevHasConfigRef + separate mount effect with a single
  //   effect keyed on hasLlmConfig. Runs analysis on mount and re-runs when config becomes available
  //   if we still have nothing to show. The cache + callPlugin paths can fire on mount with no key.
  // Prompt: "remove prevHasConfigRef ... refs are code smells"
  useEffect(() => {
  ...
  // [Claude claude-opus-4.7] Task: render no-config UI only when no fallback path produced suggestions
  // Prompt: "if no API key, the widget should still try cached note + Ample Agent Pro callPlugin"
  // Deferred render check (instead of an early-return gate) so analysis can hit paths 1/2 first.
