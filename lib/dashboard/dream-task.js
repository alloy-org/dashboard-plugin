...
// ---------------------------------------------------------------------------
// App-based note lifecycle (used in dev mode)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// [Claude] Task: run analysis — dev mode uses app for note lifecycle, prod uses callPlugin
// Prompt: "extract all possible functionality to local functions"
// Date: 2026-03-14 | Model: claude-4.6-opus-high-thinking
async function runDreamTaskAnalysis(app) {
  if (IS_DEV_ENVIRONMENT && app) {
    ...
    const result = await callPlugin('dreamTaskAnalyze');
  ...
  return await callPlugin('dreamTaskAnalyze');
}
...
async function handleOpenNote(noteUUID) {
  if (!noteUUID) return null;
  return await navigateToNote(noteUUID);
}
...
// Date: 2026-03-14 | Model: claude-4.6-opus-high-thinking
export default function DreamTaskWidget({ gridHeightSize = 1, settings, onOpenSettings, app }) {
  ...
  const onOpenNote = async (e) => {
    e.preventDefault();
    const result = await handleOpenNote(noteUUID);
