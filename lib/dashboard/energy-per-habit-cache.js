...
export function monthTableMarkdown(rows) {
  const header = "| Task | Completions | Weeks streak | Mood on done days | Mood on off days | Last completed |\n"
    + "| --- | --- | --- | --- | --- | --- |";
  const body = (rows || []).map(row =>
    `| ${escapeCell(row.label)} | ${row.count} | ${row.weekStreak || 0} | ${cellFromMoodArray(row.doneMoods)} | ${cellFromMoodArray(row.offMoods)} | ${row.mostRecentDayKey || ''} |`
  ).join('\n');
  return body ? `${header}\n${body}` : header;
}
...
function rowsFromSectionBody(sectionBody) {
  ...
  for (const line of sectionBody.split('\n')) {
    ...
    const mostRecentDayKey = /^\d{4}-\d{2}-\d{2}$/.test(cells[5] || '') ? cells[5] : null;
