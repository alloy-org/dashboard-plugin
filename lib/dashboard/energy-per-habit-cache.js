...
// ------------------------------------------------------------------------------------------
// @desc Parse a single month section's table rows into cache row objects. Tolerates legacy 4-column rows
//   (no "Weeks streak" column) written before that column existed by defaulting weekStreak to 0.
...
function rowsFromSectionBody(sectionBody) {
  ...
  for (const line of sectionBody.split('\n')) {
    ...
    // 5-column (current): Task | Completions | Weeks streak | done | off. 4-column (legacy): no streak column.
    const hasStreak = cells.length >= 5;
    const weekStreak = hasStreak ? (parseInt(cells[2], 10) || 0) : 0;
    const doneMoods = moodArrayFromCell(cells[hasStreak ? 3 : 2]);
    const offMoods = moodArrayFromCell(cells[hasStreak ? 4 : 3]);
    rows.push({ label, key: habitKeyFromContent(label), count, weekStreak, doneMoods, offMoods });
  }
  return rows;
}
