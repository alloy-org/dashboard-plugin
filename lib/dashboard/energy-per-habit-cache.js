...
import { habitKeyFromLabel, monthKeyFromMonthLabel, monthLabelFromMonthKey } from "energy-per-habit-analysis";
...
export const HABIT_CACHE_FORMAT_VERSION = 2;
...
//   when a section is targeted; the full-note builder adds headings itself). The "Weeks streak" column stores
//   the consecutive-weeks-completed run as of the end of the month, so streaks survive beyond the fetched year.
// @param {Array<{label, count, weekStreak, doneMoods, offMoods}>} rows
// @returns {string} Markdown table body.
export function monthTableMarkdown(rows) {
  const header = "| Task | Completions | Weeks streak | Mood on done days | Mood on off days |\n"
    + "| --- | --- | --- | --- | --- |";
  const body = (rows || []).map(row =>
    `| ${escapeCell(row.label)} | ${row.count} | ${row.weekStreak || 0} | ${cellFromMoodArray(row.doneMoods)} | ${cellFromMoodArray(row.offMoods)} |`
  ).join('\n');
  return body ? `${header}\n${body}` : header;
}

// ------------------------------------------------------------------------------------------
// @desc Parse a single month section's table rows into cache row objects. Rows are the 5-column
//   format: Task | Completions | Weeks streak | Mood on done days | Mood on off days.
// @param {string} sectionBody - Markdown between a month heading and the next heading.
// @returns {Array<{label, key, count, weekStreak, doneMoods, offMoods}>}
function rowsFromSectionBody(sectionBody) {
  ...
  for (const line of sectionBody.split('\n')) {
    ...
    const weekStreak = parseInt(cells[2], 10) || 0;
    const doneMoods = moodArrayFromCell(cells[3]);
    const offMoods = moodArrayFromCell(cells[4]);
    rows.push({ label, key: habitKeyFromLabel(label), count, weekStreak, doneMoods, offMoods });
  }
  return rows;
}
...
export async function loadHabitCache(app) {
  ...
  return { formatVersion, noteHandle, monthsByKey: monthsFromNoteContent(rawContent), rawContent };
}
