...
// Prompt: "record whether each line was auto-populated or human-edited; restore only human-edited
//   on load, then asynchronously re-confirm auto lines from current tasks and events"
// @description Parses the body of a Day Sketch note into entries and a set of human-edited hours.
//   Each hour line is expected in the format "9am: some text". A "human-edited: 6,9,14" footer
//   line identifies which hours were directly edited by the user (never auto-populated again).
// @param {string|null} content - Raw note body text
// @returns {{ entries: Object<number,string>, humanEditedHours: Set<number> }}
...
// ------------------------------------------------------------------------------------------
// [Claude claude-sonnet-4-6] Task: persist human-edited hour tracking to note footer
// Prompt: "record whether each line was auto-populated or human-edited; restore only human-edited
//   on load, then asynchronously re-confirm auto lines from current tasks and events"
// @description Serializes the entries map into note body text with a "human-edited: ..." footer
//   listing which hours were directly edited by the user.
// @param {Object<number, string>} entries - Map of hour → user-entered or auto-populated text
// @param {Set<number>} humanEditedHours - Hours the user has directly edited
// @returns {string} Note body with lines like "9am: standup meeting" plus a human-edited footer
...
// ------------------------------------------------------------------------------------------
// [Claude claude-4.6-opus] Task: map calendar event start/end times to DaySketch hour rows
// Prompt: "place calendar events in the day sketch at their scheduled times"
// @description Returns all DaySketch row hours a calendar event should populate.
//   Skips allDay events. Includes every hour the event spans (start hour through end hour).
// @param {Object} event - Calendar event with start/end Date objects
// @returns {number[]} 24-hour integers for candidate DaySketch rows
...
// ------------------------------------------------------------------------------------------
// [Claude claude-sonnet-4-6] Task: prevent prefill from overwriting human-edited DaySketch rows
// Prompt: "if user input changes a line at any point, it is recorded as human-edited and never
//   modified again by auto-population"
// @description Fills blank, non-human-edited rows in an entries object from agenda task timings/text.
// @param {Object<number, string>} entries - Existing hour-row values
// @param {Object[]} tasks - Agenda tasks for the current day
// @param {Set<number>} humanEditedHours - Hours that must never be overwritten by prefill
// @returns {Object<number, string>} New entries object with eligible rows prefilled
...
// Prompt: "if user input changes a line at any point, it is recorded as human-edited and never
//   modified again by auto-population"
// @description Fills blank, non-human-edited rows in an entries object from today's calendar events.
// @param {Object<number, string>} entries - Existing hour-row values
// @param {Array} calendarEvents - Calendar event objects with start/end Date and title
// @param {string|Date} currentDate - Current selected date for filtering events to today
// @param {Set<number>} humanEditedHours - Hours that must never be overwritten by prefill
// @returns {Object<number, string>} New entries object with eligible rows prefilled
...
//   On load, only human-edited rows are restored from the note; auto-populated rows are left
//   blank so the prefill effects re-validate them against current tasks and events.
// @param {Object} params
// @param {Object} params.app - Amplenote app interface
// @param {string} params.noteName - Day Sketch note name for current date
// @returns {Object} State + handlers for entries lifecycle, saving, and human-edit tracking
    ...
    (async () => {
      ...
      try {
        ...
        if (existing?.uuid) {
          ...
          // Restore only human-edited rows; leave auto-populated rows blank so prefill
          // re-validates them against current tasks and events (requirement #2).
...
// Prompt: "if user input changes a line at any point, it is recorded as human-edited and never
//   modified again by auto-population"
// @description Prefills blank, non-human-edited DaySketch rows from agenda tasks for the current date.
// @param {Object} params
// @param {Object} params.agendaTasks - Tasks grouped by date key
// @param {string|Date} params.currentDate - Current selected date
// @param {Set<number>} params.humanEditedHours - Hours that must not be overwritten
...
// Prompt: "if user input changes a line at any point, it is recorded as human-edited and never
//   modified again by auto-population"
// @description Prefills blank, non-human-edited DaySketch rows from calendar events at their scheduled hours.
// @param {Object} params
// @param {Array} params.calendarEvents - Calendar event objects with start/end Date and title
// @param {string|Date} params.currentDate - Current selected date
// @param {Set<number>} params.humanEditedHours - Hours that must not be overwritten
// @param {boolean} params.loading - Whether base note load is still in progress
// @param {Function} params.setEntries - State setter for hour-row entries
...
// ------------------------------------------------------------------------------------------
// @description Calculates the scroll offset needed to bring an hour row near the middle
//   of the visible notebook viewport.
// @param {HTMLElement|null} notebookElement - Scroll container for the DaySketch rows
// @param {number} hour - 24-hour row to reveal
// @returns {number} ScrollTop value for the notebook container
...
// ------------------------------------------------------------------------------------------
// @description Auto-scrolls the DaySketch notebook to the current hour once rows have loaded.
// @param {Object} params
// @param {string|Date} params.currentDate - Selected DaySketch date
// @param {boolean} params.loading - Whether row content is still loading
// @param {Object} params.notebookRef - Ref for the scrollable notebook container
// @returns {void}
// [Claude] Task: auto-scroll DaySketch to the current hour after note rows render
// Prompt: "Update @lib/dashboard/day-sketch.js such that, upon loading, it automatically scrolls down to the current hour of the day"
// Date: 2026-05-10 | Model: gpt-5.4
...
function renderHourLine({ entries, h, handleBlur, handleInputChange, handleInputKeyDown, hour, setInputRef, timeFormat }) {
  return h("div", { key: hour, className: "day-sketch-line", "data-hour": hour },
...
//   persistence and pre-fills empty, non-human-edited hours from agenda tasks and calendar events.
//   On load, only human-edited rows are restored from the note; auto-populated rows are refreshed
//   asynchronously from current tasks and events. Human-edited rows are never overwritten.
// @param {Object} props
...
// @param {Array} props.calendarEvents - Calendar event objects with start/end Date and title
// @param {string|Date} props.currentDate - The current date used to derive the note name
// @param {string} props.timeFormat - Time format preference for hour labels
...
export default function DaySketchWidget({ agendaTasks, app, calendarEvents, currentDate, timeFormat }) {
  ...
  return h(WidgetWrapper, {
    ...
    h("div", { className: "day-sketch-notebook", ref: notebookRef },
      loading
