...
// ------------------------------------------------------------------------------------------
// [Claude claude-4.6-opus] Task: fill blank DaySketch rows from calendar events at their scheduled hours
// Prompt: "place calendar events in the day sketch at their scheduled times"
// @description Fills blank rows in an entries object from today's calendar events.
// @param {Object<number, string>} entries - Existing hour-row values
...
// @returns {Object<number, string>} New entries object with empty rows prefilled
function entriesPrefilledFromCalendarEvents(entries, calendarEvents, currentDate) {
  ...
  calendarEvents.forEach(event => {
    ...
    hoursFromCalendarEvent(event).forEach(hour => {
      if (HOURS.includes(hour) && !nextEntries[hour]) nextEntries[hour] = event.title;
    });
  });
  return nextEntries;
}
...
// ------------------------------------------------------------------------------------------
// @description Prefills blank DaySketch rows from calendar events at their scheduled hours.
// @param {Object} params
...
// @param {Function} params.setEntries - State setter for hour-row entries
// @returns {void}
// [Claude claude-4.6-opus] Task: prefill DaySketch rows from calendar events
// Prompt: "place calendar events in the day sketch at their scheduled times"
function useDaySketchCalendarPrefill({ calendarEvents, currentDate, loading, setEntries }) {
  useEffect(() => {
    if (!calendarEvents || !currentDate || loading) return;
    setEntries(prevEntries => {
      const nextEntries = entriesPrefilledFromCalendarEvents(prevEntries, calendarEvents, currentDate);
      return JSON.stringify(nextEntries) === JSON.stringify(prevEntries) ? prevEntries : nextEntries;
    });
  }, [calendarEvents, currentDate, loading, setEntries]);
}
...
export default function DaySketchWidget({ agendaTasks, app, calendarEvents, currentDate, timeFormat }) {
  ...
  useDaySketchCalendarPrefill({ calendarEvents, currentDate, loading, setEntries });
