...
// ------------------------------------------------------------------------------------------
// [Claude] Task: normalize agenda task text for prefilling DaySketch rows
// Prompt: "use the task text as prefilled row value"
// Date: 2026-03-18 | Model: gpt-5.3-codex
// @description Strips markdown links and Amplenote footnote refs, then keeps first line.
...
function renderHourLine({ entries, h, handleBlur, handleInputChange, handleInputKeyDown, hour, setInputRef }) {
    ...
    h("div", { className: "day-sketch-notebook" },
      loading
        ...
        : HOURS.map(hour => renderHourLine({
            entries, h, handleBlur, handleInputChange, handleInputKeyDown, hour, setInputRef,
          }))
    )
  );
}
