...
import { BACKGROUND_MODE_OPTIONS, SETTING_KEYS } from 'constants/settings';
...
// [Claude] Task: render dashboard settings — reads from app.settings directly
// Prompt: "widget reads app.settings[SETTING_KEYS.X] instead of receiving individual setting props"
// Date: 2026-03-14 | Model: claude-4.6-opus-high-thinking
export default function DashboardSettingsPopup({ app, onCancel, onSave }) {
  const h = createElement;
  const currentLlmProvider = app.settings?.[SETTING_KEYS.LLM_PROVIDER];
  const currentApiKey = app.settings?.[SETTING_KEYS.LLM_API_KEY];
  const currentBackgroundImageUrl = app.settings?.[SETTING_KEYS.BACKGROUND_IMAGE_URL];
  const currentBackgroundMode = app.settings?.[SETTING_KEYS.BACKGROUND_IMAGE_MODE];
