...
//   by persistence. An explicitDate bypasses automatic date rollover and fills that day's agenda note.
// @returns {Promise<void>}
export async function runProposedAgendaGeneration(app, { calendarEvents = null, currentDate, dateRange = null,
  ...
  try {
    ...
    const agendaNote = explicitDate ? await prepareAgendaNote(app, { domainName, domainUuid, targetDate: days[0] }) : null;
    // ------------------------------------------------------------------------------------------
    ...
    const obligationsForDay = async day => {
      ...
      const noteObligations = obligationsFromTasksAndEvents(agendaNote?.tasks || [], [], day);
    ...
    const dayResults = await generateProposedAgendaRange(app, { calendarEvents, days, domainName, domainUuid,
      forceRegenerate: forceRegenerate || !!explicitDate, obligationsForDay, priorityKey, providerEmOverride: providerEm || null });
    ...
    if (agendaNote && !result.error) {
      await populateAgendaNote(app, agendaNote.note, [...result.activities, ...untimedSuggestions]);
      if (isCurrentGeneration()) await app.navigate(`https://www.amplenote.com/notes/${ agendaNote.note.uuid }`);
    }
