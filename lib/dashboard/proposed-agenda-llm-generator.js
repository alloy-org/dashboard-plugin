...
export async function runProposedAgendaGeneration(app, { calendarEvents = null, currentDate, domainUuid,
    forceRegenerate = false, priorityKey, providerEm, setApproving, setAttribution, setDateLabel, setDismissedKeys,
    setError, setIsFutureDay, setLoading, setObligations, setProposed, setRecordProviderEm, setScheduledKeys }) {
  setLoading(true);
  setError(null);
  setApproving(false);
  ...
  try {
    ...
    const result = await generateProposedAgenda(app, { calendarEvents, forceRegenerate, obligations, priorityKey,
      providerEmOverride: providerEm || null, targetDate });
    if (result.error) {
