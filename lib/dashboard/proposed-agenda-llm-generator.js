...
import { approveProposedAgenda, generateProposedAgenda, resolveProposedAgendaDate, scheduleProposedActivity } from "proposed-agenda-service";
...
// @param {object|null} llmDateRecord - { date, domainName, domainUuid, priorityKey, providerEm } identity.
...
export async function recordProposedTaskStatus(app, llmDateRecord, activityKeys, scheduledEm) {
  if (!llmDateRecord?.date || !llmDateRecord?.domainName || !llmDateRecord?.providerEm
      || !(activityKeys?.length)) return;
...
//   When the agenda is for today, proposed activities whose start time has already passed are hidden — the
//   widget only shows the portion of the proposed agenda that still lies ahead (e.g. at noon, only 12pm→end).
//   Obligations are never hidden: a not-yet-elapsed commitment still belongs on the day's timeline.
...
// @param {object} [options] - { hidePastBeforeMinutes }: minutes-since-midnight before which proposed rows are
//   hidden as "in the past" (null/omitted for a future day, where nothing has elapsed yet).
// @returns {Array<object>} Rows ({ ...record, isObligation }) sorted ascending by startMinutes.
// [Claude claude-opus-4-8 (1M context)] Task: hide proposed rows already in the past for today's agenda
// Prompt: "Do not show proposed tasks in the past ... at noon, only see 12pm-6pm portion"
export function mergedAgendaRows(obligations, proposed, dismissedKeys, { hidePastBeforeMinutes = null } = {}) {
  ...
  const proposedRows = (proposed || []).filter(a => !dismissedKeys.has(activityKey(a)))
    .filter(a => hidePastBeforeMinutes == null || a.startMinutes >= hidePastBeforeMinutes)
...
//   were saved with it.
// @param {object} app - Amplenote app bridge.
// @param {object} options - { currentDate, domainName, domainUuid, forceRegenerate, priorityKey, providerEm } plus
//   setters; record-identity setters capture the resolved domain/provider used by persistence.
// @returns {Promise<void>}
...
export async function runProposedAgendaGeneration(app, { calendarEvents = null, currentDate, domainName, domainUuid,
    forceRegenerate = false, priorityKey, providerEm, setApproving, setAttribution, setDateLabel, setDismissedKeys,
    setError, setIsFutureDay, setLoading, setObligations, setProposed, setRecordDomainName, setRecordDomainUuid,
  ...
  try {
    ...
    const targetDate = resolveProposedAgendaDate(dateForResolution);
    const obligations = await requestTodayObligations(app, { calendarEvents, currentDate: targetDate, domainUuid });
    setObligations(obligations);
    const result = await generateProposedAgenda(app, { calendarEvents, domainName, domainUuid, forceRegenerate,
      obligations, priorityKey, providerEmOverride: providerEm || null, targetDate });
    if (result.error) {
      setError({ error: result.error, errorCode: result.errorCode || null });
      setProposed([]);
    } else {
      ...
      if (!result.fromCache) snapDashboardAction("generateProposedAgenda", { count: result.activities?.length || 0 });
    }
  } finally {
    setLoading(false);
  }
