...
export async function recordProposedRowStatuses(app, llmDateRecord, rows, scheduledEm) {
  ...
  for (const [dayMillis, activityKeys] of keysByDayMillis) {
    const dayChangedKeys = await updateProposedTaskStatuses(app, { activityKeys, date: new Date(dayMillis),
      ...
      error => { logIfEnabled("[proposed-agenda] failed to persist task status", error?.message); return []; });
  ...
  await recordAgendaDecisions(app, { decisions, domainName: llmDateRecord.domainName }).catch(
    error => logIfEnabled("[proposed-agenda] failed to log agenda decision", error?.message));
}
