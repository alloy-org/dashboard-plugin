...
export async function runProposedAgendaGeneration(app, { currentDate, domainUuid, forceRegenerate = false, priorityKey,
    providerEm, setApproving, setAttribution, setDateLabel, setDismissedKeys, setError, setIsFutureDay, setLoading,
    setObligations, setProposed, setRecordProviderEm, setScheduledKeys }) {
  setLoading(true);
  setError(null);
  setApproving(false);
  ...
  try {
    ...
    // Never surface a raw exception (e.g. an internal "find is not a function") to the user — log it and show a
    // clean, retryable message, mirroring how DreamTask degrades a failed LLM attempt rather than crashing.
    logIfEnabled("[proposed-agenda] runProposedAgendaGeneration caught", err);
    setError({ error: "Couldn't build a schedule right now. Please try again.", errorCode: "llm_error" });
  } finally {
    setLoading(false);
  }
}
