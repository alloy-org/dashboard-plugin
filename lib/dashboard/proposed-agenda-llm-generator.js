...
import { existingAgendaNoteTasks } from "proposed-agenda-note";
import { obligationsFromTasksAndEvents, requestTodayObligations } from "proposed-agenda-obligations";
...
//   by persistence. An explicitDate bypasses automatic date rollover and generates that day in place; it does
//   not write or open the day's agenda note, because choosing a date is a request to view the day rather than
//   to save it. Saving stays with the widget's Accept-agenda action.
// @returns {Promise<void>}
export async function runProposedAgendaGeneration(app, { calendarEvents = null, currentDate, dateRange = null,
    domainName, domainUuid, explicitDate = null, forceRegenerate = false, isCurrentGeneration = () => true,
    priorityKey, providerEm, setApproving, setAttribution,
    setDateLabel, setDismissedKeys, setError, setIsFutureDay, setLoading, setObligations, setProjectNotice, setProposed,
  ...
  if (setProjectNotice) setProjectNotice(null);
  try {
    ...
    const days = explicitDate ? [localMidnightFromDateInput(explicitDate)]
      : rangeDays.length > 0 ? rangeDays : [resolveProposedAgendaDate(dateForResolution)];
    const agendaNoteTasks = explicitDate ? await existingAgendaNoteTasks(app, { domainName, targetDate: days[0] }) : [];
    // ------------------------------------------------------------------------------------------
    // @desc Include fixed tasks already in the dated note even when the note is outside the active Task Domain.
    // @param {Date} day - Day whose obligations are being collected.
    const obligationsForDay = async day => {
      const obligations = await requestTodayObligations(app, { calendarEvents, currentDate: day, domainUuid });
      const noteObligations = obligationsFromTasksAndEvents(agendaNoteTasks, [], day);
      const knownUuids = new Set(obligations.map(row => row.taskUuid).filter(Boolean));
      return [...obligations, ...noteObligations.filter(row => !knownUuids.has(row.taskUuid))];
    };
    // A chosen date reads its own cache record like any other day (loadCachedProposedAgenda keys on date plus
    // domain/priority/provider), so stepping back to an already-generated day is instant instead of re-billing
    // the LLM. Regenerate stays explicit, via the widget's Regenerate button.
    ...
    const untimedSuggestions = dayResults.flatMap(day => day.unscheduledProjects || []);
    if (setProjectNotice && untimedSuggestions.length) {
      const titles = untimedSuggestions.map(suggestion => suggestion.title).join("; ");
      setProjectNotice(`These projects need attention, but have no free calendar slot: ${ titles }`);
    }
