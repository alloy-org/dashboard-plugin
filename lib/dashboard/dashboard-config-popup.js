...
function renderItem(h, widgetId, ctx) {
  ...
  return h('div', {
    ...
    className: ['dashboard-config-item',
      ctx.draggingId === widgetId ? 'dashboard-config-item--dragging' : '',
      ctx.dragOverId === widgetId ? 'dashboard-config-item--drag-over' : '',
    ].filter(Boolean).join(' '),
    draggable: true,
    ...
    h('span', { className: 'dashboard-config-item-handle', 'aria-hidden': 'true' }, '⠿'),
    h('span', { className: 'dashboard-config-item-icon', 'aria-hidden': 'true' }, widget.icon),
    h('div', { className: 'dashboard-config-item-info' },
      h('span', { className: 'dashboard-config-item-name' }, widget.name),
      widget.description && h('span', { className: 'dashboard-config-item-description' }, widget.description)
    ),
    h('div', { className: 'dashboard-config-item-actions' },
      h('button', {
        className: 'dashboard-config-arrow-button', type: 'button', disabled: idx === 0,
        title: 'Move up', 'aria-label': `Move ${widget.name} up`,
        onClick: (e) => { e.stopPropagation(); ctx.onMove(widgetId, 'up'); },
      }, '↑'),
      h('button', {
        className: 'dashboard-config-arrow-button', type: 'button', disabled: idx === combined.length - 1,
...
function renderSection(h, sectionId, label, icon, items, ctx) {
  return h('div', {
    className: ['dashboard-config-section', ctx.dragOverSection === sectionId ? 'dashboard-config-section--drag-over' : ''].filter(Boolean).join(' '),
    onDragOver: (e) => ctx.onDragOverSection(e, sectionId),
    onDragLeave: ctx.onDragLeaveSection,
    onDrop: (e) => ctx.onDropOnSection(e, sectionId),
  },
    h('div', { className: 'dashboard-config-section-header' },
      h('span', { className: 'dashboard-config-section-icon', 'aria-hidden': 'true' }, icon),
      h('h4', { className: 'dashboard-config-section-title' }, label)
    ),
    items.length === 0
      ? h('div', { className: 'dashboard-config-empty' },
          sectionId === 'rendered'
            ? 'Drag components here to show them on the dashboard'
            : 'All components are currently shown'
        )
      : h('div', { className: 'dashboard-config-list' }, ...items.map(id => renderItem(h, id, ctx)))
  );
}
...
export default function DashboardConfigPopup({ currentLayout, onSave, onCancel }) {
  ...
  return h('div', { className: 'config-popup-overlay', onClick: (e) => e.target === e.currentTarget && onCancel() },
    h('div', { className: 'config-popup-container dashboard-config-container' },
      h('div', { className: 'config-popup-header' },
        h('h3', { className: 'config-popup-title' }, '⚙\uFE0F Dashboard Layout')
      ),
      h('div', { className: 'config-popup-body dashboard-config-body' },
