...
export async function generateProposedAgenda(app, { aiModelOverride = null, forceRegenerate = false, now = null,
    obligations = [], priorityKey = null, providerEmOverride = null, targetDate: targetDateOverride = null } = {}) {
  const nowDate = now || new Date();
  const targetDate = targetDateOverride ? _localMidnight(targetDateOverride) : resolveProposedAgendaDate(nowDate);
  ...
  if (!forceRegenerate) {
    ...
    if (cached) return _reconcileCachedAgenda(app, { aiModelOverride, cached, dateLabel, dayWord, isFutureDay,
      nowMinutes, obligations, priorityOption, providerEm, providerEmOverride, targetDate });
  }

  const result = await _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, nowMinutes,
    obligations, priorityOption, providerEmOverride, targetDate });
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { aiModelOverride, dateLabel, dayWord, isFutureDay, nowMinutes, obligations,
//   priorityOption, providerEmOverride, targetDate }.
...
async function _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, nowMinutes, obligations,
    priorityOption, providerEmOverride, targetDate }) {
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { aiModelOverride, cached, dateLabel, dayWord, isFutureDay, nowMinutes, obligations,
//   priorityOption, providerEm, providerEmOverride, targetDate }. `cached` is loadCachedProposedAgenda()'s payload.
...
async function _reconcileCachedAgenda(app, { aiModelOverride, cached, dateLabel, dayWord, isFutureDay, nowMinutes,
    obligations, priorityOption, providerEm, providerEmOverride, targetDate }) {
  ...
  await storeProposedAgenda(app, { activities: mergedActivities, date: targetDate, llmAttributionFooter,
    priorityKey: priorityOption.key, providerEm });
...
async function _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, nowMinutes = null,
    obligations = [], priorityOption, providerEmOverride, quarterlyContent, targetDate, tasks }) {
  ...
  const prompt = _buildSchedulePrompt({ dateLabel, isFutureDay, nowMinutes, obligations, priorityOption,
    quarterlyContent, targetDate, tasks: proposableTasks });
...
// @param {object} params - { dateLabel, isFutureDay, nowMinutes, obligations, priorityOption, quarterlyContent,
//   targetDate, tasks }.
// @returns {string}
...
function _buildSchedulePrompt({ dateLabel, isFutureDay, nowMinutes = null, obligations = [], priorityOption,
    quarterlyContent, targetDate, tasks }) {
...
// @param {object} [options] - { nowMinutes, obligations }: the current time-of-day to clamp today's schedule to,
//   and the day's committed obligations the schedule must never overlap.
...
function _validateActivities(activities, validUuids, noteUuidFromTaskUuid, targetDate, { nowMinutes = null,
    obligations = [] } = {}) {
