...
// @returns {Promise<{activities: Array<object>, dateLabel: string, fromCache: boolean, providerEm: string,
//   dismissedKeys?: Array<string>, scheduledKeys?: Array<string>, llmAttributionFooter: string|null}|
...
export async function generateProposedAgenda(app, { aiModelOverride = null, forceRegenerate = false, obligations = [],
  ...
  logIfEnabled("[proposed-agenda] generateProposedAgenda entry", { dateLabel, priority: priorityOption.key,
    obligationCount: obligations.length, providerEm, forceRegenerate });

  if (!forceRegenerate) {
    const cached = await loadCachedProposedAgenda(app, { date: now, priorityKey: priorityOption.key, providerEm })
      .catch(error => { logIfEnabled("[proposed-agenda] cache lookup failed", error?.message); return null; });
    if (cached) return { activities: cached.activities, dateLabel, dismissedKeys: cached.dismissedKeys,
      fromCache: true, llmAttributionFooter: cached.llmAttributionFooter, providerEm,
  ...
  if (tasks.length === 0) {
    return { activities: [], error: "No tasks found available to schedule in Task Domain.",
      errorCode: "no_tasks" };
  }

  const result = await _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, obligations, priorityOption,
    providerEmOverride, quarterlyContent, tasks });
  if (!result.error && Array.isArray(result.activities) && result.activities.length > 0) {
    await storeProposedAgenda(app, { activities: result.activities, date: now,
...
async function _selectRelevantTasks(app, priorityOption) {
  ...
  const allTasks = domainUuid ? await app.getTaskDomainTasks(domainUuid).catch(() => []) : [];
