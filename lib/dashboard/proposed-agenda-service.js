...
import { loadCachedProposedAgenda, PROPOSED_TASK_STATUS, proposedTaskKey,
  storeProposedAgenda } from "proposed-agenda-archive";
...
export async function generateProposedAgenda(app, { aiModelOverride = null, forceRegenerate = false, obligations = [],
  ...
  const result = await _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, obligations,
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { aiModelOverride, dateLabel, dayWord, isFutureDay, obligations, priorityOption,
//   providerEmOverride, targetDate }.
...
async function _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, obligations,
    priorityOption, providerEmOverride, targetDate }) {
  ...
  return _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, obligations,
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { aiModelOverride, cached, dateLabel, dayWord, isFutureDay, obligations, priorityOption,
//   providerEm, providerEmOverride, targetDate }. `cached` is loadCachedProposedAgenda()'s payload.
// @returns {Promise<object>} Agenda payload with fromCache:true and the reconciled activities/keys.
async function _reconcileCachedAgenda(app, { aiModelOverride, cached, dateLabel, dayWord, isFutureDay, obligations,
    priorityOption, providerEm, providerEmOverride, targetDate }) {
  ...
  const fresh = await _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, obligations,
    priorityOption, providerEmOverride, targetDate });
...
async function _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, dayWord, isFutureDay, obligations = [],
  ...
  const proposableTasks = tasks.filter(task => !scheduledTaskUuids.has(task.taskUuid));
  logIfEnabled("[proposed-agenda] excluded already-scheduled tasks from candidates", { candidateCount: tasks.length,
    proposableCount: proposableTasks.length, alreadyScheduledCount: scheduledTaskUuids.size });

  const prompt = _buildSchedulePrompt({ dateLabel, isFutureDay, obligations, priorityOption, quarterlyContent,
    targetDate, tasks: proposableTasks });
