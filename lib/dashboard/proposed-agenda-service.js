...
export async function generateProposedAgenda(app, { aiModelOverride = null, providerEmOverride = null } = {}) {
  const now = new Date();
  const dateLabel = now.toLocaleDateString([], { weekday: "long", month: "long", day: "numeric", year: "numeric" });
  logIfEnabled("[proposed-agenda] generateProposedAgenda entry", { dateLabel, providerEmOverride });

  const tasks = await _selectRelevantTasks(app);
  ...
  return _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, providerEmOverride, quarterlyContent, tasks });
}
...
//   capped at 1,000 total. Returns compact records ({ duration, important, taskText, taskUuid, noteUuid }).
// @param {object} app - Amplenote app bridge.
// @returns {Promise<Array<{duration: number|null, important: boolean, noteUuid: string|null,
//   taskText: string, taskUuid: string}>>}
// [Claude claude-opus-4-8 (1M context)] Task: gather recent task-domain tasks per the volume rules
// Prompt: "retrieve at least 200 most recent tasks, or tasks in notes updated in the past month, up to 1000"
async function _selectRelevantTasks(app) {
  ...
  const selected = sortedByRecency.slice(0, targetCount);
  logIfEnabled("[proposed-agenda] _selectRelevantTasks", { domainUuid, openTaskCount: openTasks.length,
    recentWindowCount: recentTasks.length, selectedCount: selected.length });
  return selected.map(_compactTaskRecord).filter(Boolean);
}
...
// @returns {{duration: number|null, important: boolean, noteUuid: string|null, taskText: string,
//   taskUuid: string}|null}
// [Claude claude-opus-4-8 (1M context)] Task: shape compact [text, uuid, duration, important] task records
function _compactTaskRecord(task) {
  const taskText = String(task.content || "").replace(/\s+/g, " ").trim().substring(0, MAX_TASK_TEXT_CHARS);
  if (!taskText || !task.uuid) return null;
  return { duration: task.duration ?? null, important: !!task.important, noteUuid: task.noteUUID || null,
    taskText, taskUuid: task.uuid };
}
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { aiModelOverride, dateLabel, providerEmOverride, quarterlyContent, tasks }.
...
async function _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, providerEmOverride, quarterlyContent,
    tasks }) {
  ...
  const prompt = _buildSchedulePrompt({ dateLabel, quarterlyContent, tasks });
...
// @param {object} params - { dateLabel, quarterlyContent, tasks }.
// @returns {string}
// [Claude claude-opus-4-8 (1M context)] Task: write the schedule-proposal prompt
function _buildSchedulePrompt({ dateLabel, quarterlyContent, tasks }) {
...
- Favor the user's "important" tasks and tasks that advance the quarterly plan below.
  ...
  const normalized = activities
    .map(activity => {
      ...
      return { durationMinutes, isExisting: !!taskUuid,
        ...
        reason: String(activity?.reason || "").trim(), startMinutes,
