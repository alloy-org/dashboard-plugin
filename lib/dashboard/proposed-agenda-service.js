...
export async function generateProposedAgenda(app, { aiModelOverride = null, calendarEvents = null,
    domainName = null, domainUuid = null, forceRegenerate = false, now = null, obligations = [],
  ...
  if (!forceRegenerate) {
    ...
    if (cached) return _reconcileCachedAgenda(app, { aiModelOverride, cached, dateLabel, dayWord,
      domainName: resolvedDomainName, domainUuid: resolvedDomainUuid, isFutureDay,
  ...
  const result = await _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord,
    domainName: resolvedDomainName, domainUuid: resolvedDomainUuid, isFutureDay,
    migrationDomainName: activeDomainInfo.migrationDomainName, nowMinutes, obligations, priorityOption,
    providerEmOverride, recommendationContext, targetDate });
  if (!result.error && Array.isArray(result.activities) && result.activities.length > 0) {
    await storeProposedAgenda(app, { activities: result.activities, date: targetDate,
      domainName: resolvedDomainName, domainUuid: resolvedDomainUuid,
  ...
  return { ...result, domainName: resolvedDomainName, domainUuid: resolvedDomainUuid, fromCache: false, providerEm };
}
...
//   generation latency the collection pass exists to move off this path.
// @param {object} app - Amplenote app bridge.
// @param {object} options - { domainName, domainUuid, planQuarter } where planQuarter ({ quarter, year }) is the
//   quarter resolveProjectPlanQuarter chose for the day being planned.
// @returns {Promise<string>} Ideas markdown for the prompt, empty when nothing has been collected yet.
async function _collectedProjectIdeas(app, { domainName, domainUuid, planQuarter }) {
  try {
    ...
    return collectedIdeasMarkdown(storedRecords);
  } catch (error) {
...
// ----------------------------------------------------------------------------------------------
// @desc Gather the planning quarter's complete plan, Plan Builder projects, and persisted completion evidence,
//   then ask the LLM for a fresh hour-by-hour schedule WITHOUT persisting it (callers decide whether/how to
//   store). The planning quarter is normally the target day's, but in the days before a new quarter it is the
//   upcoming one when only that quarter holds Plan Builder projects. Shared by the cache-miss path and by cached-
//   agenda reconciliation when completed suggestions must be replaced.
// @param {object} app - Amplenote app bridge.
// @param {object} params - { aiModelOverride, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
//   migrationDomainName, nowMinutes, obligations, priorityOption, providerEmOverride, recommendationContext,
//   targetDate }.
...
async function _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
    migrationDomainName, nowMinutes, obligations, priorityOption, providerEmOverride, recommendationContext,
    targetDate }) {
  ...
  const planQuarter = await resolveProjectPlanQuarter(app, { allowLegacyMigration, domainName, domainUuid, targetDate });
  const quarterlyContent = planQuarter.planContent;
  const projectProgress = await loadProjectProgress(app, { domainName, domainUuid, planQuarter, quarterlyContent, targetDate });
  projectProgress.collectedIdeas = await _collectedProjectIdeas(app, { domainName, domainUuid, planQuarter });
  ...
  return _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
    nowMinutes, obligations, priorityOption, projectProgress, providerEmOverride, quarterlyContent, recommendationContext,
    targetDate, tasks });
}
...
//   the stale entries never resurface. Already scheduled/dismissed cached entries are left untouched. Suggestions
//   the user accepted outside the widget (from the calendar) are also detected and logged here.
// @param {object} app - Amplenote app bridge.
// @param {object} params - { aiModelOverride, cached, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
//   migrationDomainName, nowMinutes, obligations, priorityOption, providerEm, providerEmOverride,
//   recommendationContext, targetDate }. `cached` is loadCachedProposedAgenda()'s payload.
// @returns {Promise<object>} Agenda payload with fromCache:true and the reconciled activities/keys.
async function _reconcileCachedAgenda(app, { aiModelOverride, cached, dateLabel, dayWord, domainName, domainUuid,
    isFutureDay, migrationDomainName, nowMinutes, obligations, priorityOption, providerEm, providerEmOverride,
    recommendationContext, targetDate }) {
  const { activities: cachedActivities, scheduledKeys } = await _reconcileExternalApprovals(app,
  ...
  const staleUuids = await _staleCachedTaskUuids(app, cachedActivities);
  if (staleUuids.size === 0) {
    return { activities: cachedActivities, dateLabel, dayWord, dismissedKeys: cached.dismissedKeys, domainName,
      domainUuid, fromCache: true, isFutureDay, llmAttributionFooter: cached.llmAttributionFooter, providerEm,
      scheduledKeys };
  }
  const keptActivities = cachedActivities.filter(activity => !(activity.taskUuid && staleUuids.has(activity.taskUuid)));
  const staleSlots = cachedActivities.filter(activity => activity.taskUuid && staleUuids.has(activity.taskUuid));
  ...
  const fresh = await _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid,
    isFutureDay, migrationDomainName, nowMinutes, obligations, priorityOption, providerEmOverride,
    recommendationContext, targetDate });
  ...
  await storeProposedAgenda(app, { activities: mergedActivities, date: targetDate, domainName, domainUuid,
    llmAttributionFooter, priorityKey: priorityOption.key, providerEm,
    recommendationContextKey: recommendationContext.cacheKey });
  return { activities: mergedActivities, dateLabel, dayWord, domainName, domainUuid, fromCache: true, isFutureDay,
    llmAttributionFooter, providerEm,
...
- "reason": REQUIRED — exactly one sentence stating why affirming this suggestion is worth the user's time today.
...
// ----------------------------------------------------------------------------------------------
// @desc Schedule an existing task, reuse a dated project-step checkbox, or insert a new travel task in its note.
// @param {object} app - Amplenote app bridge.
