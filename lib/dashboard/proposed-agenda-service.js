...
async function _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
  ...
  return _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
    nowMinutes, obligations, priorityOption, providerEmOverride, quarterlyContent, recommendationContext,
    targetDate, tasks });
}
...
//   once — updateProposedTaskStatuses reports which entries really moved. Failures are logged, never thrown, so
//   serving the cached agenda never depends on the bookkeeping succeeding.
// @param {object} app - Amplenote app bridge.
...
async function _reconcileExternalApprovals(app, { activities, domainName, domainUuid, obligations, priorityOption,
  ...
  const changedKeys = await updateProposedTaskStatuses(app, { activityKeys: approvedKeys, date: targetDate,
    ...
    scheduledEm: PROPOSED_TASK_STATUS.SCHEDULED }).catch(error => {
      logIfEnabled("[proposed-agenda] failed to mark externally-approved suggestions", error?.message); return []; });
  ...
  await recordAgendaDecisions(app, { decisions, domainName }).catch(
    error => logIfEnabled("[proposed-agenda] failed to log calendar approval", error?.message));
...
async function _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid,
    isFutureDay, nowMinutes = null, obligations = [], priorityOption, providerEmOverride, quarterlyContent,
  ...
  // accepts. A read failure degrades to no history rather than blocking generation.
  ...
  const decisionHistoryMarkdown = await recentAgendaDecisionsMarkdown(app, { date: targetDate, domainName })
    .catch(error => { logIfEnabled("[proposed-agenda] decision-history lookup failed", error?.message); return ""; });
