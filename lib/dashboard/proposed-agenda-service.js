...
//   ({ quarter, year }) is the quarter resolveProjectPlanQuarter chose for the day being planned, and
...
async function _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
  ...
  const quarterlyContent = planQuarter.planContent ? contentWithoutCompletedProjects(planQuarter.planContent) : null;
  const projectProgress = await loadProjectProgress(app, { domainName, domainUuid, planQuarter, quarterlyContent, targetDate });
  const liveProjectUuids = new Set(projectProgress.projects.map(project => project.uuid));
  projectProgress.collectedIdeas = await _collectedProjectIdeas(app, { domainName, domainUuid, liveProjectUuids, planQuarter });
