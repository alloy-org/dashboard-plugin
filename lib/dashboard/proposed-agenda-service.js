...
async function _collectedProjectIdeas(app, { domainName, domainUuid, targetDate }) {
  try {
...
// ----------------------------------------------------------------------------------------------
// @desc Gather the selected quarter's complete plan, Plan Builder projects, and persisted completion evidence,
//   then ask the LLM for a fresh hour-by-hour schedule,
...
async function _generateFreshSchedule(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid, isFutureDay,
  ...
  const quarterLabel = `Q${ Math.floor(targetDate.getMonth() / 3) + 1 } ${ targetDate.getFullYear() }`;
  const planNote = await resolveQuarterlyPlanNote(app, allowLegacyMigration, domainName, quarterLabel);
  let quarterlyContent = null;
  if (planNote) {
    const content = await app.getNoteContent({ uuid: planNote.uuid });
    quarterlyContent = content || null;
  }
  const projectProgress = await loadProjectProgress(app, { domainName, domainUuid, quarterlyContent, targetDate });
  projectProgress.collectedIdeas = await _collectedProjectIdeas(app, { domainName, domainUuid, targetDate });
...
async function _generateScheduleFromLlm(app, { aiModelOverride, dateLabel, dayWord, domainName, domainUuid,
  ...
  const projectInstructions = projectProgress.markdown ? `\n## Quarterly project progress and Plan Builder choices\n${ projectProgress.markdown }\nInclude a task for EVERY due project, prioritizing projects with no completion in the past week. Cite the recorded weekly pace and actual completion count in its reason. Respect focus months and preferred weekdays. Task counts are a proxy for blocks, not elapsed work time.\n` : "";
