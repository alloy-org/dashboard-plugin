...
export function obligationsFromTasksAndEvents(tasks, events, currentDate) {
  ...
  const taskObligations = (tasks || []).filter(task => task && !task.completedAt && !task.dismissedAt)
    ...
    .map(task => _obligationRecord({ durationMinutes: _taskDurationMinutes(task), source: "task",
      startMinutes: _minutesSinceMidnight(task.startAt || task.deadline), taskUuid: task.uuid || null,
      title: task.content }));
