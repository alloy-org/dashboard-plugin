...
import { calendarFetchDaysForDate, normalizeExternalCalendarEvents } from "util/calendar-utility";
...
export function obligationsFromTasksAndEvents(tasks, events, currentDate) {
  ...
  const openTasks = (tasks || []).filter(task => task && !task.completedAt && !task.dismissedAt);
  const tasksOnDate = openTasks.filter(task => _isOnDateKey(task.startAt || task.deadline, todayKey));
  const taskObligations = tasksOnDate.map(task => _obligationRecord({ durationMinutes: _taskDurationMinutes(task),
    source: "task", startMinutes: _minutesSinceMidnight(task.startAt || task.deadline), taskUuid: task.uuid || null,
    title: task.content }));
  const timedEvents = (events || []).filter(event => event && !event.allDay);
  const eventsOnDate = timedEvents.filter(event => _isOnDateKey(event.start, todayKey));
  const eventObligations = eventsOnDate.map(event => _obligationRecord({ durationMinutes: _eventDurationMinutes(event),
    source: "event", startMinutes: _minutesSinceMidnight(event.start), title: event.title || "Calendar event" }));
  const presentObligations = [...taskObligations, ...eventObligations].filter(Boolean);
  const sortedObligations = presentObligations.sort((a, b) => a.startMinutes - b.startMinutes);
...
function _minutesSinceMidnight(timestamp) {
  const millis = millisFromDateInput(timestamp);
...
// ----------------------------------------------------------------------------------------------
// @desc Whole-minute duration of a scheduled task from its `duration` attribute (Amplenote stores it in
//   seconds), or null when the task carries no duration. Lets a scheduled task contribute an end time to the
//   occupied-times block the proposed-agenda prompt sends, not just a start point.
// @param {object} task - Native Amplenote task object.
// @returns {number|null}
// [Claude claude-opus-4-8 (1M context)] Task: capture a scheduled task's duration for the occupied-times block
function _taskDurationMinutes(task) {
  const seconds = task?.duration;
...
// @param {object} options - { calendarEvents, currentDate, domainUuid }.
...
// [OpenAI GPT-5.5] Task: use dashboard-provided events for the actual Proposed Agenda target date
export async function deriveTodayObligations(app, { calendarEvents = null, currentDate = null, domainUuid = null } = {}) {
  const tasks = await fetchDomainOrAllNotesTasks(app, domainUuid).catch(() => []);
  let events = [];
  if (Array.isArray(calendarEvents)) {
    events = normalizeExternalCalendarEvents(calendarEvents);
  } else {
    try {
      const days = calendarFetchDaysForDate(currentDate || new Date());
      const calendarOptions = domainUuid ? { days, taskDomainUUID: domainUuid } : { days };
...
// @param {object} app - Amplenote app bridge.
// @param {object} options - { calendarEvents, currentDate, domainUuid, timeoutMs }.
// @returns {Promise<Array<object>>} Obligation records.
// [Claude claude-opus-4-8 (1M context)] Task: ask Agenda for today's obligations, else derive them ourselves
// [OpenAI GPT-5.5] Task: bypass today's broadcast when generating for a resolved non-today schedule date
export function requestTodayObligations(app, { calendarEvents = null, currentDate = null, domainUuid = null,
    timeoutMs = OBLIGATIONS_RESPONSE_TIMEOUT_MS } = {}) {
  if (Array.isArray(calendarEvents)) return deriveTodayObligations(app, { calendarEvents, currentDate, domainUuid });
