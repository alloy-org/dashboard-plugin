...
// Date: 2026-02-22 | Model: claude-opus-4-6
export default function CalendarWidget({ app, completedTasksByDate, currentDate, onDateSelect, openTasks, selectedDate, settings }) {
  const h = createElement;
  const today = new Date(currentDate);
  const [viewDate, setViewDate] = useState(new Date(today.getFullYear(), today.getMonth(), 1));
  const settingsWeekStart = settings?.['dashboard_calendar_config']?.[0] === '1' ? '1' : '0';
  ...
  const handleConfigSubmit = async () => {
    await app.saveSetting('calendar', [weekStart]);
  ...
  return h(WidgetWrapper, {
    title: widgetTitleFromId('calendar'), icon: '📅', widgetId: 'calendar', configurable: true,
