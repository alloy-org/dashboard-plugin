...
// ----------------------------------------------------------------------------------------------
// @desc Complete a DreamTask suggestion in Amplenote and mirror dismissal metadata in the cache note.
// @param {object} app - Amplenote app bridge.
// @param {string|null} noteUUID - Daily proposed-tasks note UUID.
// @param {object} task - Dream task payload from the service/cache.
// [Claude GPT-5.5] Task: complete existing tasks via app.updateTask and completed invented tasks in daily jots
// Prompt: "when the user clicks Complete link on a task that already exists, use app.updateTask"
export async function completeDreamTask(app, noteUUID, task) {
  if (!task) return false;
  const completedAt = Math.floor(Date.now() / 1000);
  if (task.isExisting && task.uuid) {
    await app.updateTask(task.uuid, { completedAt });
  } else {
    const dailyJotNote = await _dailyJotNoteHandleForToday(app);
    await app.insertNoteContent(
      { uuid: dailyJotNote.uuid },
      _completedTaskMarkdown(task, completedAt),
      { atEnd: true },
    );
  }
  await updateDreamTaskTaskMetadata(app, noteUUID, task, { completedAt, removedAt: null });
  return true;
}
...
// ----------------------------------------------------------------------------------------------
// @desc Find or create today's Daily Jot note for completed invented DreamTask suggestions.
// @param {object} app - Amplenote app bridge.
// [Claude GPT-5.5] Task: resolve daily jot target for completed invented tasks
// Prompt: "look for a note with today's name and create such a note, with the daily-jots tag"
async function _dailyJotNoteHandleForToday(app) {
  const noteName = _todayDailyJotNoteName();
  const existingNote = await app.findNote({ name: noteName, tags: [DAILY_JOTS_TAG] });
  if (existingNote?.uuid) return existingNote;
  const uuid = await app.createNote(noteName, [DAILY_JOTS_TAG]);
  return { uuid };
}

// ----------------------------------------------------------------------------------------------
// @desc Build markdown for an already-completed Amplenote task with task metadata in an HTML comment.
// @param {object} task - DreamTask suggestion.
// @param {number} completedAt - Unix timestamp in seconds.
// [Claude GPT-5.5] Task: format completed invented task markdown for insertNoteContent
// Prompt: "use insertNoteContent to add that task, with the completed timestamp"
function _completedTaskMarkdown(task, completedAt) {
  const title = String(task?.title || "Untitled task").replace(/\s+/g, " ").trim();
  return `\n- [ ] ${ title } <!-- ${ JSON.stringify({ completedAt }) } -->\n`;
}
...
function _ordinalSuffix(dayOfMonth) {
...
// [Claude GPT-5.5] Task: derive today's daily jot note name for invented completed tasks
// Prompt: "look for a note with today's name, e.g., April 29th, 2026"
function _todayDailyJotNoteName() {
