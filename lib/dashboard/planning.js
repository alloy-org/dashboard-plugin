 ...
 * @returns {Promise<{ devEdit?: boolean, noteUUID?: string }|void>}
 */
async function handleCreateWeekPlan(plan, weekLabel, setWeekLoading, setWeekContent) {
  setWeekLoading(true);
  try {
...
export default function PlanningWidget({ quarterlyPlans, gridHeightSize = 1 }) {
  ...
  useEffect(() => {
    ...
    if (!noteUUID) return;
    setWeekLoading(true);
    fetchSectionContent(noteUUID, weekLabel)
      .then(result => {
  ...
  return h(WidgetWrapper, { title: widgetTitleFromId('planning'), icon: '📋', widgetId: 'planning' },
    ...
    isTwoTall
      ? renderWeeklyPlanSection(h, weekLabel, quarterlyPlans.current.year, weekLoading, weekContent, async () => {
          const result = await handleCreateWeekPlan(quarterlyPlans.current, weekLabel, setWeekLoading, setWeekContent);
          handleDevEdit(result);
        })
      : null,
  );
