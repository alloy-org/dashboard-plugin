 ...
 * @param {Object} month - Month object with index, full name, and associated plan.
 ...
 * @returns {Promise<void>}
 */
async function handleCreatePlan(monthContent, { setMonthLoading, setMonthContent }) {
 ...
 */
function renderQuarterCard(h, plan) {
  ...
  return h('div', { key: plan.label, className: cardClass, onClick: () => handleOpenPlan(plan) },
  ...
  return h(WidgetWrapper, { title: widgetTitleFromId('planning'), icon: '📋', widgetId: 'planning' },
    ...
    activeTab !== null
      ? h('div', { className: 'month-content-area' },
          renderMonthContentArea(h, monthLoading, monthContent, () => handleCreatePlan(monthContent, createPlanDeps))
        )
      : null
  );
}
