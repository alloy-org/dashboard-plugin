...
// [Claude] Task: render quarterly plan cards with created-plan styling and monthly-details indicator
// Prompt: "solid border + green bg when plan exists; checkmark or WIP icon based on monthly details"
  ...
  return h(WidgetWrapper, { title: widgetTitleFromId('planning'), icon: '📋', widgetId: 'planning' },
    h('div', { className: 'planning-quarters' },
      [quarterlyPlans.current, quarterlyPlans.next].map(plan => {
        const hasNote = !!effectiveNoteUUID(plan);
        ...
        return h('div', { key: plan.label, className: cardClass, onClick: () => handleOpenPlan(plan) },
          ...
          h('div', { className: 'quarter-status-row' },
            ...
            hasNote
              ? h('span', {
                  className: 'quarter-plan-indicator text-intip tooltip',
                  title: indicatorTip,
