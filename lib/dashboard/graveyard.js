...
export default function GraveyardWidget({ app, gridHeightSize = 1, taskDomainUUID }) {
  ...
  // [Claude claude-sonnet-4-6] Task: dismiss task via updateTask dismissedAt, append to retired note, and celebrate
  // Prompt: "expand graveyard with confetti + retired note"
  const onGraveyardTask = useCallback(async (event, task) => {
    ...
    await appendTaskToRetiredNote(app, task).catch(
