...
function renderTaskCard(h, isDismissing, onGraveyard, onKeep, task) {
...
// [Claude claude-sonnet-4-6] Task: render graveyard candidate task list with progressive meadow reveal
// Prompt: "fade visibility of a green image placed behind tasks, more visible as each is dismissed"
function renderTaskList(dismissingUuids, h, meadowOpacity, meadowUrl, onGraveyard, onKeep, tasks) {
  return h(WidgetWrapper, { icon: '⚰️', subtitle: 'Consider retiring these forgotten tasks',
    ...
    h('div', { className: 'graveyard-meadow-wrapper' },
      ...
      h('ul', { className: 'graveyard-task-list' },
        ...tasks.map(task => renderTaskCard(h, dismissingUuids.has(task.uuid), onGraveyard, onKeep, task))
      )
    )
  );
}
...
export default function GraveyardWidget({ app, gridHeightSize = 1, taskDomainUUID }) {
  ...
  // [Claude claude-sonnet-4-6] Task: dismiss task — fade out card, reveal meadow, fire confetti, update task
  // Prompt: "fade visibility of a green image placed behind tasks, more visible as each is dismissed"
  const onGraveyardTask = useCallback(async (event, task) => {
    ...
    setTimeout(() => {
      ...
      setDismissingUuids(prev => { const next = new Set(prev); next.delete(task.uuid); return next; });
    }, CARD_FADE_MS);
  }, [app]);

  // [Claude claude-sonnet-4-6] Task: keep task — fade card out without counting toward meadow reveal
  // Prompt: "leave the other tasks in place around them when one task is removed"
  ...
  return renderTaskList(dismissingUuids, h, meadowOpacity, meadowUrl, onGraveyardTask, onKeepTask, tasks);
}
