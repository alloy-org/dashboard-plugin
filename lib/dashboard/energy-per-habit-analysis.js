...
//   paths derive the grouping key as habitKeyFromContent(thisLabel), so storing this label makes the
...
//   content. The cache stores only that label and re-derives its key as habitKeyFromContent(label) when a
...
//   and either way key === habitKeyFromContent(label), so the cache round-trips exactly.
// @param {string} content - Raw task content.
...
export function habitGroupKey(content) {
  return habitKeyFromContent(habitGroupLabel(content));
}
...
// @param {Map<string, Object>} aggByKey - Per-habit aggregates keyed by normalized content.
// @returns {Map<string, Object>} Clustered aggregates keyed by each cluster's representative key.
function clusterHabitAggregates(aggByKey) {
  const keys = [...aggByKey.keys()].sort();
  // Short habits are exact-match singletons; only phrase habits (>= MIN_HABIT_WORDS label words) can merge.
  const phraseKeys = keys.filter(key => habitWords(aggByKey.get(key).label).length >= MIN_HABIT_WORDS);
  const wordsByKey = new Map(phraseKeys.map(key => [key, habitWords(aggByKey.get(key).label)]));
  ...
  for (const memberKeys of clusters.values()) {
    ...
    const repKey = memberKeys.slice().sort((a, b) =>
      (aggByKey.get(b).completions || 0) - (aggByKey.get(a).completions || 0) || (a < b ? -1 : 1))[0];
    const rep = aggByKey.get(repKey);
    const combined = { label: rep.label, sampleContent: rep.sampleContent || rep.label, completions: 0,
      doneMoods: [], offMoods: [], weekStreak: 0 };
    let sawDaysDone = false, sawDoneDayKeys = false;
    for (const key of memberKeys) {
      const agg = aggByKey.get(key);
      combined.completions += agg.completions || 0;
      if (Array.isArray(agg.doneMoods)) combined.doneMoods.push(...agg.doneMoods);
      if (Array.isArray(agg.offMoods)) combined.offMoods.push(...agg.offMoods);
      // Show the strongest member's week streak (habitsFromAggregates recomputes day-streak from doneDayKeys).
      if (agg.weekStreak != null) combined.weekStreak = Math.max(combined.weekStreak, agg.weekStreak);
      if (agg.daysDone != null) { combined.daysDone = (combined.daysDone || 0) + agg.daysDone; sawDaysDone = true; }
      if (agg.doneDayKeys) { if (!combined.doneDayKeys) combined.doneDayKeys = new Set();
        for (const dk of agg.doneDayKeys) combined.doneDayKeys.add(dk); sawDoneDayKeys = true; }
    }
    if (!sawDaysDone && !sawDoneDayKeys) combined.daysDone = 0;
    merged.set(repKey, combined);
  }
  return merged;
}
...
export function habitsFromAggregates(aggByKey, { orderedWindowKeys = [], minCompletions = MIN_HABIT_COMPLETIONS,
  ...
  const clustered = clusterHabitAggregates(aggByKey);
  const habits = [];
  for (const [key, agg] of clustered.entries()) {
