 ...
 */
import { LAYOUT_PROFILES, getProfileById } from "layout-profiles";
...
function useLayoutState(currentLayout) {
  ...
  const onReset = (currentSelectedProfileId) => {
    ...
    if (profile) {
      const profileWidgetIds = profile.widgets.map(widget => widget.widgetId);
      const allWidgetIds = WIDGET_REGISTRY.map(w => w.widgetId);
...
// Date: 2026-03-07 | Model: claude-4.6-opus-high-thinking
export default function DashboardLayoutPopup({ currentLayout, onSave, onCancel, selectedLayoutProfile }) {
  ...
  return h('div', { className: 'config-popup-overlay', onClick: (e) => e.target === e.currentTarget && onCancel() },
    h('div', { className: 'config-popup-container dashboard-layout-popup' },
      h('div', { className: 'config-popup-header' },
        ...
        h('div', { className: 'explanation-paragraph' }, `Customize the order of your widgets by dragging or sizing them in this menu. You can also reorder widgets outside this menu by clicking on their header for 2 seconds to engage drag reordering.`),
        h('div', { className: 'explanation-paragraph' },
          h('label', { className: 'dashboard-layout-popup-profile-label' }, 'Default layout'),
          h('select', {
            className: 'dashboard-layout-popup-profile-select',
            value: selectedProfileId || '',
            onChange: e => setSelectedProfileId(e.target.value || null),
          },
            h('option', { value: '' }, 'Custom (no preset)'),
            ...LAYOUT_PROFILES.map(profile =>
              h('option', { key: profile.id, value: profile.id }, profile.name)
            )
          )
        ),
      ...
      h('div', { className: 'config-popup-actions' },
        h('button', { className: 'config-popup-link', type: 'button', onClick: () => onReset(selectedProfileId) }, 'Reset to defaults'),
