...
// @param {object} options - { domainName, domainUuid, planQuarter, quarterlyContent, targetDate }; planQuarter
//   ({ quarter, year }) names the quarter whose projects apply, defaulting to the target day's own quarter.
...
// @param {object} app - Host-compatible app bridge.
//   When the plan quarter has not begun yet, focus months are judged from its first day, so a project planned for
//   the upcoming quarter's first month counts as in focus during the final days of the current one.
// @param {object} options - { domainName, domainUuid, planQuarter, quarterlyContent, targetDate }.
// @returns {Promise<object>} { candidates, markdown, projects }.
async function refreshProjectProgress(app, { domainName, domainUuid, planQuarter = null, quarterlyContent, targetDate }) {
  const scope = resolvePlanScope({ domainName, domainUuid, quarter: planQuarter?.quarter ?? Math.floor(targetDate.getMonth() / 3) + 1,
    year: planQuarter?.year ?? targetDate.getFullYear() });
  ...
  const focusDate = targetDate < quarterStart ? quarterStart : targetDate;
  const evidenceProjects = updatedProjects.map(project => ({ ...project, ...projectProgressEvidence(project, targetDate, { focusDate }) }));
  const pacedFirstProjects = [...evidenceProjects].sort((first, second) => !!second.blocksPerWeek - !!first.blocksPerWeek);
  const markdown = pacedFirstProjects.map(projectProgressMarkdown).join("\n");
...
// ----------------------------------------------------------------------------------------------
// @desc Describe one project's pace and completion evidence for the schedule prompt. A project without a chosen
//   weekly pace is labeled optional so the model fills paced projects first.
// @param {object} project - Project merged with projectProgressEvidence output.
// @returns {string} A bullet with indented detail lines.
function projectProgressMarkdown(project) {
  const pace = project.blocksPerWeek ? `${ project.blocksPerWeek } block(s) per week` : "none chosen";
  const status = project.blocksPerWeek ? (project.due ? "due" : "not due") : "optional";
  return `- ${ project.summary } (project:${ project.uuid }): ${ project.reason }
  Weekly pace: ${ pace }; completions this week: ${ project.completedThisWeek }; completions in past week: ${ project.completedPastWeek }; status: ${ status }.
  Task UUIDs: ${ project.relatedTasks.join(", ") || "none" }.
  Focus months: ${ project.focusMonths?.join(", ") || "whole quarter" }; preferred weekdays: ${ project.preferredWeekdays?.join(", ") || "any" }.`;
}
