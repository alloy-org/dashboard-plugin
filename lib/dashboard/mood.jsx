...
function buildMoodTooltipHTML(data) {
  ...
  for (const entry of entries) {
    ...
    let html = `<div class="mood-viz-tooltip-entry">` +
      ...
      `<span class="mood-viz-tooltip-mood">${emoji} <span class="mood-viz-label">${label} (${entry.rating > 0 ? `+${entry.rating}` : entry.rating})</span></span>`;
    if (entry.notes) {
...
async function handleSubmit(app, selectedMood, notes, setSubmitting, setSubmitted, onMoodRecorded) {
  ...
  if (trimmedNotes) {
    await app.updateMoodRating(moodRatingUUID, { note: trimmedNotes });
    logIfEnabled(`Mood rating uuid ${ moodRatingUUID } updated with notes`);
  }
  setSubmitted(true);
  if (onMoodRecorded) {
    logIfEnabled("Retrieved mood rating UUID:", moodRatingUUID, "calling onMoodRecorded");
    onMoodRecorded({ rating: selectedMood.value, timestamp: Math.floor(Date.now() / 1000), note: trimmedNotes || null });
    logIfEnabled("onMoodRecorded callback complete");
  } else {
    logIfEnabled("onMoodRecorded callback not provided, skipping");
  }
...
export default function MoodWidget({ app, moodRatings, onMoodRecorded }) {
  ...
  const moodConfig = parseWidgetConfig(pluginSettings(), 'mood');
  ...
  useEffect(() => {
    if (!submitted) return;
    logIfEnabled("Setting timeout to reset mood component after submission");
