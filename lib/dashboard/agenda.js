          ...
          visibleTasks.length === 0 && eventsForDate.length === 0
            ...
            : [
                ...visibleTasks.map(task => h('div', {
                    key: task.uuid, className: 'agenda-item agenda-task-row',
                  ...
                  h('div', { className: 'agenda-content' },
                    ...
                    getTaskNoteUuid(task)
                      ? h('button', { type: 'button', className: 'agenda-note-link',
                          onClick: (event) => navigateToTaskNote(task, event) }, getTaskNoteLabel(task))
                      : null
                  ),
                  toMillis(task.endAt) && toMillis(task.startAt) && toMillis(task.endAt) > toMillis(task.startAt)
                    ? h('span', { className: 'agenda-duration' },
                        Math.round((toMillis(task.endAt) - toMillis(task.startAt)) / 60000) + 'm')
                    : null
                )),
                ...eventsForDate.map((event, i) => h('div', {
                    key: `cal-${ i }`, className: 'agenda-item agenda-task-row',
                  },
                  h('div', { className: 'agenda-indicator priority-normal' }),
                  h('div', { className: 'agenda-content' },
                    event.allDay
                      ? h('span', { className: 'agenda-time' }, 'All day')
                      : event.start ? h('span', { className: 'agenda-time' }, formatTime(event.start)) : null,
                    h('span', { className: 'agenda-text' }, event.title || 'Calendar event'),
                    event.calendar?.name
                      ? h('span', { className: 'agenda-note-link' }, event.calendar.name) : null
                  ),
