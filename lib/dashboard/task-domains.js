...
import "./styles/_task-domains.scss"
...
export default function TaskDomains({ activeTaskDomain, app, domains, onDomainChange }) {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      const result = await app.refreshTaskDomains();
  ...
  const handleSelect = async (domainUuid) => {
    ...
    const result = await app.setActiveTaskDomain(domainUuid);
    if (result && onDomainChange) {
  ...
  const handleOpenDomainSettings = async (event) => {
    ...
    await app.navigateToUrl('https://www.amplenote.com/notes?tag=task_calendar');
  };
