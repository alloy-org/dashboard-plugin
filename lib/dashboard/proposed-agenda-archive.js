...
async function readMonthRecords(app, date) {
  const noteName = proposedAgendaNoteNameFromDate(date);
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { date, windowDays? }.
...
// [Claude claude-opus-4-8 (1M context)] Task: expose the previous week of proposals per task for de-duplication
// Prompt: "keep a log of its previous week of proposed suggestions ... not suggest the same task two days in a
//   row; not more than twice in any 5 day period"
export async function recentProposedTaskHistory(app, { date, windowDays = HISTORY_WINDOW_DAYS }) {
  ...
  const recordArrays = await Promise.all(distinctMonthDates(windowStart, date).map(monthDate =>
    readMonthRecords(app, monthDate).catch(() => [])));
  ...
  for (const record of recordArrays.flat()) {
    ...
    if (!recordKey || recordKey >= targetKey || recordKey < windowStartKey) continue;
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { date, priorityKey, providerEm, recommendationContextKey }.
...
export async function loadCachedProposedAgenda(app, { date, priorityKey, providerEm, recommendationContextKey = null }) {
...
// @param {object} app - Amplenote app bridge.
// @param {object} params - { activities, date, llmAttributionFooter, priorityKey, providerEm, recommendationContextKey }.
//   `recommendationContextKey` contains the target date key, all-day event keys, same-weekday completion-pattern
...
export async function storeProposedAgenda(app, { activities, date, llmAttributionFooter = null, priorityKey,
    providerEm, recommendationContextKey = null }) {
  const llmDateRecord = { dateKey: dateKeyFromDateInput(date), priorityKey, providerEm };
