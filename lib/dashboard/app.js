...
export default function DashboardApp() {
  ...
  const domainTasks = useDomainTasks();
  ...
  useEffect(() => {
    callPlugin('init').then(result => {
      if (result?.error) {
        ...
        domainTasks.initializeDomainTasks(result);
  ...
  const widgetData = {
    ...
    weeklyVictoryValue,
    currentDate,
    openTasks: domainTasks.openTasks,
    completedTasks: domainTasks.completedTasks,
    buildAgendaTasksByDate: domainTasks.buildAgendaTasksByDate
  };

  return h('div', { className: 'dashboard' },
    h(TaskDomains, {
      domains: domainTasks.taskDomains,
      activeTaskDomain: domainTasks.activeTaskDomain,
