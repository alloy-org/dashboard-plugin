...
export default function DashboardApp() {
  ...
  // Persist the new ordered widget list and update local settings so the grid re-renders.
  const handleLayoutSave = useCallback(async (newRenderedWidgetIds) => {
    ...
    const newLayout = newRenderedWidgetIds.map(widgetId =>
      configByWidgetId[widgetId] ||
      DEFAULT_DASHBOARD_COMPONENTS.find(d => d.widgetId === widgetId) ||
      { widgetId, gridWidthSize: 1, gridHeightSize: 1, settings: {} }
    );
  ...
  return h('div', { className: 'dashboard' },
    ...
    h('div', { className: 'dashboard-toolbar' },
      ...
      h('div', { className: 'dashboard-toolbar-actions' },
        h('button', {
          className: 'dashboard-configure-btn',
          type: 'button',
