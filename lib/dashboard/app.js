 ...
 * @param {object} completedTasks - Completed tasks grouped by date key (`YYYY-MM-DD`).
...
// [Claude] Task: pass widgetData as individual named params
// Prompt: "pass widgetData as individual named params, rather than the aggregated params"
// Date: 2026-02-23 | Model: claude-sonnet-4-5-20250929
function renderActiveComponents(createElementFunction, buildAgendaTasksByDate, completedTasks,
    currentDate, dailyVictoryValues, moodRatings, openTasks,
    quarterlyPlans, settings, weeklyVictoryValue, { selectedDate, onDateSelect } = {}) {
  ...
  return activeComponents.map((componentConfig, index) => {
    ...
    switch (widgetIdentifier) {
      ...
      case 'ai-plugins':
        ...
        break;
      case 'calendar':
        renderedComponent = createElementFunction(CalendarWidget, { completedTasks, currentDate, openTasks,
  ...
  return h('div', { className: 'dashboard' },
    ...
    h('div', { className: 'dashboard-grid' },
      ...renderActiveComponents(h, buildAgendaTasksByDate,
        completedTasks, currentDate, dailyVictoryValues,
        moodRatings, openTasks, quarterlyPlans,
        settings, weeklyVictoryValue,
        { selectedDate, onDateSelect: setSelectedDate }
      )
    )
  );
}
