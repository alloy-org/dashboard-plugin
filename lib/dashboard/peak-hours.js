...
export default function PeakHoursWidget({ app, selectedDate, currentDate }) {
  ...
  if (totalTasks === 0) {
    return h(WidgetWrapper, { title: widgetTitleFromId(WIDGET_ID), icon: '\u23F0', widgetId: WIDGET_ID },
  ...
  return h(WidgetWrapper, { title: widgetTitleFromId(WIDGET_ID), icon: '\u23F0', widgetId: WIDGET_ID },
    ...
    dateRange && h('div', { className: 'peak-hours-footer' },
      dateRange.start.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })
    )
  );
}
