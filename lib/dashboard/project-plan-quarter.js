...
import { isCompletedActionProspect, resolvePlanScope } from "plan-wizard/plan-models";
import { isBuilderMarkedText, isCompleteMarkedText, planSectionRange, projectBlocksFromBody } from "plan-wizard/quarterly-plan-markdown";
...
//   heading carrying the builder marker) or still held as live prospects in that quarter's vision guide. A project
//   marked Complete no longer counts, since it contributes nothing to suggest.
// @param {object} app - Host-compatible Amplenote app bridge.
...
async function _hasPlanBuilderProjects(app, { domainName, domainUuid, plan }) {
  ...
  if (blocks.some(block => isBuilderMarkedText(block.headingText) && !isCompleteMarkedText(block.headingText))) return true;
  try {
