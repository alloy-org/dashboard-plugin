...
async function findStaleTaskNotes(seed, maxNotes = MAX_NOTES) {
  ...
  const resolvedHandles = await Promise.all(
    combined.map(entry =>
      callPlugin('findNote', { uuid: entry.noteHandle.uuid })
...
// Prompt: "ensure more note candidates are returned when component is two units tall"
// Date: 2026-03-07 | Model: claude-4.6-opus-high-thinking
export default function RecentNotesWidget({ gridHeightSize = 1 }) {
  ...
  useEffect(() => {
    setStaleNotes(null);
    setError(null);
    findStaleTaskNotes(seed, maxNotes)
      .then(results => {
        setStaleNotes(results);
        console.log("[RecentNotes] staleNotes:", results, "seed:", seed, "reseedCount:", reseedCount);
      })
      .catch(err => {
  ...
  return h(WidgetWrapper, { title: 'Revisit Candidates', icon: '📝', widgetId: 'recent-notes', headerActions: reseedButton },
    renderBody()
  );
}
