...
const MIN_NOTES = 3;
const MAX_NOTES = 5;
const MAX_HISTORY_UUIDS_PER_DAY = 25;
...
function normalizeRecentNotesHistory(value) {
  ...
  for (const dayName of WEEKDAY_NAMES) {
    ...
    const validUuids = dayUuids.filter(uuid => typeof uuid === 'string' && uuid.length > 0);
    normalized[dayName] = [...new Set(validUuids)].slice(-MAX_HISTORY_UUIDS_PER_DAY);
  }
  return normalized;
}
...
// [GPT-5.5] Task: bound note-handle collection to reduce mobile memory and API pressure
// Prompt: "evaluate the rest of the component for any unbounded memory or looping that might lead to app crashing"
// Date: 2026-04-29 | Model: GPT-5.5
...
async function findStaleTaskNotes(app, seed, maxNotes = MAX_NOTES) {
  ...
  for (const noteHandle of notes) {
    if (staleCandidates.length >= MAX_STALE_CANDIDATES) break;
    try {
