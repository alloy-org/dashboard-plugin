...
async function findStaleTaskNotes() {
  ...
  // [Claude] Task: resolve note titles in parallel using app.findNote
  // Prompt: "use app.findNote to retrieve in parallel the note titles from the note handles"
  // Date: 2026-03-04 | Model: claude-4.6-sonnet-medium-thinking
  const resolvedHandles = await Promise.all(
    staleNotes.map(entry =>
...
export default function RecentNotesWidget() {
  ...
  useEffect(() => {
    findStaleTaskNotes().then(results => {
      setStaleNotes(results);
      console.log("[RecentNotes] rendering RecentNotesWidget, staleNotes:", results);
    }).
    catch(err => {
