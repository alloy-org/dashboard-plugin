...
// ----------------------------------------------------------------------------------------------
// @desc Read the tasks already sitting in a date's agenda note without creating the note when it is absent.
//   Browsing to a date in the widget must not leave a trail of empty "Proposed Agenda" notes behind it, so
//   generation uses this while the Accept-agenda flow keeps using prepareAgendaNote to write.
// @param {object} app - Amplenote app bridge.
// @param {object} options - { domainName, targetDate }.
// @returns {Promise<Array<object>>} Existing tasks, or an empty array when the note does not exist yet.
export async function existingAgendaNoteTasks(app, { domainName, targetDate }) {
  const name = `Proposed Agenda ${ dateKeyFromDateInput(targetDate) } ${ domainName || "All Notes" }`;
  const note = checkedAppResult(await app.findNote({ name }));
  if (!note?.uuid) return [];
  const tasks = checkedAppResult(await app.getNoteTasks({ uuid: note.uuid }, { includeDone: true }));
