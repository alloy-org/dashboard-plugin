...
export const FOCUS_WINDOW_COLOR_COUNT = 5;
...
// ----------------------------------------------------------------------------------------------
// @desc Keep named projects that are still in play, so Not now, removed, and completed work never appear on the
//   timeline.
...
function windowDraftFromProspect(prospect, window, quarterMonths, index) {
  ...
  return { approvalStatusEm: prospect.approvalStatusEm, colorIndex: focusWindowColorIndex(index),
    deadlineOn: prospect.deadlineOn ?? null, endOn: window.endOn, focusMonths, paceEm: prospect.paceEm ?? null,
    preferredWeekdays: prospect.preferredWeekdays ?? prospect.preferredDows ?? [], priorityEm: prospect.priorityEm,
