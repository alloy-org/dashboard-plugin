// Translate the Name the quarter page into stored records: three name ideas drawn from Focus projects, and a
// start/end window per active project that becomes ActionProspect.focusMonths. Defaults stay on or before a
// sprint deadline so a suggested bar cannot land after the date the work has to be done.
...
// ----------------------------------------------------------------------------------------------
// @desc Seed one timeline row per active project. Stored focusMonths win when present; otherwise the window is
//   staggered and then clamped to the project's deadline.
...
export function draftWindowsFromProspects(prospects = [], scope) {
  ...
  return activeProspects.map((prospect, index) => {
    ...
    const window = storedMonths.length
      ? windowFromFocusMonths({ deadlineOn: prospect.deadlineOn, focusMonths: storedMonths, quarterEndOn, quarterStartOn })
      : defaultFocusWindow({ deadlineOn: prospect.deadlineOn, index, quarterEndOn, quarterStartOn,
        totalCount: activeProspects.length });
...
// ----------------------------------------------------------------------------------------------
// @desc Detect whether any timeline draft's months differ from what is stored, so Next can persist the suggested
//   defaults on a first visit without rewriting the note when the user only passed through.
