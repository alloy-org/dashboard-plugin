...
// ----------------------------------------------------------------------------------------------
// @desc Escape user and inference text before inserting it into DashboardTippy's HTML content.
// @param {unknown} value - Text-like value to render.
// @returns {string} HTML-safe text.
function htmlEscapedText(value) {
...
// ----------------------------------------------------------------------------------------------
// @desc Summarize the full project name and salient choices from earlier wizard steps for an immediate tooltip.
// @param {object} draft - Focus-window draft carrying project, priority, pace, deadline, weekday, and reasoning data.
// @returns {string} Escaped HTML content for DashboardTippy.
export function projectTooltipHtmlFromDraft(draft) {
  const paceLabels = { deadlineSprint: "Deadline sprint", maintenanceOnly: "Other / TBD",
  ...
  const priorityLabels = { monthFocus: "Month focus", quarterFocus: "Focus this quarter", stayWarm: "Keep warm" };
  const rows = [
    ["Category", draft.userCategoryEm === "personal" ? "Personal" : "Professional"],
    ["Priority", priorityLabels[draft.priorityEm]],
    ["Pace", paceLabels[draft.paceEm]],
    ["Preferred days", (draft.preferredWeekdays ?? []).map(day => `${ day[0].toUpperCase() }${ day.slice(1) }`).join(", ")],
    ["Deadline", draft.deadlineOn],
  ].filter(([, value]) => value);
  const rowHtml = rows.map(([label, value]) => `<div class="dashboard-tooltip-row"><span class="dashboard-tooltip-row-label">`
    + `${ htmlEscapedText(label) }</span><span class="dashboard-tooltip-row-value">${ htmlEscapedText(value) }</span></div>`);
  const reasons = (draft.substantiations ?? []).filter(Boolean).slice(0, 3);
  const reasonHtml = reasons.length
    ? `<div class="dashboard-tooltip-section"><strong>Why this project:</strong><br>${ reasons.map(htmlEscapedText).join("<br>") }</div>` : "";
  return `<div class="dashboard-tooltip-header">${ htmlEscapedText(draft.summary) }</div>${ rowHtml.join("") }${ reasonHtml }`;
}
