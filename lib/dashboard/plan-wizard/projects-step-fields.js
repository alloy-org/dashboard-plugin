...
// @returns {Array<object>} Draft rows: { approvalStatus, isStored, linkedGoalUuids, preferredWeekdays, summary,
//   userCategoryEm, uuid }.
...
export function emptyProjectRow(userCategoryEm) {
  return { approvalStatus: "humanProvided", isStored: false, linkedGoalUuids: [], preferredWeekdays: [], summary: "",
    userCategoryEm, uuid: planningRecordUuid() };
}
...
// Editing an inferred proposal affirms it: the user has taken a position, so it stops awaiting judgement.
export function prospectRecordsFromDraftRows(draftRows, capturedAt) {
  ...
  for (const row of draftRows) {
    ...
    records.push({ approvalStatus, capturedAt, linkedGoalUuids: row.linkedGoalUuids, preferredWeekdays: row.preferredWeekdays,
      substantiation, summary, userCategoryEm: row.userCategoryEm, uuid: row.uuid });
  }
  return records;
}
...
export function rejectionRecordFromRow(row, capturedAt) {
  ...
  return { approvalStatus: "humanRejected", capturedAt, decidedAt: capturedAt, linkedGoalUuids: row.linkedGoalUuids,
    preferredWeekdays: row.preferredWeekdays, substantiation, summary: row.summary.trim(),
