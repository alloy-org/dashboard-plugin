...
export function projectSectionHeadingText(project) {
  return `${ project.summary } (project:${ project.uuid })`;
}
...
export function projectSectionMarkdown(project) {
  ...
  const existingLines = _taskListMarkdown((project.relatedTaskRecords || []).map(
    task => `[${ task.taskText || "Untitled task" }](https://www.amplenote.com/notes/tasks/${ task.taskUuid })`));
  const suggestedLines = _taskListMarkdown((project.suggestedTasks || []).map(task => task.taskText));
