...
async function saveLayout(app, currentSettings, setSettings, setFocusState,
  ...
  const existingLayout = Array.isArray(currentSettings?.[SETTING_KEYS.DASHBOARD_COMPONENTS])
    ? currentSettings[SETTING_KEYS.DASHBOARD_COMPONENTS] : DEFAULT_DASHBOARD_COMPONENTS;
  ...
  setSettings(prev => ({ ...prev, [SETTING_KEYS.DASHBOARD_COMPONENTS]: newLayout }));
  setFocusState(DASHBOARD_FOCUS.DEFAULT);
}

// [Claude] Task: extract settings-save handler using app.setSetting
// Prompt: "replace non-API saveSetting/saveBackgroundImageUrl/saveBackgroundMode with app.setSetting"
// Date: 2026-03-14 | Model: claude-4.6-opus-high-thinking
...
export default function DashboardApp({ app }) {
  ...
  const activeComponents = useMemo(() => {
    const configured = settings?.[SETTING_KEYS.DASHBOARD_COMPONENTS];
  ...
  return h('div', { className: 'dashboard-outer-container', style: backgroundStyle },
    focusState === DASHBOARD_FOCUS.LAYOUT_CONFIG
      ? h(DashboardLayoutPopup, {
          currentLayout: Array.isArray(settings?.[SETTING_KEYS.DASHBOARD_COMPONENTS]) ? settings[SETTING_KEYS.DASHBOARD_COMPONENTS] : DEFAULT_DASHBOARD_COMPONENTS,
