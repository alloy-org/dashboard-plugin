...
import { apiKeyFromProvider, DASHBOARD_FOCUS, DEFAULT_DASHBOARD_COMPONENTS, SETTING_KEYS, WIDGET_REGISTRY } from 'constants/settings';
...
// [Claude] Task: pass providerApiKey prop so DreamTask retries when a new API key is added
// Prompt: "use apiKeyFromProvider to pass a providerApiKey prop that corresponds with the LLM provider"
// Date: 2026-04-04 | Model: claude-4.6-opus-high-thinking
// [Claude] Task: pass providerEm to DreamTask for provider-specific error messages
...
const PeakHoursCell = createWidgetCell('peak-hours', PeakHoursWidget, ({ app, selectedDate, currentDate }) => ({
  app,
  selectedDate,
  currentDate,
}));

// [Claude] Task: register DaySketch widget cell in dashboard grid
// Prompt: "create a DaySketch component persisted to a Day Sketch note"
// Date: 2026-03-16 | Model: claude-4.6-opus-high-thinking
const DaySketchCell = createWidgetCell('day-sketch', DaySketchWidget, ({ app, agendaTasks, currentDate }) => ({
  app,
  agendaTasks,
  currentDate,
}));
...
async function saveSettings(app, setConfigParams, setFocusState,
    { llmProvider, apiKey, apiKeyProvider, backgroundMode, backgroundImageUrl }) {
  logIfEnabled('[dashboard] handleSettingsSave called with:', { llmProvider, apiKeyProvider, backgroundMode, backgroundImageUrl: backgroundImageUrl != null ? '(set)' : '(unchanged)', });
  ...
  app.settings[SETTING_KEYS.LLM_PROVIDER] = llmProvider;
    ...
    h('div', { className: 'dashboard-content' },
      ...
      h('div', { className: `dashboard-grid${draggingWidgetId ? ' dashboard-grid--dragging' : ''}` },
        ...displayedComponents.map((config, index) => {
          ...
          const llmProvider = configParams?.[SETTING_KEYS.LLM_PROVIDER];
          const providerSettingKey = llmProvider ? apiKeyFromProvider(llmProvider) : null;
