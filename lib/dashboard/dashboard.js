...
const AgendaCell = createWidgetCell('agenda', AgendaWidget, ({ agendaTasks, app, calendarEvents, currentDate, selectedDate, timeFormat }) => ({
  app, calendarEvents, currentDate, selectedDate, tasks: agendaTasks, timeFormat,
}));
const DaySketchCell = createWidgetCell('day-sketch', DaySketchWidget, pickProps('agendaTasks', 'app', 'calendarEvents', 'currentDate', 'timeFormat'));
...
export default function DashboardApp({ app }) {
  ...
  const { calendarEvents } = useExternalCalendarEvents(app, activeTaskDomain?.uuid);
