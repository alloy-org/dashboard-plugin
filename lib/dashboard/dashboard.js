...
// ------------------------------------------------------------------------------------------
// @description Root dashboard component. Initializes app data, manages shared state (mood ratings, plans, victory values,
//   selected date), and renders the widget grid along with toolbar controls for settings and layout configuration.
// @param {Object} props
...
export default function DashboardApp({ app }) {
  ...
  const debugConsoleEnabled = String(configParams?.[SETTING_KEYS.DEBUG_CONSOLE] || '').trim() === 'true' || IS_DEV_ENVIRONMENT;
