...
import LayoutPickerWidget from 'layout-picker';
...
import { parseComponentsSeen, saveDashboardLayoutAndSeen } from "util/new-component-popin";
...
const LayoutPickerCell = createWidgetCell('layout-picker', LayoutPickerWidget,
  pickProps('currentLayout', 'onLayoutApply'));
...
async function saveLayout(app, currentConfigParams, setConfigParams, newRenderedWidgetIds, isReset = false,
  ...
  const currentComponentsSeen = parseComponentsSeen(currentConfigParams?.[SETTING_KEYS.COMPONENTS_SEEN]);
  ...
  const nextComponentsSeen = await saveDashboardLayoutAndSeen(app, currentComponentsSeen, newLayout, newRenderedWidgetIds);
  app.settings = app.settings || {};
  app.settings[SETTING_KEYS.COMPONENTS_SEEN] = nextComponentsSeen;
  app.settings[SETTING_KEYS.DASHBOARD_COMPONENTS] = newLayout;
  setConfigParams(prev => ({ ...prev, [SETTING_KEYS.COMPONENTS_SEEN]: nextComponentsSeen, [SETTING_KEYS.DASHBOARD_COMPONENTS]: newLayout }));
}
...
export default function DashboardApp({ app }) {
  ...
  // [Claude claude-sonnet-4-6] Task: persist selected layout profile alongside widget layout
  // Prompt: "handleProfileApply wraps handleLayoutPersist and also saves/clears the selected profile setting"
  const handleProfileApply = useCallback(
    async (widgetIds, isReset = false, sizing = null, profileId = undefined) => {
      await handleLayoutPersist(widgetIds, isReset, sizing);
      if (profileId !== undefined && profileId !== null) {
        app.setSetting(SETTING_KEYS.SELECTED_LAYOUT_PROFILE, profileId);
        setConfigParams(prev => ({ ...prev, [SETTING_KEYS.SELECTED_LAYOUT_PROFILE]: profileId }));
      } else if (profileId === null) {
        app.setSetting(SETTING_KEYS.SELECTED_LAYOUT_PROFILE, '');
        setConfigParams(prev => {
          const next = { ...prev };
          delete next[SETTING_KEYS.SELECTED_LAYOUT_PROFILE];
          return next;
        });
      }
    },
    [app, handleLayoutPersist]
  );

  const handleLayoutSave = useCallback(
    async (newRenderedWidgetIds, isReset = false, sizing = null, profileId = undefined) => {
      await handleProfileApply(newRenderedWidgetIds, isReset, sizing, profileId);
      setFocusState(DASHBOARD_FOCUS.DEFAULT);
    },
    [handleProfileApply]
  );
  ...
  const { activeComponents } = useDashboardLayout({ configParams, onLayoutPersist: handleLayoutPersist });
        ...
        h('div', {
          ...
          ...displayedComponents.map((config, index) => {
            ...
            return h(CellComponent, {
              ...
              currentLayout: Array.isArray(configParams?.[SETTING_KEYS.DASHBOARD_COMPONENTS]) ? configParams[SETTING_KEYS.DASHBOARD_COMPONENTS] : DEFAULT_DASHBOARD_COMPONENTS,
              onLayoutApply: handleProfileApply,
