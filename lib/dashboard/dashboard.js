...
// [Claude] Task: pass llmApiKey prop so DreamTask retries when a new API key is added
// Prompt: "update the input props for dream-task.js such that it will retry its query"
// Date: 2026-04-04 | Model: claude-4.6-opus-high-thinking
const DreamTaskCell = createWidgetCell('dream-task', DreamTaskWidget, ({ app, config, llmApiKey, onOpenSettings }) => ({
  gridHeightSize: Number(config?.gridHeightSize) || 1,
...
// ------------------------------------------------------------------------------------------
// @async
// @description Persists LLM provider, API key, and background image settings via app.setSetting,
//   then updates the local configParams state to reflect the saved values
// @param {Object} app - Application instance with setSetting method
// @param {Function} setConfigParams - State setter for config parameters
// @param {Function} setFocusState - State setter for dashboard focus/popup state
// @param {Object} settings - Settings to save
// @param {string} settings.llmProvider - Selected LLM provider identifier
// @param {string} settings.apiKey - API key for the LLM provider
...
async function saveSettings(app, setConfigParams, setFocusState,
  ...
  // [Claude] Task: sync app.settings so widgets reading it directly pick up the new values
  // Prompt: "update the input props for dream-task.js such that it will retry its query"
  // Date: 2026-04-04 | Model: claude-4.6-opus-high-thinking
  app.settings[SETTING_KEYS.LLM_PROVIDER] = llmProvider;
  app.settings[SETTING_KEYS.LLM_API_KEY] = apiKey;
      ...
      h('div', { className: `dashboard-grid${draggingWidgetId ? ' dashboard-grid--dragging' : ''}` },
        ...displayedComponents.map((config, index) => {
          ...
          return h(CellComponent, {
            ...
            draggingWidgetId,
            llmApiKey: configParams?.[SETTING_KEYS.LLM_API_KEY] || '',
            moodRatings,
