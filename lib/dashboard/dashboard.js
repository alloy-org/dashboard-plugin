...
export default function DashboardApp() {
  ...
  // [Claude] Task: refetch mood ratings, extending range to cover previous week when early in current week
  // Prompt: "when beginning of new week with less than 3 full days of stats, show previous week"
  // Date: 2026-03-09 | Model: claude-4.6-opus-high-thinking
  ...
  const handleLayoutSave = useCallback(async (newRenderedWidgetIds, { isReset, sizing } = {}) => {
    const existingLayout = Array.isArray(settings?.[DASHBOARD_COMPONENTS]) ? settings[DASHBOARD_COMPONENTS] : DEFAULT_DASHBOARD_COMPONENTS;
