...
export default function DashboardApp({ app }) {
  ...
  const debugConsoleEnabled = String(configParams?.[SETTING_KEYS.DEBUG_CONSOLE] || '').trim() === 'true' || IS_DEV_ENVIRONMENT || app.context.pluginUUID === "6da03574-0f4b-11f1-ba9e-11ba9c716f59";
  if (debugConsoleEnabled) {
    logIfEnabled('[dashboard] Debug console enabled, including in layout popup');
  } else {
    logIfEnabled(`[dashboard] Debug console disabled (${ SETTING_KEYS.DEBUG_CONSOLE } is '${ configParams?.[SETTING_KEYS.DEBUG_CONSOLE] }', pluginUUID ${ app.context.pluginUUID }), excluding from layout popup`);
  }
