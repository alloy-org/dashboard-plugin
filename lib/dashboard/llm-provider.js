...
// ----------------------------------------------------------------------------------------------
// @desc Resolve which provider options the chooser should display.
// @param {object} options - { allowKeyless, configuredProviderEms, currentProviderEm }.
//   - {boolean} [allowKeyless=false] - When true, show all offered providers (Agent Pro fallback path).
//   - {Array<string>} [configuredProviderEms=[]] - Provider enums that have an API key configured.
//   - {string|null} [currentProviderEm=null] - Provider enum to always keep visible.
// @returns {Array<object>} The provider options to render.
export function selectableProviderOptions({ allowKeyless = false, configuredProviderEms = [],
    currentProviderEm = null } = {}) {
  if (allowKeyless) return LLM_PROVIDER_OPTIONS;
  const allowed = new Set(configuredProviderEms);
  const filtered = LLM_PROVIDER_OPTIONS.filter(option => allowed.has(option.providerEm)
    || option.providerEm === currentProviderEm);
  return filtered.length > 0 ? filtered : LLM_PROVIDER_OPTIONS;
}
