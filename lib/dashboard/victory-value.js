...
// [Claude] Task: centralize widget config parsing and defaults
...
function getCurrentConfig(settings) {
  return settings?.['dashboard_victory-value_config'] || [];
}

// [Claude] Task: parse mood visibility setting from persisted config
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
function isMoodSettingEnabled(rawValue) {
  return rawValue === 'true' || rawValue === true;
}

// [Claude] Task: normalize max bar value for chart scaling
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
function getMaxValue(dailyValues) {
  return Math.max(...dailyValues.map((entry) => entry.value), 1);
}

// [Claude] Task: build persisted config payload
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
function getConfigResult(timeRange, showMood) {
  return [timeRange, String(showMood)];
}

// [Claude] Task: convert time range key into UI label
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
...
// [Claude] Task: open widget config popup
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
function handleOpenConfig(setConfigOpen) {
  setConfigOpen(true);
}
...
async function handleConfigSubmit(timeRange, showMood, setConfigOpen) {
  const result = getConfigResult(timeRange, showMood);
...
function handleConfigCancel(currentConfig, setTimeRange, setShowMood, setConfigOpen) {
  ...
  setShowMood(isMoodSettingEnabled(currentConfig[1]));
  setConfigOpen(false);
}
...
// [Claude] Task: clear hovered bar when pointer leaves chart
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
function handleCanvasMouseLeave(setHoveredBar) {
  setHoveredBar(null);
}

// [Claude] Task: update mood overlay checkbox state from event
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
function handleMoodOverlayChange(setShowMood, event) {
  setShowMood(event.target.checked);
}

// [Claude] Task: update selected time range from radio option
// Prompt: "all possible functions are local functions, not embedded in the component"
// Date: 2026-02-28 | Model: gpt-5.3-codex
function handleTimeRangeChange(setTimeRange, value) {
  setTimeRange(value);
}
...
function renderTimeRangeOptions(h, timeRange, setTimeRange) {
  return ['week', 'month', '30days'].map((value) => h('label', { key: value },
    h('input', {
      ...
      onChange: handleTimeRangeChange.bind(null, setTimeRange, value)
    }),
...
export default function VictoryValueWidget({ dailyValues, weeklyTotal, moodRatings, completedTasksByDate, settings }) {
  ...
  const currentConfig = getCurrentConfig(settings);
  const [timeRange, setTimeRange] = useState(currentConfig[0] || 'week');
  const [showMood, setShowMood] = useState(isMoodSettingEnabled(currentConfig[1]));
  const onConfigure = handleOpenConfig.bind(null, setConfigOpen);
  ...
  const onMoodOverlayChange = handleMoodOverlayChange.bind(null, setShowMood);
  const onCanvasMouseMove = handleCanvasMouseMove.bind(null, canvasRef, setHoveredBar);
  const onCanvasMouseLeave = handleCanvasMouseLeave.bind(null, setHoveredBar);
