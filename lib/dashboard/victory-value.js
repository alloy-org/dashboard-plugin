...
// [Claude] Task: decide whether tooltip should pop above or below chart based on available window space
...
function getTooltipDirection(canvasRef) {
  const canvas = canvasRef.current;
  if (!canvas) return 'above';
  const rect = canvas.getBoundingClientRect();
...
function getHoveredDayMood(index, moodRatings, dailyValues) {
...
// Prompt: "change the line between missing dates to be a dashed line"
// Date: 2026-03-05 | Model: claude-sonnet-4-6
function drawMoodOverlay(ctx, moodRatings, chartDailyValues, barW, chartH) {
  const today = new Date();
  today.setHours(23, 59, 59, 999);
...
function drawChart(canvasRef, dailyValues, maxValue, moodRatings, showMood) {
  const canvas = canvasRef.current;
  logIfEnabled("Drawing chart in VictoryValue with dailyValues", dailyValues, "and moodRatings", moodRatings);
  if (!canvas) return;
  ...
  if (showMood && moodRatings && moodRatings.length > 0) {
    drawMoodOverlay(ctx, moodRatings, dailyValues, barW, chartH);
  }
}
...
// --------------------------------------------------------------------------------------------------
// [Claude] Task: render tooltip element from hovered state, direction-aware above/below placement
// Prompt: "calculate whether more space above or below chart and pop tooltip on whichever side"
// Date: 2026-03-08 | Model: claude-sonnet-4-6
function renderTooltip(h, hoveredBar, dailyValues, completedTasksByDate, moodRatings, canvasRef) {
  ...
  const mood = hoveredBar !== null ? getHoveredDayMood(hoveredBar, moodRatings, dailyValues) : null;
  const dateLabel = hoveredBar !== null && dailyValues[hoveredBar]
    ? tooltipLabelFromDateInput(dailyValues[hoveredBar].date)
    : '';
  const direction = hoveredBar !== null ? getTooltipDirection(canvasRef) : 'above';
