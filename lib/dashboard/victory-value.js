...
// Prompt: "calculate whether more space above or below chart and pop tooltip on whichever side"
// Date: 2026-03-08 | Model: claude-sonnet-4-6
function renderTooltip(h, hoveredBar, dailyValues, completedTasksByDate, moodByDay, canvasRef) {
  const tasks = hoveredBar !== null ? getHoveredDayTasks(hoveredBar, dailyValues, completedTasksByDate) : [];
  const mood = hoveredBar !== null ? getHoveredDayMood(hoveredBar, moodByDay, dailyValues) : null;
  ...
  return h(WidgetWrapper, {
    ...
    h('div', { className: 'vv-chart-container' },
      ...
      h('div', { className: 'vv-chart-wrap' },
        ...
        renderTooltip(h, hoveredBar, chartDailyValues, completedTasksByDate, moodByDay, canvasRef)
      ),
      h('button', {
        className: 'vv-nav-arrow',
        type: 'button',
