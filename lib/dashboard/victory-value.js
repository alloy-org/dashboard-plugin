...
import { widgetTitleFromId, parseWidgetConfig } from "../constants/settings"
...
async function handleConfigSubmit(app, timeRange, showMood, setConfigOpen) {
  ...
  await app.setSetting('dashboard_victory-value_config', JSON.stringify(result));
  setConfigOpen(false);
}
...
// Prompt: "replace hand-rolled tooltips with tippy.js"
// Date: 2026-03-09 | Model: claude-4.6-opus-high-thinking
export default function VictoryValueWidget({ app, completedTasksByDate, dailyValues, moodRatings, onReferenceDateChange, referenceDate, weeklyTotal }) {
  ...
  const currentConfig = parseWidgetConfig(app, 'victory-value');
