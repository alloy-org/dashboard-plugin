...
function buildDailyValuesFromCompletedTasks(dailyValues, completedTasksByDate) {
  ...
  return dailyValues.map((entry) => {
    const dateKey = dateIsoToKey(entry.date);
...
export default function VictoryValueWidget({ dailyValues, weeklyTotal, moodRatings, completedTasksByDate, settings }) {
  ...
  const chartDailyValues = buildDailyValuesFromCompletedTasks(dailyValues, completedTasksByDate);
  ...
  return h(WidgetWrapper, {
    ...
    h('div', { className: 'vv-header' },
      h('span', { className: 'vv-total' }, chartWeeklyTotal || weeklyTotal || 0),
