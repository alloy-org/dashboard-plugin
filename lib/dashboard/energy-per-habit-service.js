...
import {
  aggregateMonthlyHabits, computeMonthlyAggregates, habitGroupKey, habitLabelFromContent,
...
function logDerivedHabits(completedTasks) {
  ...
  for (const task of completedTasks || []) {
    ...
    seen.set(key, { label: habitLabelFromContent(task.content), sample: String(task.content).slice(0, 140) });
  }
  ...
  for (const [key, info] of seen.entries()) {
    logIfEnabled(`[${WIDGET_ID}]   key="${key}" label="${info.label}" ⟵ content=${JSON.stringify(info.sample)}`);
  }
}
