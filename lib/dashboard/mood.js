...
function RadialRing({ moodData }) {
  ...
  const handleCanvasMouseMove = (e) => {
    ...
    if (index >= 0 && index < geo.segmentCount) {
      ...
      setTooltip({ x: mx, y: my, data: { fullDate: entry.fullDate, entries: entry.entries || [entry] } });
    } else {
      setTooltip(null);
    }
  };
...
function WaveGraph({ moodData }) {
  ...
  const handleCanvasMouseMove = (e) => {
    ...
    if (closestIdx >= 0) {
      ...
      setTooltip({ x: pt.x, y: pt.y, data: { fullDate: entry.fullDate, entries: entry.entries || [entry] } });
    } else {
      setTooltip(null);
    }
  };
