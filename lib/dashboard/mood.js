...
async function handleSubmit(app, selectedMood, notes, setSubmitting, setSubmitted, onMoodRecorded) {
  ...
  try {
    ...
    await app.saveMoodNote(selectedMood.value, selectedMood.label, notes.trim());
    setSubmitted(true);
    if (onMoodRecorded) {
...
// Date: 2026-03-07 | Model: claude-4.6-sonnet-medium-thinking
export default function MoodWidget({ app, moodRatings, onMoodRecorded, settings }) {
  const h = createElement;
  ...
  const handleConfigureSubmit = () => {
    ...
    app.saveSetting('mood', [pendingVisualizationMode]);
  };
