...
const WAVE_LABEL_OPACITY_SCALE = 0.85;
const WAVE_DAY_LABEL_Y_OFFSET  = 14; // px below graphHeight baseline
const WAVE_DATE_LABEL_Y_OFFSET = 24; // px below graphHeight baseline
...
// ---------------------------------------------------------------------------------------------------------
function renderMoodTooltip(h, tooltip) {
  if (!tooltip || !tooltip.data) return null;
  const { fullDate, entries } = tooltip.data;
  return h('div', {
    className: 'mood-viz-tooltip',
    style: { left: tooltip.x + 'px', top: tooltip.y + 'px' },
  },
    h('div', { className: 'mood-viz-tooltip-date' }, fullDate),
    entries.map((entry, i) =>
      h('div', { key: i, className: 'mood-viz-tooltip-entry' },
        h('span', { className: 'mood-viz-tooltip-time' }, entry.time),
        h('span', { className: 'mood-viz-tooltip-mood' },
          MOODS.find(m => m.value === entry.rating)?.emoji || '❓',
          ' ',
          pluginMoodLabel(entry.rating)
        ),
        entry.notes ? h('div', { className: 'mood-viz-tooltip-note' }, entry.notes) : null
      )
    )
  );
}
...
function RadialRing({ moodData }) {
  ...
  const handleCanvasMouseMove = (e) => {
    ...
    if (!geo || !geo.segmentCount) { setTooltip(null); return; }
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    if (index >= 0 && index < geo.segmentCount) {
      const dateKey = moodData[index].dateKey;
      const entries = moodData.filter(d => d.dateKey === dateKey);
      setTooltip({ x: mx, y: my, data: { fullDate: moodData[index].fullDate, entries } });
    } else {
      setTooltip(null);
    }
  };

  const handleCanvasMouseLeave = () => setTooltip(null);
...
function WaveGraph({ moodData }) {
  ...
  useEffect(() => {
    ...
    for (let v = 1; v <= 5; v++) {
      ...
      ctx.textAlign = 'right';
      ctx.textBaseline = 'middle';
      ctx.fillText(v, paddingLeft - WAVE_AXIS_LABEL_X_GAP, y);
    }
    ...
    const animatedPoints = points.map((point) => ({
      ...point,
      y: paddingTop + graphHeight - ((point.val - 1) / 4) * graphHeight * Math.min(progress * WAVE_ANIM_SCALE, 1),
    }));
  ...
  const handleCanvasMouseMove = (e) => {
    ...
    if (!pts || !pts.length) { setTooltip(null); return; }
    let closestIdx = -1;
    ...
    if (closestIdx >= 0) {
      ...
      const dateKey = moodData[closestIdx].dateKey;
      const entries = moodData.filter(d => d.dateKey === dateKey);
      setTooltip({ x: pt.x, y: pt.y, data: { fullDate: moodData[closestIdx].fullDate, entries } });
    } else {
      setTooltip(null);
    }
  };
