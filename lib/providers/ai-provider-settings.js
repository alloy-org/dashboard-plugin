import { apiKeyFromProvider, IS_TEST_ENVIRONMENT, SETTING_KEYS } from "constants/settings"
import {
...
export function preferredModels(app) {
  const providerEm = app.settings[SETTING_KEYS.LLM_PROVIDER];
  if (!providerEm || providerEm === 'none') return [];
  const models = MODELS_PER_PROVIDER[providerEm];
  if (!models || models.length === 0) return [];
  const defaultModel = PROVIDER_DEFAULT_MODEL[providerEm];
  if (defaultModel) {
