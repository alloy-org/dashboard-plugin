...
// --------------------------------------------------------------------------
// [Claude claude-opus-4-7] Task: skip callPlugin on mobile, where Ample Agent Pro is unavailable
// Prompt: "only call app.callPlugin when the platform is not mobile"
// @description Attempts to send a prompt string to the Ample Agent Pro plugin via app.callPlugin.
//   If the plugin returns a falsy value (null, undefined, empty string), falls back to querying
//   the LLM directly via llmPrompt with the same prompt and options. On mobile platforms the
//   plugin call is skipped entirely and the LLM path is used directly.
// @param {Object} app - Amplenote app interface with callPlugin and settings
// @param {string} prompt - The prompt string to send
// @param {Object} [options] - Options forwarded to llmPrompt on fallback (aiModel, apiKey, jsonResponse, etc.)
// @returns {Promise<*>} The plugin response, or the LLM response if the plugin returned falsy
export async function llmPromptWithPluginFallback(app, prompt, options = {}) {
  // [Claude claude-opus-4.7] Task: capture which fallback path ran (callPlugin vs llmPrompt) with timing
  // Prompt: "add adequate debugging to understand why all of these failed"
  ...
  logIfEnabled("[llmPromptWithPluginFallback] starting", { promptLength: prompt?.length ?? 0,
    ...
    jsonResponse: !!options.jsonResponse, aiModelOverride: options.aiModel ?? null,
    apiKeyOverridePresent: !!options.apiKey });
  try {
    ...
    if (pluginResult) {
      logIfEnabled("[llmPromptWithPluginFallback] callPlugin returned", {
        durationMs: pluginDurationMs, resultType: typeof pluginResult,
        resultLength: typeof pluginResult === "string" ? pluginResult.length : null,
      }, ", skipping LLM fallback");
      return options.jsonResponse && typeof pluginResult === "string"
        ? extractJsonFromString(pluginResult)
        : pluginResult;
    }
    logIfEnabled("[llmPromptWithPluginFallback] callPlugin returned falsy, falling back to llmPrompt", {
      durationMs: pluginDurationMs, pluginResultValue: pluginResult,
    });
  } catch (error) {
    logIfEnabled("[llmPromptWithPluginFallback] callPlugin failed, falling back to llmPrompt:", error,
      { durationMs: Number((performance.now() - pluginStart).toFixed(1)) });
  }
  const llmStart = performance.now();
  const llmResult = await llmPrompt(app, null, prompt, options);
  logIfEnabled('[llmPromptWithPluginFallback] llmPrompt fallback complete', {
    durationMs: Number((performance.now() - llmStart).toFixed(1)),
    resultType: llmResult === null ? 'null' : typeof llmResult,
    hasTasksField: !!(llmResult && typeof llmResult === 'object' && 'tasks' in llmResult),
  });
  return llmResult;
}
