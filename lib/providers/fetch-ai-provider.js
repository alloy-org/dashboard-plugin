...
export async function llmPromptWithPluginFallback(app, prompt, { jsonResponse, aiModel, apiKey, timeoutSeconds } = {}) {
  ...
  try {
    ...
    if (typeof(pluginResult) === "undefined") {
      ...
      logIfEnabled("[llmPromptWithPluginFallback] callPlugin returned undefined; checking for Ample Agent Pro note existence, got", agentProNote);
      if (agentProNote) {
        pluginResult = await app.callPlugin({ uuid: agentProNote.uuid }, "", prompt, aiModel);
        logIfEnabled(`[llmPromptWithPluginFallback] Attempt to callPlugin for note uuid ${ agentProNote.uuid } yielded`, pluginResult);
      }
    }
    ...
    logIfEnabled(`[llmPromptWithPluginFallback] callPlugin returned '${ pluginResult }' (typeof ${ typeof pluginResult }) — falling back to direct LLM`, { durationMs: pluginDurationMs, pluginResultValue: pluginResult, pluginResultType: typeof pluginResult, });
  } catch (error) {
