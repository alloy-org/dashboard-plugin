...
export async function llmPromptWithPluginFallback(app, prompt, options = {}) {
  if (isMobilePlatform()) {
    logIfEnabled("[llmPromptWithPluginFallback] mobile platform detected, skipping callPlugin");
    return llmPrompt(app, null, prompt, options);
  }
  try {
    const pluginResult = await app.callPlugin({ uuid: AMPLE_AGENT_PRO_UUID }, prompt);
    if (pluginResult) {
      logIfEnabled("[llmPromptWithPluginFallback] callPlugin returned", pluginResult, ", skipping LLM fallback");
