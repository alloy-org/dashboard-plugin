...
export async function llmPromptWithPluginFallback(app, prompt, { jsonResponse, aiModel, apiKey, timeoutSeconds } = {}) {
  ...
  logIfEnabled("[llmPromptWithPluginFallback] starting", { promptLength: prompt?.length ?? 0, ampleAgentProUuid: AMPLE_AGENT_PRO_UUID, hasCallPlugin: typeof app?.callPlugin === "function", jsonResponse: !!jsonResponse, aiModelOverride: aiModel ?? null, apiKeyOverridePresent: !!apiKey });
  try {
    const pluginResult = await app.callPlugin({ source: AMPLE_AGENT_PRO_UUID }, prompt);
