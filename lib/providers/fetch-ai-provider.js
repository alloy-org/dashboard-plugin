import { AMPLE_AGENT_PRO_NOTE_NAME, AMPLE_AGENT_PRO_UUID, defaultProviderModel, preferredModels, providerFromModel,
  providerNameFromProviderEm, providerEndpointUrl, apiKeyFromApp } from "providers/ai-provider-settings"
import {
...
import { STREAMING_PROVIDERS, streamedResponseText } from "providers/stream-response-text"
...
// @param {string|null|undefined} reasoningEffort - OpenAI reasoning budget ("low", "medium", "high"). A caller on a
//   fixed latency budget passes "low" so a reasoning model spends its time answering rather than deliberating past
//   the deadline; ignored by every provider that is not OpenAI
// @param {Function|null|undefined} onPartialText - When given and the provider can stream, the response is streamed
//   and this receives the answer text accumulated so far as it arrives; the return value is still the whole answer.
//   A provider that cannot stream answers in one piece and never calls it
// [Composer] Task: discrete parameters instead of bundled options for llmPrompt
// [GPT-5.6 Terra] Task: report an absent LLM provider before resolving a provider model
export async function llmPrompt(app, plugin, prompt, aiModel, apiKey, jsonResponse, timeoutSeconds, reasoningEffort,
    onPartialText) {
  ...
  // A disabled or absent provider leaves no candidate model. This is a valid user configuration,
  // not a provider lookup failure, so report the setting the user can act on before calling
  // providerFromModel(undefined).
  if (!modelToUse) {
    throw new Error("No AI provider is configured. Choose a provider in Dashboard Settings.");
  }
  ...
  if (reasoningEffort) requestOptions.reasoningEffort = reasoningEffort;
  const providerEm = providerFromModel(modelToUse);
  const streamsResponse = typeof onPartialText === "function" && STREAMING_PROVIDERS.includes(providerEm);
  if (streamsResponse) requestOptions.stream = true;
  const requestStartedAt = Date.now();
  const fetchResponse = await makeRequest(app, messages, modelToUse, requestOptions);

  if (streamsResponse) {
    // The caller's timeout covers the whole request, so the read gets whatever the fetch left of it.
    const elapsedSeconds = (Date.now() - requestStartedAt) / 1000;
    const remainingSeconds = Math.max((timeoutSeconds || TIMEOUT_SECONDS) - elapsedSeconds, 1);
    const streamedText = await streamedResponseText(fetchResponse, providerEm, { onPartialText, timeoutSeconds: remainingSeconds });
    return jsonResponse ? extractJsonFromString(streamedText) : streamedText;
  }

  const promptKey = jsonResponse ? "llmPromptJson" : null;
  // The caller's timeout governs the whole request, not only the fetch: without passing requestOptions through,
  // reading the response body falls back to its own 30s default and a caller asking for longer never gets it.
...
// @param {string} prompt - Prompt string; forwarded to the sibling plugin as the user `input` (blank `instructions`)
...
// @param {string|null|undefined} params.aiModel - Model override; forwarded to the sibling plugin call and used for LLM fallback
...
export async function llmPromptWithPluginFallback(app, prompt, { jsonResponse, aiModel, apiKey, timeoutSeconds } = {}) {
  logIfEnabled("[llmPrompt] starting", { promptLength: prompt?.length ?? 0, ampleAgentProUuid: AMPLE_AGENT_PRO_UUID, hasCallPlugin: typeof app?.callPlugin === "function", jsonResponse: !!jsonResponse, aiModelOverride: aiModel ?? null, apiKeyOverridePresent: !!apiKey });
  const pluginResult = await agentProPrompt(app, prompt, { aiModel, jsonResponse });
  if (pluginResult !== null && typeof pluginResult !== "undefined") return pluginResult;
...
// --------------------------------------------------------------------------
// @desc Ask Ample Agent Pro to run a prompt on the user's behalf, returning its parsed result or null when the
//   plugin is unavailable, declines, or throws. This is the plugin leg of llmPromptWithPluginFallback on its own,
//   so a caller that wants to race Agent Pro against a direct provider call rather than wait for it in sequence
//   can do so without reimplementing the note-uuid retry the sibling-plugin lookup needs.
//
//   A null return is the only failure signal: Agent Pro not being installed is an ordinary outcome rather than an
//   error, and a caller racing this against another source must not see a rejection that would settle the race.
// @param {Object} app - Amplenote app exposing callPlugin and findNote.
// @param {string} prompt - Prompt forwarded as the sibling plugin's user `input`, with blank `instructions`.
// @param {Object} params - An object with the following properties:
//   - {string|null|undefined} aiModel - Model id forwarded to the plugin so it knows which model to run
//   - {boolean|undefined} jsonResponse - When truthy, a string result is parsed as JSON before it is returned
// @returns {Promise<*|null>} The plugin's parsed result, or null when it produced nothing usable.
export async function agentProPrompt(app, prompt, { aiModel, jsonResponse } = {}) {
  const pluginStart = performance.now();
  try {
    let pluginResult = await app.callPlugin({ source: AMPLE_AGENT_PRO_UUID }, "", prompt, aiModel);
    if (typeof(pluginResult) === "undefined") {
      const agentProNote = await app.findNote({ name: AMPLE_AGENT_PRO_NOTE_NAME });
      logIfEnabled("[agentProPrompt] callPlugin returned undefined; checking for Ample Agent Pro note existence, got", agentProNote);
      if (agentProNote) {
        pluginResult = await app.callPlugin({ uuid: agentProNote.uuid }, "", prompt, aiModel);
        logIfEnabled(`[agentProPrompt] Attempt to callPlugin for note uuid ${ agentProNote.uuid } yielded`, pluginResult);
      }
    }
    ...
    if (pluginResult) {
      ...
      logIfEnabled("[agentProPrompt] callPlugin raw result", { durationMs: pluginDurationMs, resultType: typeof pluginResult, resultLength: typeof pluginResult === "string" ? pluginResult.length : null, resultSnippet, });
      const parsed = jsonResponse && typeof pluginResult === "string"
        ? extractJsonFromString(pluginResult)
        : pluginResult;
      logIfEnabled("[agentProPrompt] callPlugin parsed result", { parsedType: parsed === null ? "null" : typeof parsed, hasTasksField: !!(parsed && typeof parsed === "object" && "tasks" in parsed), tasksLength: Array.isArray(parsed?.tasks) ? parsed.tasks.length : null, hasGoalsSummary: !!(parsed && typeof parsed === "object" && parsed.goalsSummary), });
      return parsed;
    }
    logIfEnabled(`[agentProPrompt] callPlugin returned '${ pluginResult }' (typeof ${ typeof pluginResult })`, { durationMs: pluginDurationMs, pluginResultValue: pluginResult, pluginResultType: typeof pluginResult, });
  } catch (error) {
    logIfEnabled("[agentProPrompt] callPlugin threw", { durationMs: Number((performance.now() - pluginStart).toFixed(1)), errorMessage: error?.message, errorName: error?.name, });
  }
  return null;
}
...
// @param {string|null} reasoningEffort - OpenAI reasoning budget ("low", "medium", "high"); only the OpenAI branch
//   carries it, because every other provider here rejects an unknown body field outright
// @returns {{ model: string, messages: Array, stream: boolean, max_tokens?: number, system?: string,
//   tools?: Array, frequency_penalty?: number, reasoning_effort?: string }}
function requestBodyForProvider(messages, model, stream, tools, { promptKey = null, reasoningEffort = null } = {}) {
  let body;
  const providerEm = providerFromModel(model);
  ...
  switch (providerEm) {
    ...
    default: {
      ...
      if (tools) body.tools = tools;
      body.response_format = { type: "json_object" };
      // Only a genuine OpenAI model takes this: the default branch also catches models whose provider did not
      // match any case above, and sending them an unrecognized field would fail the request outright.
      if (reasoningEffort && providerEm === "openai") body.reasoning_effort = reasoningEffort;
      break;
    }
  }
...
async function makeRequest(app, messages, model, { apiKey = null, attemptNumber = 1, promptKey = null,
    reasoningEffort = null, stream = null, timeoutSeconds = TIMEOUT_SECONDS, tools = null } = {}) {
  const providerEm = providerFromModel(model);
  ...
  const body = requestBodyForProvider(messages, model, stream, tools, { promptKey, reasoningEffort });
