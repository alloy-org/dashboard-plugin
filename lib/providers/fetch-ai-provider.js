...
// --------------------------------------------------------------------------
// [Claude claude-opus-4-6] Task: generalizable callPlugin-with-LLM-fallback for Ample Agent Pro
// Prompt: "create a function that calls callPlugin with a prompt, falling back to llmPrompt if falsy"
...
//   the LLM directly via llmPrompt with the same prompt and options.
...
export async function llmPromptWithPluginFallback(app, prompt, options = {}) {
  try {
    ...
    if (pluginResult) {
      logIfEnabled("[llmPromptWithPluginFallback] callPlugin returned result, skipping LLM fallback");
      return options.jsonResponse && typeof pluginResult === "string"
        ? extractJsonFromString(pluginResult) : pluginResult;
    }
    logIfEnabled("[llmPromptWithPluginFallback] callPlugin returned falsy, falling back to llmPrompt");
  } catch (err) {
    logIfEnabled("[llmPromptWithPluginFallback] callPlugin failed, falling back to llmPrompt:", err);
  }
