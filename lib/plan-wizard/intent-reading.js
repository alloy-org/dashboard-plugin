...
//   both kinds of source, and capped so the page reads as a sample rather than a dump. Completed tasks come first,
//   then the recent tasks the prompt lists beside them, so a domain with little completed work still shows its tasks.
...
export function readItemsFromEvidence(evidence) {
  ...
  const taskReferences = (evidence?.work?.references ?? []).concat(evidence?.work?.supplementalReferences ?? []);
