...
//   archiving it and letting the wizard create a new one, which a bare version number does not tell anybody.
function unsupportedSchemaError(metadata, noteHandle) {
  ...
  if (metadata.schemaVersion > GUIDE_SCHEMA_VERSION) {
    return new Error(`Vision Guide ${ noteIdentity } was written by a newer version of the dashboard `
      + `(schema ${ metadata.schemaVersion }, this build reads ${ GUIDE_SCHEMA_VERSION }); update the plugin to open it`);
  }
  return new Error(`Vision Guide ${ noteIdentity } uses the retired schema ${ metadata.schemaVersion } and cannot be `
