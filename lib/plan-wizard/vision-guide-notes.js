...
export async function replaceGuideSection(app, content, noteHandle, section) {
  if (content.length > MAXIMUM_GUIDE_SECTION_CHARACTERS) {
    ...
    const details = { headingLevel, headingText, limit: MAXIMUM_GUIDE_SECTION_CHARACTERS,
      noteUuid: noteHandle?.uuid ?? null, sectionCharacters: content.length };
    console.error("[plan-wizard] Vision Guide section write exceeds the plugin size bound", details);
    throw new Error(`Vision Guide section "${ headingText }" is ${ content.length } characters; the write limit is ${ MAXIMUM_GUIDE_SECTION_CHARACTERS }`);
  }
