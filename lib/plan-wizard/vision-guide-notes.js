...
// app.replaceNoteContent and app.insertNoteContent are both documented to throw above 100k characters, so a
// write past this point is outside the published contract even when a host build happens to accept it.
export const MAXIMUM_GUIDE_SECTION_CHARACTERS = 100000;
...
// ----------------------------------------------------------------------------------------------
// @desc Find an existing guide, optionally deleting retired schema 1 notes when the wizard opens.
...
// @param {object} options - { deleteRetiredGuide? }; only wizard loading enables schema 1 cleanup.
...
//   unreadable guide cannot block every other scope from loading; an unreadable note in this scope reports its source UUID.
export async function findVisionGuide(app, scope, options = {}) {
  ...
  for (const handle of uniqueHandles.values()) {
    ...
    try {
      ...
      if (!inScope) continue;
      metadataError.noteUuid = noteHandle.uuid;
      throw metadataError;
    }
    if (metadata.domainUuid !== scope.domainUuid || metadata.year !== scope.year) continue;
    if (options.deleteRetiredGuide === true && metadata.schemaVersion === 1 && GUIDE_SCHEMA_VERSION === 2) {
      try {
        const deleted = checkedAppResult(await app.deleteNote({ uuid: noteHandle.uuid }));
        if (deleted !== true) throw new Error("The note could not be deleted; retry opening Plan Builder");
      } catch (error) {
        throw Object.assign(new Error(`Could not delete retired Vision Guide "${ noteHandle.name }" (${ noteHandle.uuid }): ${ error.message }`),
          { noteUuid: noteHandle.uuid });
      }
      continue;
    }
    if (metadata.schemaVersion !== GUIDE_SCHEMA_VERSION) throw unsupportedSchemaError(metadata, noteHandle);
...
// ----------------------------------------------------------------------------------------------
// @desc Explain a schema mismatch in terms of what would resolve it, naming the note it was found in.
// @param {object} metadata - Metadata read from the guide.
// @param {object} noteHandle - The note carrying it.
// @returns {Error} Error with a source note UUID and stable code so optional readers can skip incompatible guide data.
// There is deliberately no migration between schema versions: a guide predating the current one is rebuilt by
//   retagging it and letting the wizard create a new one, which a bare version number does not tell anybody.
function unsupportedSchemaError(metadata, noteHandle) {
  const noteIdentity = noteHandle?.name ? `"${ noteHandle.name }" (${ noteHandle.uuid })` : noteHandle?.uuid ?? "(unknown)";
  let message;
  if (metadata.schemaVersion > GUIDE_SCHEMA_VERSION) {
    message = `Vision Guide ${ noteIdentity } was written by a newer version of the dashboard `
      + `(schema ${ metadata.schemaVersion }, this build reads ${ GUIDE_SCHEMA_VERSION }); update the plugin to open it`;
  } else {
    message = `Vision Guide ${ noteIdentity } uses the retired schema ${ metadata.schemaVersion } and cannot be `
      + `read by this build, which uses schema ${ GUIDE_SCHEMA_VERSION }. Delete or retag that note and the wizard will `
      + "build a new guide; nothing migrates the old one.";
  }
  return Object.assign(new Error(message), { code: "VISION_GUIDE_UNSUPPORTED_SCHEMA", noteUuid: noteHandle?.uuid || null });
}
...
// Reject replacements larger than the plugin API's documented 100,000-character content write limit.
