...
// Version 2 interns evidence UUIDs in a per-leaf table, stops storing the fields ActionProspect derives, scopes
// the prospect index leaves to a quarter, and records provenance for every proposal that shaped a project.
...
// Fields ActionProspect recomputes from evidence or from a canonical sibling. They stay on the instance, because
// consumers read them, and are left out of the note, because storing them cost 32% of a leaf that overflowed.
export const DERIVED_PROSPECT_FIELDS = ["primaryNote", "preferredDows", "relatedNotes", "relatedTasks", "substantiation"];
// Which source answered the prompt that proposed a project. Spellings match the labels raceWizardPrompt gives its
// two legs, so a stored record and a race log name the same thing.
export const PROMPT_SOURCES = ["agent-pro", "direct-provider"];
// Whether a proposal became the stored project or was folded into one. A contributor keeps its summary and its
// source and gives up its evidence, which is already carried by the project that absorbed it.
export const PROVENANCE_ROLES = ["originator", "mergedContributor"];
...
// ----------------------------------------------------------------------------------------------
// @desc Validate the record of one proposal that shaped a stored project: when it arrived, what set it going,
//   which provider answered, and whether it became the project or was merged into it.
// @param {object} entry - Proposed provenance record.
// @returns {object} Normalized entry, retaining extension fields.
// Unknown prompt sources and trigger actions are retained rather than rejected: provenance is an audit trail, and
//   refusing to store a source this build does not recognize would lose the very record that explains the run.
export function assertProvenanceEntry(entry) {
  requireRecord(entry);
  const normalized = { ...entry, contributedAt: normalizedTimestamp(entry.contributedAt),
    role: entry.role ?? "originator", summary: requiredText("summary", entry.summary),
    triggerAction: requiredText("triggerAction", entry.triggerAction) };
  if (!PROVENANCE_ROLES.includes(normalized.role)) throw new Error(`role must be one of ${ PROVENANCE_ROLES.join(", ") }`);
  normalized.aiModel = normalized.aiModel ?? null;
  if (normalized.aiModel !== null) normalized.aiModel = requiredText("aiModel", normalized.aiModel);
  normalized.promptSource = normalized.promptSource ?? null;
  if (normalized.promptSource !== null) normalized.promptSource = requiredText("promptSource", normalized.promptSource);
  return normalized;
}

// ----------------------------------------------------------------------------------------------
// @desc Identify a project the user has marked Complete, so agenda and task suggestions stop drawing from it while
//   the projects page and the plan note still show it as finished work rather than forgetting it.
// @param {object} prospect - ActionProspect record or draft.
// @returns {boolean} True when the project carries a completion timestamp.
export function isCompletedActionProspect(prospect) {
  return Boolean(prospect.completedAt);
}
