...
//   configured. Completed tasks are required here, so done items are explicitly included.
// @param {object} app - Host-compatible Amplenote API.
...
async function domainTasks(app, domainUuid) {
...
//   evidence for regenerating those same intents.
// @param {object} app - Host-compatible Amplenote API.
// @returns {Promise<Set<string>>} Note UUIDs to exclude from every evidence source.
async function planningNoteUuids(app) {
