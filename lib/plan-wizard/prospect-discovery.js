...
export async function discoverActionProspects(app, evidence, scope, { promptRunner = raceWizardPrompt } = {}) {
  ...
  try {
    // Or maybe? `response = await promptRunner(app, prompt, { jsonResponse: true, timeoutSeconds: MAX_LLM_PROCESSING_SECONDS });`
...
function prospectRecordFromEntry(entry, { citedTaskUuids, evidence, linkedGoalUuids, scope, userCategoryEm }) {
  ...
  const record = { approvalStatusEm: "awaitingJudgement", capturedAt: evidence.coverage.collectedAt,
    ...
    focusMonths, linkedGoalUuids, primaryNoteUuid: null, priorityEm: null, quarterKey: scope.quarterKey,
