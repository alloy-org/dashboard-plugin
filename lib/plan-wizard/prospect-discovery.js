...
export function prospectPromptFromEvidence(evidence, scope) {
  ...
  const sections = [
    ...
    "substantiations contains at least one sentence stating what completing this project would resolve and why the evidence supports it. "
...
function prospectRecordFromEntry(entry, { citedTaskUuids, evidence, linkedGoalUuids, provenanceContext, scope,
  ...
  const substantiations = suppliedReasons.map(reason => String(reason ?? "").replace(/\s+/g, " ").trim())
    .filter(Boolean);
