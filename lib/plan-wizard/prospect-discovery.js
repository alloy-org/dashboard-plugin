...
import { logPromptOutcome, logPromptSubmission,
  prospectEvidenceDiagnostics, wizardLlmOptions } from "plan-wizard/wizard-prompt-diagnostics";
import { raceWizardPrompt } from "plan-wizard/wizard-prompt-runner";
import { logIfEnabled } from "util/log";

const DISCOVERY_LOG_LABEL = "[prospect-discovery]";
// Recorded when a caller did not say what set the pass going. Naming the pass rather than leaving the field empty
// keeps an audit trail readable: every stored proposal says at least which kind of pass produced it.
const DEFAULT_DISCOVERY_TRIGGER = "projectDiscoveryPass";
...
// its full reasoning is kept in substantiations, which is where the page shows it anyway.
export const MAXIMUM_SUMMARY_LENGTH = 80;
// A substantiation is read in place under the project name, where roughly two lines of the page's width fit before
// the reasoning stops being skimmable. Asked for two lines and given a paragraph, a provider is taken at the
// sentence boundary nearest this, so the kept text still ends where a thought does.
export const MAXIMUM_SUBSTANTIATION_LENGTH = 220;
const MAX_LLM_PROCESSING_SECONDS = 60;
// A professional candidate is worth proposing only when it would resolve a meaningful share of what is on the list.
export const MINIMUM_RESOLVED_TASKS = MINIMUM_TASKS_PER_PROSPECT;
// A personal candidate faces no such bar. Personal work largely does not live in the task list the way
// professional work does — a quarter spent on a relationship, a fitness aim, or a hobby leaves few tasks behind
// and often none at all — so requiring two cited tasks of it filtered out every personal proposal rather than
// only the weak ones. What it must still do is advance a personal intent the user chose, which is the rule that
// keeps the category honest now that evidence no longer can.
export const MINIMUM_RESOLVED_TASKS_BY_CATEGORY = { personal: 0, work: MINIMUM_RESOLVED_TASKS };
...
// ----------------------------------------------------------------------------------------------
// @desc Shorten a substantiation to the two lines the page gives it, preferring to end at a sentence boundary so the
//   kept text reads as a finished thought rather than a clause cut mid-way. A paragraph whose first sentence already
//   overruns the limit falls back to the word boundary, which is the same treatment a long summary gets.
// @param {string} substantiation - Reasoning text as the provider gave it.
// @returns {string} Whitespace-collapsed reasoning of at most MAXIMUM_SUBSTANTIATION_LENGTH characters.
export function shortenedSubstantiation(substantiation) {
  const collapsed = String(substantiation ?? "").replace(/\s+/g, " ").trim();
  if (collapsed.length <= MAXIMUM_SUBSTANTIATION_LENGTH) return collapsed;
  const clipped = collapsed.slice(0, MAXIMUM_SUBSTANTIATION_LENGTH);
  const lastSentenceEnd = Math.max(clipped.lastIndexOf(". "), clipped.lastIndexOf("! "), clipped.lastIndexOf("? "));
  if (lastSentenceEnd > 0) return clipped.slice(0, lastSentenceEnd + 1);
  const lastSpaceIndex = clipped.lastIndexOf(" ");
  const trimmed = lastSpaceIndex > 0 ? clipped.slice(0, lastSpaceIndex) : clipped;
  return `${ trimmed.replace(/[\s,;:.-]+$/, "") }…`;
}
...
export function prospectPromptFromEvidence(evidence, scope) {
  ...
  const sections = [
    ...
    `A professional candidate qualifies only when at least ${ MINIMUM_RESOLVED_TASKS } of the tasks above concern `
      + "it; cite those task identities exactly as they appear in brackets. A personal candidate is held to no such "
      + "bar: personal intents leave little behind in a task list, so propose the projects that would carry a "
      + "personal intent forward whether or not tasks above concern them, and cite tasks only where they genuinely "
      + "do. Never invent a task identity to meet a bar. Tie every candidate to the identities of the intents it "
    ...
    `Aim for at least ${ MINIMUM_TOTAL_PROSPECTS } projects across both categories when the evidence supports that `
      ...
      + "Propose a professional project only when it meets the two-task evidence requirement, and a personal "
      + "project only when it genuinely serves one of the personal intents above, rather than padding either list.",
    'Respond with strict JSON only: { "prospects": [{ "focusMonths": [string], "linkedGoalUuids": [string], '
      + '"resolvedTaskUuids": [string], "substantiations": [string], "summary": string, "userCategoryEm": '
      + '"work" | "personal" }] }',
    ...
    `substantiations contains one entry of at most ${ MAXIMUM_SUBSTANTIATION_LENGTH } characters — roughly two lines — `
      + "stating what completing this project would resolve and why the evidence supports it. Say it in one or two "
      + "sentences rather than a paragraph; the page shows this text under the project name and a longer answer is "
...
//   Allow MAX_LLM_PROCESSING_SECONDS seconds for the direct provider request to reason over the task evidence.
// @param {object} app - Host-compatible Amplenote API, used for the provider call.
// @param {object} evidence - Bundle from collectProspectEvidence.
// @param {object} scope - Resolved plan scope.
// @param {object} [options] - { promptRunner } to substitute a deterministic provider for the racing runner in
//   tests, and { triggerAction } naming what set this pass going, which is recorded on every proposal.
// @returns {Promise<object>} { coverage, failureReason, prospects }.
export async function discoverActionProspects(app, evidence, scope,
  { promptRunner = raceWizardPrompt, triggerAction = DEFAULT_DISCOVERY_TRIGGER } = {}) {
  ...
  const sizeDiagnostics = logPromptSubmission(DISCOVERY_LOG_LABEL, { model: llmOptions.aiModel, prompt,
    submission: prospectEvidenceDiagnostics(evidence), timeoutSeconds: WIZARD_LLM_TIMEOUT_SECONDS });
  ...
  try {
    ...
    logIfEnabled(`${ DISCOVERY_LOG_LABEL } provider call failed`, failureReason);
  }
  logPromptOutcome(DISCOVERY_LOG_LABEL, { durationMs: performance.now() - providerStart, failureReason,
    sizeDiagnostics, timeoutSeconds: WIZARD_LLM_TIMEOUT_SECONDS });
  const provenanceContext = { aiModel: llmOptions.aiModel ?? null, contributedAt: evidence.coverage.collectedAt,
    promptSource: response?.wizardPromptSource ?? null, triggerAction };
  const prospects = prospectsFromResponse(response?.prospects, { evidence, provenanceContext, scope });
...
//   tying itself to no chosen intent, or repeating a summary already accepted in this pass, is dropped, as is a
//   professional candidate citing fewer than MINIMUM_RESOLVED_TASKS known tasks. A personal candidate faces no
//   task-count bar per MINIMUM_RESOLVED_TASKS_BY_CATEGORY; citations it does make are still checked against the
//   evidence, so an invented task UUID is dropped from its provenance rather than stored.
// @param {*} entries - Candidate prospects from the response; any non-array value yields none.
// @param {object} params - { evidence, provenanceContext, scope }.
// @returns {Array<object>} Validated prospect records, at most PROSPECTS_PER_CATEGORY per category.
function prospectsFromResponse(entries, { evidence, provenanceContext, scope }) {
  ...
  for (const entry of entries) {
    ...
    if (citedTaskUuids.length < MINIMUM_RESOLVED_TASKS_BY_CATEGORY[userCategoryEm]) {
    ...
    const record = prospectRecordFromEntry(entry, { citedTaskUuids, evidence, linkedGoalUuids, provenanceContext,
      scope, userCategoryEm });
    if (!record) continue;
...
// @param {object} params - { citedTaskUuids, evidence, linkedGoalUuids, provenanceContext, scope, userCategoryEm }.
// @returns {object|null} A JSON prospect record, or null when the model rejects it.
function prospectRecordFromEntry(entry, { citedTaskUuids, evidence, linkedGoalUuids, provenanceContext, scope,
  userCategoryEm }) {
  ...
  const suppliedReasons = Array.isArray(entry?.substantiations) ? entry.substantiations : [entry?.substantiation];
  // A provider that splits its reasoning across several entries would otherwise stack them into the paragraph the
  // two-line limit exists to prevent, so they are joined first and the limit applied to the whole.
  const joinedReasons = suppliedReasons.map(reason => String(reason ?? "").trim()).filter(Boolean).join(" ");
  const shortenedReason = shortenedSubstantiation(joinedReasons);
  const substantiations = shortenedReason ? [shortenedReason] : [];
  const record = { approvalStatusEm: "awaitingJudgement", capturedAt: evidence.coverage.collectedAt,
    evidence: citedTaskUuids.map(taskUuid => ({ noteUuid: noteUuidForTask(evidence, taskUuid), taskUuid })),
    focusMonths, linkedGoalUuids, primaryNoteUuid: null, priorityEm: null,
    provenance: [{ ...provenanceContext, role: "originator", summary }], quarterKey: scope.quarterKey,
    refreshedProspectAt: evidence.coverage.collectedAt, refreshedTasksAt: evidence.coverage.collectedAt,
    substantiations, summary, userCategoryEm, uuid: prospectIdentityFromSummary(scope, userCategoryEm, summary) };
  try {
    const prospect = new ActionProspect(record, scope);
    return { ...prospect };
  } catch (error) {
    logIfEnabled("[prospect-discovery] discarded an invalid candidate", error?.message, record);
    return null;
  }
}
