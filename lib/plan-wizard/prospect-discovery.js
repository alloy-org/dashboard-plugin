...
import { WIZARD_LLM_MODEL, WIZARD_LLM_TIMEOUT_SECONDS } from "plan-wizard/plan-models";
import { MINIMUM_TASKS_PER_PROSPECT } from "plan-wizard/prospect-evidence";
import { logPromptOutcome, logPromptSubmission,
  prospectEvidenceDiagnostics } from "plan-wizard/wizard-prompt-diagnostics";
...
export async function discoverActionProspects(app, evidence, scope, { promptRunner = llmPromptWithPluginFallback } = {}) {
  ...
  const sizeDiagnostics = logPromptSubmission(DISCOVERY_LOG_LABEL, { model: WIZARD_LLM_MODEL, prompt,
    submission: prospectEvidenceDiagnostics(evidence), timeoutSeconds: WIZARD_LLM_TIMEOUT_SECONDS });
  const providerStart = performance.now();
  let response = null;
  let failureReason = null;
  try {
    response = await promptRunner(app, prompt, { aiModel: WIZARD_LLM_MODEL, jsonResponse: true,
