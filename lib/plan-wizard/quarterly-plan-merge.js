...
// wizard captures a single name and repeating it two lines later would say the same thing twice. The intents that
// page one captured are written as bullets under Quarter Theme, above Success Looks Like, so that every answer the
// wizard asks for is somewhere in the note a user reads and the plan services prompt from.
//
...
import { BUILDER_MARKER, COMPLETE_MARKER, KEEP_WARM_MARKER, bulletValueFromBody, contentWithSectionBody, isBuilderMarkedText,
...
//   forward from the block it is replacing so a rewrite cannot erase the user's own Outcome wording. A completed
//   project carries the Complete marker and opens with a Status bullet naming the day it was finished.
...
function projectBlockMarkdown(project, previousBody) {
  const liveMarker = project.isKeptWarm ? KEEP_WARM_MARKER : BUILDER_MARKER;
  const marker = project.completedOn ? COMPLETE_MARKER : liveMarker;
  ...
  if (project.completedOn) bullets.unshift(`- Status: Completed ${ project.completedOn }`);
...
//   template placeholders once real projects exist to take their place. Completed projects follow the live ones,
//   so the section reads as the work still ahead before the work already delivered.
// @param {string} body - Existing Projects section body.
// @param {Array<object>} projects - Published live project records, already in note order.
// @param {Array<object>} completedProjects - Published project records marked Complete, already in note order.
// @returns {string} Updated body.
function projectsSectionBody(body, projects, completedProjects = []) {
  ...
  const publishedProjects = [...projects, ...completedProjects];
  const retainedBlocks = blocks.filter(block => !isBuilderMarkedText(block.headingText)
    && !(publishedProjects.length && isEmptyProjectPlaceholder(block)));
  const retainedMarkdown = retainedBlocks.map(block => `## ${ block.headingText }\n${ block.body.replace(/^\n+|\s+$/g, "") }`);
  const builderMarkdown = publishedProjects.map(project => projectBlockMarkdown(project,
    previousBodyBySummary.get(project.summary) ?? ""));
...
// ----------------------------------------------------------------------------------------------
// @desc Record the quarter's intents and the day's sufficiency bar beneath the quarter's outcomes, leaving the
//   theme sentence itself to the user. The quarter's name already leads the note as its own heading, and the
//   wizard captures only that one string, so writing it here as well would state the same words twice.
// @param {string} body - Existing Quarter Theme body, including its nested Success Looks Like section.
// @param {object} publication - Publication carrying intents and dailySufficiencyText.
// @returns {string} Updated body.
// The intents go above Success Looks Like rather than into it: they are what the quarter is for, while that
// section lists the outcomes that would show it happened, and a reader scanning the note should meet the
// former first.
function quarterThemeBody(body, publication) {
  const successRange = planSectionRange(body, SUCCESS_HEADING);
  if (!successRange) return intentLinesAppended(body.replace(/^\n+|\s+$/g, ""), publication.intents);
  const themeText = body.slice(0, successRange.start).replace(/^\n+|\s+$/g, "");
  const intentText = intentLinesAppended(themeText, publication.intents);
  const successBody = successLooksLikeBody(body.slice(successRange.bodyStart, successRange.end), publication);
  return `${ intentText }\n\n## ${ SUCCESS_HEADING }\n${ successBody }`;
}

// ----------------------------------------------------------------------------------------------
// @desc Restate the intents the user named on the wizard's first page as marked bullets under the theme, so the
//   sentence they typed into the note stays the section's own opening line.
// @param {string} themeText - Theme body above Success Looks Like, with builder bullets still to be replaced.
// @param {Array<object>} intents - Published intent records carrying text and userCategoryEm.
// @returns {string} Updated theme body.
// Each bullet names its category, since one quarter holds both professional and personal intents and the note
// has no other place that distinguishes them.
function intentLinesAppended(themeText, intents) {
  const keptLines = themeText.split("\n").filter(line => !isBuilderMarkedText(line));
  const retainedLines = trimmedTrailingBlankLines(keptLines);
  const builderLines = (intents ?? []).map(intent =>
    `- ${ intentCategoryLabel(intent.userCategoryEm) }: ${ intent.text } ${ BUILDER_MARKER }`);
  if (!builderLines.length) return retainedLines.join("\n").replace(/\n{3,}/g, "\n\n");
  const lines = [...retainedLines, "", ...builderLines].map(line => line.replace(/\s+$/, ""));
  return lines.join("\n").replace(/\n{3,}/g, "\n\n");
}

// ----------------------------------------------------------------------------------------------
// @desc Name an intent's category the way the plan note labels it.
// @param {string} userCategoryEm - work or personal.
// @returns {string} Label for the bullet.
function intentCategoryLabel(userCategoryEm) {
  return userCategoryEm === "personal" ? "Personal intent" : "Professional intent";
}
...
//   accumulating one per run. Remove all leading builder title headings to repair earlier duplicates, preserving
//   prose between them. Stop at the first user heading and insert the current title above the retained content.
...
export function contentWithPlanTitle(content, quarterNameText) {
  const builderTitles = [];
  for (const heading of planHeadingRanges(content)) {
    if (heading.level !== 1 || !isBuilderMarkedText(heading.text)) break;
    builderTitles.push(heading);
  }
  let titledContent = content;
  for (const heading of builderTitles.reverse()) {
    titledContent = `${ titledContent.slice(0, heading.start) }${ titledContent.slice(heading.bodyStart) }`;
  }
...
export function mergedQuarterlyPlanContent(content, publication) {
  const sectionBodies = [[QUARTER_THEME_HEADING, body => quarterThemeBody(body, publication)],
    [PROJECTS_HEADING, body => projectsSectionBody(body, publication.projects, publication.completedProjects)],
