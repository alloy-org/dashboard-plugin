...
import { PLAN_MODEL_MARKER, PROSPECT_APPROVALS, PROSPECT_PACES, PROSPECT_PRIORITIES, assertCalendarDate,
  assertMonthLabels, assertProvenanceEntry, assertUserCategory, assertUuidList, assertWeekdays, copyJsonValue,
  normalizedTimestamp, requireRecord, requiredText } from "plan-wizard/plan-models";
...
// @property {string|null} completedAt - When the user marked the project Complete; null while it is still live work.
// @property {string|null} deadlineOn - YYYY-MM-DD landing date when paceEm is deadlineSprint; null otherwise.
...
// @property {string|null} paceEm - How vigorously this project should be preferred on a given day.
...
// @property {Array<object>} provenance - One entry per proposal that shaped this project: the originator, plus a mergedContributor for each near-duplicate folded into it
...
export default class ActionProspect {
  ...
  constructor(record, scope) {
    ...
    prospect.paceEm = prospect.paceEm ?? null;
    if (prospect.paceEm !== null && !PROSPECT_PACES.includes(prospect.paceEm)) {
      throw new Error(`paceEm must be null or one of ${ PROSPECT_PACES.join(", ") }`);
    }
    ...
    prospect.deadlineOn = prospect.paceEm === "deadlineSprint" && prospect.deadlineOn
      ? assertCalendarDate(prospect.deadlineOn) : null;
    ...
    prospect.provenance = prospect.provenance ?? [];
    if (!Array.isArray(prospect.provenance)) throw new Error("provenance must be an array");
    prospect.provenance = prospect.provenance.map(assertProvenanceEntry);
    ...
    prospect.completedAt = prospect.completedAt ? normalizedTimestamp(prospect.completedAt) : null;
