...
import { readItemsFromEvidence, themeJudgementsWithChange, themeKey } from "plan-wizard/intent-reading";
...
//   called with { phase: "evidence", readItems } once the notes have been read and before the provider is asked, so
//   the reading page can list what was read while inference is still running.
...
export async function refreshPlanIntentPossibilities(app, options = {}) {
  ...
  if (options.onProgress) options.onProgress({ phase: "evidence", readItems });
  const inference = await inferIntentPossibilities(app, evidence, scope, { promptRunner: options.promptRunner, themeJudgements });
  const generatedAt = evidence.coverage.collectedAt;
  const keptThemes = inference.themes.filter(theme => themeJudgements[themeKey(theme.label)]?.judgement !== "dismissed");
