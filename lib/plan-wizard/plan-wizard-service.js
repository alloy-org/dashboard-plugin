...
import { readItemsFromEvidence, themeJudgementsWithChange, undismissedThemes } from "plan-wizard/intent-reading";
import { USER_CATEGORIES, assertUserCategory, copyJsonValue, resolvePlanScope } from "plan-wizard/plan-models";
import { consolidateActionProspects } from "plan-wizard/prospect-consolidation";
...
import { projectSourcesFromEvidence, withNamedSourceNotes } from "plan-wizard/project-sources";
...
import { clearProspectPlacementSections, readVisionGuide, updateVisionGuideSection,
  writeProspectPlacementSections } from "plan-wizard/vision-guide-repository";
...
function planningContextFromGuide(guide, scope) {
  ...
  const intentReading = { readItems: guide?.work.reading?.readItems ?? [], themeJudgements: guide?.work.themeJudgements ?? {},
    themes: guide?.work.reading?.themes ?? [] };
  return { dailySufficiency, generatedAt, goalRecords, goals, intentReading, noteUuid: guide?.noteUuid ?? null,
    possibilities, prospectRecords: quarterProspects, prospects: liveProspects, quarterName, scope };
}
...
// First read path for the wizard, the agenda, and the quarterly plan note publisher.
// The wizard passes deleteRetiredGuide: true to delete schema 1 storage and start with an empty context.
export async function readPlanGoals(app, options = {}) {
  const scope = resolvePlanScope(options);
  const guide = await readVisionGuide(app, scope, { deleteRetiredGuide: options.deleteRetiredGuide });
...
// @param {object} options - Scope, plus { promptRunner, referenceDate } for deterministic tests, and { onProgress },
//   called with { phase: "evidence", readItems, themes } once the notes have been read and before the provider is
//   asked, so the reading page can list what was read while inference is still running, and again with
//   { phase: "themes" } each time a streamed answer completes another undismissed theme.
...
// The themes the model found are stored with the professional suggestions, beside the judgements the user has
//   passed on earlier themes; a theme the user dismissed is dropped even when the model reports it anyway.
export async function refreshPlanIntentPossibilities(app, options = {}) {
  const scope = resolvePlanScope(options);
  const priorContext = await readPlanGoals(app, scope);
  const themeJudgements = priorContext.intentReading.themeJudgements;
  ...
  logIfEnabled("[plan-wizard] intent reading listed", { noteContextCount: evidence.work.noteContext.length,
    noteItemCount: readItems.filter(item => item.kind === "note").length, readItemCount: readItems.length,
    referenceCount: evidence.work.references.length, supplementalReferenceCount: evidence.work.supplementalReferences.length,
    taskItemCount: readItems.filter(item => item.kind === "task").length });
  if (options.onProgress) options.onProgress({ phase: "evidence", readItems, themes: [] });
  const handlePartialThemes = themes => options.onProgress({ phase: "themes", readItems,
    themes: undismissedThemes(themes, themeJudgements) });
  const inference = await inferIntentPossibilities(app, evidence, scope, { onPartialThemes: options.onProgress ? handlePartialThemes : null,
    promptRunner: options.promptRunner, themeJudgements });
  const generatedAt = evidence.coverage.collectedAt;
  const keptThemes = undismissedThemes(inference.themes, themeJudgements);
  const reading = { readItems, themes: keptThemes };
  logIfEnabled("[plan-wizard] intent reading ready", { dismissedThemeCount: inference.themes.length - keptThemes.length,
    personalPossibilityCount: inference.personal.length, readItemCount: readItems.length, themeCount: keptThemes.length,
    workPossibilityCount: inference.work.length, workReadingSaved: inference.work.length > 0 });
  let planningContext = priorContext;
  for (const userCategoryEm of USER_CATEGORIES) {
    const possibilities = inference[userCategoryEm];
    if (!possibilities.length) continue;
    const categoryReading = userCategoryEm === "work" ? reading : undefined;
    planningContext = await savePlanIntentPossibilities(app, { ...scope, generatedAt, possibilities, reading: categoryReading,
      userCategoryEm });
  }
  // A pass with no usable professional suggestions stores nothing, but the page still shows what was read.
  const intentReading = { ...planningContext.intentReading, ...reading };
  return { ...planningContext, failureReason: inference.failureReason, intentReading,
    occupationHypothesis: inference.occupationHypothesis };
}

// ----------------------------------------------------------------------------------------------
// @desc Record the user's pin or dismissal of one theme from the reading page, or clear an earlier one. The
//   judgement is stored beside the professional suggestions and steers every later inference pass for the quarter.
// @param {object} app - Amplenote host app or embed proxy.
// @param {object} options - Scope plus { judgement, label }; judgement is pinned, dismissed, or null to clear.
// @returns {Promise<object>} Verified planning context.
export async function savePlanThemeJudgement(app, options) {
  const scope = resolvePlanScope(options);
  themeJudgementsWithChange({}, options.label, options.judgement ?? null);
  const guide = await updateVisionGuideSection(app, "work", scope, previous => ({ ...previous,
    themeJudgements: themeJudgementsWithChange(previous.themeJudgements, options.label, options.judgement ?? null) }));
  return planningContextFromGuide(guide, scope);
}
...
//   { triggerAction } naming what set this pass going, which every proposal records in its provenance, and
//   { onProgress }, called with { projectSources } once the evidence is collected and before the provider is asked,
//   so the sources page can show what discovery is working from while it waits.
// @returns {Promise<object>} Planning context, extended with { failureReason, projectSources }.
...
export async function refreshPlanActionProspects(app, options = {}) {
  ...
  const projectSources = await withNamedSourceNotes(app, projectSourcesFromEvidence(evidence));
  if (options.onProgress) options.onProgress({ projectSources });
  ...
  if (!discovery.prospects.length) {
    ...
    return { ...planningContext, failureReason: discovery.failureReason, projectSources };
  }
  ...
  return { ...savedContext, failureReason: discovery.failureReason, projectSources };
}

// ----------------------------------------------------------------------------------------------
// @desc Collect the evidence discovery would read for this scope and summarize it for the sources page, without
//   asking the provider for anything. The page uses this when it is opened with no discovery pass run in this
//   session, so it can still show the notes and tasks the projects are drawn from.
// @param {object} app - Amplenote host app or embed proxy.
// @param {object} options - Scope, plus { referenceDate } for deterministic tests.
// @returns {Promise<object>} Summary from projectSourcesFromEvidence.
export async function readPlanProjectSources(app, options = {}) {
  const scope = resolvePlanScope(options);
  const planningContext = await readPlanGoals(app, scope);
  const evidence = await collectProspectEvidence(app, scope, planningContext,
    { referenceDate: options.referenceDate ?? new Date() });
...
// ----------------------------------------------------------------------------------------------
// @desc Combine the stored proposals that describe one undertaking into the single project they were all
//   reaching for, and drop the restatements that were folded in.
// @param {object} app - Amplenote host app or embed proxy.
// @param {object} options - Scope, plus { promptRunner } for deterministic tests.
// @returns {Promise<object>} Planning context, extended with { failureReason } when nothing could be combined.
export async function consolidatePlanActionProspects(app, options = {}) {
  const scope = resolvePlanScope(options);
  const planningContext = await readPlanGoals(app, scope);
  const consolidation = await consolidateActionProspects(app, planningContext.prospectRecords, scope,
    { promptRunner: options.promptRunner });
  if (!consolidation.mergedProspects.length) {
    return { ...planningContext, failureReason: consolidation.failureReason };
  }
  // Placements are cleared before the records go, while the leaf still says which category each absorbed project
  // belonged to and its month tree can still be found by its identity.
  const absorbedProspects = planningContext.prospectRecords.filter(prospect => consolidation.absorbedUuids.includes(prospect.uuid));
  for (const absorbed of absorbedProspects) await clearProspectPlacementSections(app, absorbed, scope);
  const savedContext = await savePlanProspects(app, { ...scope, prospects: consolidation.mergedProspects,
    removeProspectUuids: consolidation.absorbedUuids });
  logIfEnabled("[plan-wizard] project consolidation complete", { absorbedCount: consolidation.absorbedUuids.length,
    mergedCount: consolidation.mergedProspects.length });
  return { ...savedContext, failureReason: consolidation.failureReason };
}
...
//   An optional { reading } records the notes and themes that produced them, replacing the earlier reading.
// @returns {Promise<object>} Verified planning context.
...
export async function savePlanIntentPossibilities(app, options) {
  ...
  if (options.reading) incoming.reading = copyJsonValue(options.reading);
...
//   Pass updatePlacement: false when only paceEm, preferredWeekdays, or deadlineOn changed, and
//   removeProspectUuids to drop stored records outright, as consolidation does with the restatements it absorbs.
...
export async function savePlanProspects(app, options) {
  ...
  const removedUuids = new Set(options.removeProspectUuids ?? []);
  ...
  for (const userCategoryEm of USER_CATEGORIES) {
    ...
    // A removal names a project by identity alone, so which category holds it is not known here. Both leaves run
    // when anything is being removed; a leaf holding none of the named identities merges to itself and is not
    // written, which is the same no-op an unchanged save already performs.
    if (!categoryProspects.length && !removedUuids.size) continue;
    const kind = userCategoryEm === "work" ? "workProspects" : "personalProspects";
    guide = await updateVisionGuideSection(app, kind, scope, previous => {
      const mergedProspects = mergeActionProspects(previous.prospects, categoryProspects, scope, userCategoryEm);
      const retainedProspects = mergedProspects.filter(prospect => !removedUuids.has(prospect.uuid));
      return { ...previous, prospects: retainedProspects };
    });
  }
