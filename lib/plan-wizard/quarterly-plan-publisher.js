...
function unpublishedProjectSummaries(savedContent, publication) {
  ...
  const expectedProjects = [...publication.projects, ...(publication.completedProjects ?? [])];
  const expectedSummaries = expectedProjects.map(project => project.summary);
  const missingSummaries = expectedSummaries.filter(summary => !publishedSummaries.has(summary));
