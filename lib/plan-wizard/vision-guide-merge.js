...
import { ActionProspect, GoalSet, IntentPossibility, normalizedTimestamp, planningRecordUuid,
  requireRecord } from "plan-wizard/plan-models";
...
export function mergeActionProspects(existing, incoming, scope, userCategoryEm) {
  ...
  for (const input of incoming) {
    ...
    if (previous && candidate.capturedAt <= previous.capturedAt) continue;
    const isUnjudgedProposal = candidate.approvalStatus === "awaitingJudgement";
    if (previous && previous.approvalStatus !== "awaitingJudgement" && isUnjudgedProposal) continue;
