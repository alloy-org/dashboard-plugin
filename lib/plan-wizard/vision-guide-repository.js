...
import { guideHeadingRanges, guideSectionRange, intentRootMarkdown, intentSectionDefinition, jsonPayloadMarkdown,
  parseJsonPayload, prospectRootMarkdown, replaceJsonPayload } from "plan-wizard/vision-guide-markdown";
...
export function validatedSectionPayload(kind, payload, scope) {
  ...
  if (kind === "personalProspects" || kind === "workProspects") {
    ...
    result.prospects = result.prospects.map(prospect => new ActionProspect(prospect, { quarterKey: prospect.quarterKey }));
