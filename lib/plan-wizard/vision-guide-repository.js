...
import { validatedIntentReading } from "plan-wizard/intent-reading";
...
import { PROSPECT_CATEGORY_LABELS, PROSPECT_TASK_BUCKET_LABELS, appendProspectMonthSubtree, guideHeadingRanges,
  guideSectionRange, intentRootMarkdown, intentSectionDefinition, jsonPayloadMarkdown, parseJsonPayload,
  prospectFocusMonthLabel, prospectIndexHeadingText, prospectMonthAnchorHeading, prospectMonthHeadingText,
  prospectMonthSubtreeMarkdown, prospectPlacementBucketLabel, prospectRootMarkdown, prospectTaskBucketHeadingText,
  replaceJsonPayload } from "plan-wizard/vision-guide-markdown";
...
async function ensureIntentSection(app, guide, kind, scope) {
  ...
  let target = parent;
  if (!parent) {
    const template = definition.root ? intentRootMarkdown(scope.year) : prospectRootMarkdown(scope.year);
...
// ----------------------------------------------------------------------------------------------
// @desc Read goal/intention leaves, optionally clearing retired guides, and retain source UUIDs on validation errors.
// @param {object} app - Host app.
// @param {object} scope - Resolved planning scope.
// @returns {Promise<object|null>} Guide with normalized goals, personal, and work envelopes, or null.
// @param {object} options - { deleteRetiredGuide? }; defaults to a read-only lookup.
// Recommendation consumers can safely consult a missing datastore.
export async function readVisionGuide(app, scope, options = {}) {
  const guide = await findVisionGuide(app, scope, options);
  ...
  try {
    ...
    error.noteUuid = guide.noteHandle.uuid;
    throw error;
  }
  return snapshot;
}
...
function sectionPayload(content, kind, scope) {
  ...
  return validatedSectionPayload(kind, hydratedLeafPayload(kind, payload), scope);
}
...
// ----------------------------------------------------------------------------------------------
// @desc Empty every placement bucket a project holds, for a project that is being removed from its category leaf
//   rather than moved between buckets.
// @param {object} app - Host app or embed proxy.
// @param {object} prospect - Project whose placement is being cleared.
// @param {object} scope - Resolved planning scope.
// @returns {Promise<object>} Verified guide snapshot.
// A project absorbed into another leaves its month tree behind, and a tree still naming it in Awaiting approval
//   would put it back in front of the user after the record it refers to had gone. The headings stay, holding
//   empty buckets, because removing them would mean rewriting the category root this placement scheme exists to
//   avoid rewriting.
export async function clearProspectPlacementSections(app, prospect, scope) {
  return enqueueVisionGuideWrite(app, scope, async () => {
    let guide = await findVisionGuide(app, scope);
    if (!guide?.metadata) throw new Error("Vision Guide is missing; nothing to clear");
    ...
    for (const bucketLabel of PROSPECT_TASK_BUCKET_LABELS) {
      ...
      if (!guideSectionRange(guide.content, headingText)) continue;
      guide = await persistProspectBucketLeaf(app, bucketLabel, null, guide, kind, prospect);
    }
...
export function validatedSectionPayload(kind, payload, scope) {
  ...
  if (kind === "goals") {
    ...
    if (result.reading !== undefined || result.themeJudgements !== undefined) {
      Object.assign(result, validatedIntentReading(result.reading, result.themeJudgements));
    }
  }
  return result;
}
...
async function writeIntentSection(app, kind, scope, transform) {
  ...
  if (!samePlanningData(previous, updated)) {
    ...
    assertSupportedSectionBody(body);
    const replacement = replaceJsonPayload(body, storedLeafPayload(kind, updated));
...
// @param {object} scope - Resolved planning scope.
// @returns {Promise<void>}
// The tree is appended to the last project already placed in this category rather than written into the category
//   root, so adding one project costs one project's markdown instead of the whole category's. Only a category with
//   no project placed yet falls back to the root, where the body being rewritten is the ideas leaf alone.
async function ensureProspectMonthSection(app, destinationLabel, guide, payload, prospect, scope) {
  ...
  const indexHeadingTexts = quarterlyIndexHeadingTexts(categoryLabel, scope.year);
  ...
  if (guideSectionRange(guide.content, monthHeading)) return;
  ...
  const anchor = prospectMonthAnchorHeading(indexHeadingTexts, parentBody)
    ?? guideSectionRange(parentBody, prospectIndexHeadingText(categoryLabel, scope));
  const anchorBody = anchor ? parentBody.slice(anchor.bodyStart, anchor.end) : parentBody;
  const anchorHeading = anchor ? { level: anchor.level, text: anchor.text } : { level: parent.level, text: parent.text };
  assertSupportedSectionBody(anchorBody);
  await replaceGuideSection(app, appendProspectMonthSubtree(anchorBody, subtree), guide.noteHandle, { heading: anchorHeading });
}

// ----------------------------------------------------------------------------------------------
// @desc Name every quarterly index leaf one category holds in a year, so placing a month tree can avoid anchoring
//   to any of them.
// @param {string} categoryLabel - Professional or Personal.
// @param {number} year - Planning year the guide covers.
// @returns {Array<string>} The four index heading texts.
function quarterlyIndexHeadingTexts(categoryLabel, year) {
  const quarters = [1, 2, 3, 4];
  return quarters.map(quarter => prospectIndexHeadingText(categoryLabel, { quarter, year }));
}

// ----------------------------------------------------------------------------------------------
// @desc Validate a placement bucket's payload, which names the projects sitting in that bucket by identity rather
//   than repeating their records.
// @param {object} payload - Proposed bucket payload.
// @returns {object} Normalized payload.
// A bucket used to hold whole prospect records, so every project was stored twice and a rename had to be written
//   to both copies. The category leaf is the one place a project's fields live; a bucket only says where it sits.
function validatedBucketPayload(payload) {
  requireRecord(payload);
  const prospectUuids = payload.prospectUuids ?? [];
  if (!Array.isArray(prospectUuids)) throw new Error("Stored bucket prospect identities must be an array");
  const prospectTasks = payload.prospectTasks ?? [];
  if (!Array.isArray(prospectTasks)) throw new Error("Stored prospect tasks must be an array");
  return { ...payload, prospectTasks: prospectTasks.map(prospectTask => new ProspectTask(prospectTask)),
    prospectUuids: prospectUuids.map(uuid => requiredText("prospectUuid", uuid)) };
}
...
async function persistProspectBucketLeaf(app, bucketLabel, destinationLabel, guide, kind, prospect) {
  ...
  const prospectUuids = bucketLabel === destinationLabel ? [prospect.uuid] : [];
  const payload = validatedBucketPayload({ prospectTasks: [], prospectUuids });
...
async function persistProspectPlacementSections(app, prospect, scope) {
  ...
  const kind = prospect.userCategoryEm === "work" ? "workProspects" : "personalProspects";
  const occupiedPayload = validatedBucketPayload({ prospectTasks: [],
    prospectUuids: destinationLabel ? [prospect.uuid] : [] });
  if (!bucketsExist) {
