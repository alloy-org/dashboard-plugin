...
import { MAXIMUM_THEMES, themeGuidanceFromJudgements, themesFromResponse } from "plan-wizard/intent-reading";
...
export function intentPromptFromEvidence(evidence, scope, themeJudgements = {}) {
  ...
  const sections = [
    ...
    'Respond with strict JSON only: { "occupationHypothesis": string, "personal": [{ "confidence": number, "intent": string, '
      + '"substantiation": string }], "themes": [{ "label": string, "taskCount": number }], "work": [ same shape as personal ] }',
  ];
...
// @param {object} [options] - { promptRunner } to substitute a deterministic provider for the racing runner in tests,
//   and { themeJudgements } carrying the user's pinned and dismissed themes into the prompt.
// @returns {Promise<object>} { coverage, failureReason, occupationHypothesis, personal, themes, work }.
export async function inferIntentPossibilities(app, evidence, scope, { promptRunner = raceWizardPrompt,
    themeJudgements = {} } = {}) {
