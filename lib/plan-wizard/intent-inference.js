...
import { MAXIMUM_THEMES, themeGuidanceFromJudgements, themesFromPartialResponse, themesFromResponse } from "plan-wizard/intent-reading";
import { WIZARD_LLM_TIMEOUT_SECONDS } from "plan-wizard/plan-models";
import { pluginSettings } from "plugin-data";
import { intentEvidenceDiagnostics, logPromptOutcome, logPromptSubmission, wizardLlmOptions } from "plan-wizard/wizard-prompt-diagnostics";
import { raceWizardPrompt } from "plan-wizard/wizard-prompt-runner";
import { logIfEnabled } from "util/log";

const INFERENCE_LOG_LABEL = "[intent-inference]";
...
// @param {object} [themeJudgements] - Themes the user pinned or dismissed on the reading page, keyed by themeKey.
// @returns {string} Prompt text.
// Theme guidance sits outside the evidence markers: it is the user's instruction about this request, not note text.
// Themes are requested first so that a streamed answer delivers them while the directions are still being written.
export function intentPromptFromEvidence(evidence, scope, themeJudgements = {}) {
  ...
  const sections = [
    ...
    `Note context:\n${ noteLines.join("\n\n") || "(none)" }`, "EVIDENCE>>>",
    themeGuidanceFromJudgements(themeJudgements),
    `Name up to ${ MAXIMUM_THEMES } recurring themes in the completed and recent work, each one to three words, with the `
      + "number of listed tasks that belong to it, most frequent first.",
    ...
    "Keep each professional intent very high-level and general: around 10 words, never more than 15, so it fits on a single "
      + "row without wrapping. Omit specific project names, metrics, and dates.",
    "Confidence is 1 through 10 and must be low when the supporting evidence is thin.",
    'Respond with strict JSON only, writing its fields in this order: { "themes": [{ "label": string, "taskCount": number }], '
      + '"occupationHypothesis": string, "work": [{ "confidence": number, "intent": string, "substantiation": string }], '
      + '"personal": [ same shape as work ] }',
  ];
  const populatedSections = sections.filter(section => section);
...
//   - {Function} onPartialThemes - Receives the themes complete so far each time a streamed answer adds one. It is
//     not called once the provider has answered, nor at all when the answer arrives in one piece.
//   - {Function} promptRunner - Substitutes a deterministic provider for the racing runner in tests.
//   - {object} themeJudgements - The user's pinned and dismissed themes, carried into the prompt.
// @returns {Promise<object>} { coverage, failureReason, occupationHypothesis, personal, themes, work }.
export async function inferIntentPossibilities(app, evidence, scope, { onPartialThemes = null, promptRunner = raceWizardPrompt,
    themeJudgements = {} } = {}) {
  const prompt = intentPromptFromEvidence(evidence, scope, themeJudgements);
  const uuidPrefix = `${ scope.quarterKey }-${ evidence.coverage.collectedAt }`;
  const llmOptions = wizardLlmOptions(pluginSettings());
  const sizeDiagnostics = logPromptSubmission(INFERENCE_LOG_LABEL, { model: llmOptions.aiModel, prompt,
    submission: intentEvidenceDiagnostics(evidence), timeoutSeconds: WIZARD_LLM_TIMEOUT_SECONDS });
  const listedTaskCount = evidence.work.references.length + evidence.work.supplementalReferences.length;
  const providerStart = performance.now();
  let hasAnswered = false;
  let reportedThemeCount = 0;
  // A losing stream can keep arriving after the race is settled, and its themes must not follow the final ones.
  const handlePartialText = partialText => {
    if (hasAnswered) return;
    const partialThemes = themesFromPartialResponse(partialText, listedTaskCount);
    if (partialThemes.length === reportedThemeCount) return;
    reportedThemeCount = partialThemes.length;
    onPartialThemes(partialThemes);
  };
  let response = null;
  let failureReason = null;
  try {
    response = await promptRunner(app, prompt, { ...llmOptions, onPartialText: onPartialThemes ? handlePartialText : null });
  } catch (error) {
    failureReason = error?.message || "Intent inference request failed";
    logIfEnabled(`${ INFERENCE_LOG_LABEL } provider call failed`, failureReason);
  }
  hasAnswered = true;
  logPromptOutcome(INFERENCE_LOG_LABEL, { durationMs: performance.now() - providerStart, failureReason,
  ...
  const themes = themesFromResponse(response?.themes, listedTaskCount);
  return { coverage: evidence.coverage, failureReason, occupationHypothesis, personal, themes, work };
}
