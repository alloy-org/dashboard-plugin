...
// @returns {Object} Options for llmPromptWithPluginFallback, always carrying jsonResponse and timeoutSeconds,
//   carrying aiModel and apiKey only when a keyed provider offered them, and reasoningEffort only when that
//   provider's fast tier is a reasoning model that must be capped to answer inside the budget.
export function wizardLlmOptions(settings) {
  ...
  if (fastOptions) {
    ...
    if (fastOptions.reasoningEffort) llmOptions.reasoningEffort = fastOptions.reasoningEffort;
  }
  return llmOptions;
}
