  ...
  try {
    ...
    if (reformattedText !== jsonText) {
      try {
        ...
        logIfEnabled("Reformatted text still fails", e)
      }
    }
  }
...
export async function trimNoteContentFromAnswer(app, answer, { replaceToken = null, replaceIndex = null } = {}) {
  ...
  if (replaceIndex || replaceToken) {
    ...
    if (replaceToken) {
      ...
      if (sentenceStart && sentenceStart.trim().length > 1) {
        logIfEnabled(`Replacing sentence start fragment: "${ sentenceStart }"`);
      ...
      if (afterSentence.length) {
        ...
        if (afterSentenceIndex !== -1) {
          logIfEnabled("LLM response seems to have returned content after prompt. Truncating");
  ...
  logIfEnabled(`Answer originally ${ answer.length } length, refined answer length ${ refinedAnswer.length } ("${ refinedAnswer }"). Without repeated lines ${ withoutJunkLines.length } length`);
...
function balancedJsonFromString(string) {
  ...
  if (pendingBalance.length) {
    logIfEnabled("Found", pendingBalance.length, "characters to append to balance", jsonText, ". Adding ", pendingBalance.reverse().join(""));
