...
// The person-filter only lists collaborators seen on a note updated within this window
const SHARER_FILTER_WINDOW_MS = 365 * DAY_MS;
...
// ------------------------------------------------------------------------------------------
// @desc Compact diagnostic summary of how many returned notes each collaborator appears on — for
//   Debug Console logging without dumping every note handle (which can be several KB).
// @param {Array<Object>} notes - Result entries, each with a `collaborators` array of { name }.
// @returns {Array<Object>} Entries of { count, name } sorted by descending count, then name.
// [Cursor Grok 4.5] Task: log collaborator note counts instead of full shared-notes arrays
export function collaboratorNoteCounts(notes) {
  ...
  const summary = [...countByName.entries()].map(([name, count]) => ({ count, name }));
  const sortedSummary = summary.sort((a, b) => b.count - a.count || a.name.localeCompare(b.name));
...
export async function findCollaboratorUpdatedNotes({ app, maxNotes = Infinity,
  ...
  const sharerNames = sharerNamesFromNotes(notes, startMs - SHARER_FILTER_WINDOW_MS);
  ...
  if (unresolved.length > 0) {
    ...
    // Name + count only — full UUID lists per person can be several KB in the Debug Console.
    const notesPerPerson = peopleNoteUuidSummary(peopleIndex).map(({ count, name }) => ({ count, name }));
    logIfEnabled("[SharedNotes] known notes per person (from getPeople):", notesPerPerson);
  }
...
//   least one shown note (unlike sharerNamesFromIndex, which spans every shared note). When `sinceMs`
//   is given, only notes updated at/after it contribute names
// @param {Array<Object>} notes - Result entries, each with a `collaborators` array of { name } and `updatedMs`.
// @param {number} sinceMs - Cutoff in ms; notes with `updatedMs` below it are skipped (0 keeps all).
// @returns {Array<string>} Alphabetical, de-duplicated collaborator names present in the in-window notes.
export function sharerNamesFromNotes(notes, sinceMs = 0) {
  const names = new Set();
  if (!Array.isArray(notes)) return [];
  for (const note of notes) {
    if (sinceMs && timestampMsFromValue(note.updatedMs ?? note.updated) < sinceMs) continue;
    for (const collaborator of note.collaborators || []) {
