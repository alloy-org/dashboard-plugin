...
//   { activeMs, collaborators, noteHandle, updatedMs }; `sharerNames` is the alphabetical list of
//   collaborators appearing on the returned notes.
export async function findCollaboratorUpdatedNotes({ app, maxNotes = Infinity,
  ...
  for await (const noteHandle of handles || []) {
    ...
    results.push({ activeMs: timestampMsFromValue(noteHandle.active),
      collaborators: collaboratorsForNote({ noteHandle, peopleIndex }), noteHandle,
...
// @param {Array<Object>} notes - Result entries with `noteHandle.uuid`.
...
export function orderNotesPinnedFirst(notes, pinnedUuids) {
  ...
  const pinned = [], rest = [];
  for (const note of notes) {
    (pinnedSet.has(note.noteHandle.uuid) ? pinned : rest).push(note);
  }
  return [...pinned, ...rest];
}
