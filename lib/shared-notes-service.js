...
//   { activeMs, collaborators, openedAt, updatedMs, ...noteHandle } — the note's own attrs (e.g.
//   `uuid`/`name`) are spread onto the entry rather than nested under `noteHandle`; `sharerNames`
//   is the alphabetical list of collaborators appearing on the returned notes.
export async function findCollaboratorUpdatedNotes({ app, maxNotes = Infinity,
  ...
  for await (const noteHandle of handles) {
    if (seen.has(noteHandle.uuid)) continue;
    seen.add(noteHandle.uuid);
    if (!updatedSinceSeen(noteHandle)) continue;

    // `updatedMs` is the last edit by anyone (the collaborator); `activeMs` is when the current user
    results.push({ activeMs: timestampMsFromValue(noteHandle.active), collaborators: collaboratorsForNote({ noteHandle, peopleIndex }),
      updatedMs: timestampMsFromValue(noteHandle.updated), ...noteHandle });
