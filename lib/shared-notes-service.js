...
export async function fetchPeopleIndexByNote(app) {
  ...
  try {
    ...
    logIfEnabled(`[SharedNotes] getPeople -> ${ Array.isArray(people) ? people.length : 0 } people across ${ index.size } notes`);
    return index;
  } catch (error) {
...
//   where each `collaborators` entry is a { name, avatar } object (see collaboratorsForNote).
