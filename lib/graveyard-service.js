...
function rowsFromGraveyardContent(content) {
  ...
  logIfEnabled('[Graveyard] rowsFromGraveyardContent: parsing', lines.length, 'lines of note content');
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('| Date') || trimmed.startsWith('|---')) { skippedHeader++; continue; }
    ...
    if (cells[CACHE_ROW_TASK_ENTRIES_INDEX]) {
      try {
        ...
        if (Array.isArray(parsedEntries)) {
          ...
          logIfEnabled(`[Graveyard] rowsFromGraveyardContent: parsed ${ taskEntries.length } taskEntries for dateKey=${ JSON.stringify(dateKey) }`);
        } else {
    ...
    if (dateKey && uuids.length > 0) {
      ...
      skippedNoUuids++;
      logIfEnabled('[Graveyard] rowsFromGraveyardContent: skipping row with dateKey:', JSON.stringify(dateKey), 'uuids:', uuids);
    }
  }
  logIfEnabled(`[Graveyard] rowsFromGraveyardContent: skippedNonTable=${ skippedNonTable } skippedHeader=${ skippedHeader } skippedTooFewCells=${ skippedTooFewCells } skippedNoUuids=${ skippedNoUuids } validRows=${ rows.length }`);
  return rows;
}
...
// [Claude claude-sonnet-4-6] Task: hydrate cached graveyard tasks — domain lookup with per-note fallback
// Prompt: "getTaskDomainTasks(undefined) returns 0; fall back to per-note fetches using cached noteUUIDs"
async function hydratedTasksFromUuids(app, taskDomainUUID, taskEntries, uuids) {
  logIfEnabled('[Graveyard] hydratedTasksFromUuids: called with taskDomainUUID:', taskDomainUUID, 'uuidCount:', uuids?.length, 'taskEntryCount:', taskEntries?.length);
  if (!uuids || uuids.length === 0) {
    logIfEnabled('[Graveyard] hydratedTasksFromUuids: early return — no uuids');
    return [];
  }
  const taskEntryByUuid = new Map((taskEntries || []).filter(entry => entry?.uuid).map(entry => [entry.uuid, entry]));
  let allTasks = [];
  try {
    allTasks = await app.getTaskDomainTasks(taskDomainUUID);
    logIfEnabled('[Graveyard] hydratedTasksFromUuids: getTaskDomainTasks(', taskDomainUUID, ') returned', allTasks?.length ?? 'null/undefined', 'tasks');
    allTasks = allTasks || [];
  } catch (err) {
    logIfEnabled('[Graveyard] hydratedTasksFromUuids: getTaskDomainTasks threw:', err, '— will fall back to per-note fetches');
    allTasks = [];
  }

  // When the domain lookup returns nothing (e.g. taskDomainUUID is undefined), fall back to
  // fetching tasks directly from the individual notes stored in the cached taskEntries.
  if (allTasks.length === 0) {
    logIfEnabled('[Graveyard] hydratedTasksFromUuids: domain returned 0 tasks — attempting per-note fallback using cached noteUUIDs');
    const noteUuids = [...new Set((taskEntries || []).map(e => e?.noteUUID).filter(Boolean))];
    logIfEnabled('[Graveyard] hydratedTasksFromUuids: fallback noteUUIDs:', noteUuids);
    if (noteUuids.length > 0) {
      const settled = await Promise.allSettled(
        noteUuids.map(noteUUID =>
          app.getNoteTasks({ uuid: noteUUID }, { includeDone: false })
            .then(tasks => ({ noteUUID, tasks: tasks || [] }))
        )
      );
      for (const outcome of settled) {
        if (outcome.status === 'rejected') {
          logIfEnabled('[Graveyard] hydratedTasksFromUuids: getNoteTasks failed for a note:', outcome.reason);
          continue;
        }
        logIfEnabled('[Graveyard] hydratedTasksFromUuids: getNoteTasks for note', outcome.value.noteUUID, 'returned', outcome.value.tasks.length, 'tasks');
        allTasks.push(...outcome.value.tasks);
      }
      logIfEnabled('[Graveyard] hydratedTasksFromUuids: per-note fallback collected', allTasks.length, 'total tasks');
    } else {
      logIfEnabled('[Graveyard] hydratedTasksFromUuids: no cached noteUUIDs available — cannot fall back');
    }
  }

  const taskByUuid = new Map(allTasks.map(t => [t.uuid, t]));
  logIfEnabled('[Graveyard] hydratedTasksFromUuids: task map size:', taskByUuid.size, '— resolving', uuids.length, 'cached uuids');
  let missingFromDomain = 0, completedOrDismissed = 0, accepted = 0;
  const resolved = uuids.map((uuid) => {
    const task = taskByUuid.get(uuid);
    if (!task) { missingFromDomain++; logIfEnabled('[Graveyard] hydratedTasksFromUuids: uuid not found in domain:', uuid); return null; }
  ...
  logIfEnabled(`[Graveyard] hydratedTasksFromUuids: results — accepted=${ accepted } missingFromDomain=${ missingFromDomain } completedOrDismissed=${ completedOrDismissed } total=${ resolved.length }`);
  return resolved;
}
...
export async function loadGraveyardCandidates(app, count, taskDomainUUID) {
  ...
  // Exclude only rows other than today (today row is stale/absent and will be replaced).
  const rowsForExclusion = rows.filter(r => r.dateKey !== todayKey);
