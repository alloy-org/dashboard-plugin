...
import { dateKeyFromDateInput, millisFromDateInput } from "util/date-utility";
import { logIfEnabled } from "util/log";
import { tableHeaderFromColumns, tableRowFromCells, tableRowsFromMarkdown } from "util/markdown-table";

const CACHE_COLUMNS = [{ key: 'dateKey', label: 'Date' }, { key: 'uuids', label: 'Task UUIDs' },
  { key: 'taskEntries', label: 'Task note metadata' }];
// The metadata column was added after the first notes were written, so two-cell rows are still read.
const CACHE_ROW_MIN_CELL_COUNT = 2;
const CACHE_ROW_DATE_KEY_PATTERN = /^\d{4}-\d{2}-\d{2}$/;
const DAYS_PER_MONTH = 30;
const DAYS_PER_WEEK = 7;
const GRAVEYARD_MIN_TASK_AGE_DAYS = DAYS_PER_MONTH;
...
const GRAVEYARD_MIN_TASK_AGE_MS = GRAVEYARD_MIN_TASK_AGE_DAYS * HOURS_PER_DAY * MINUTES_PER_HOUR * SECONDS_PER_MINUTE * MS_PER_SECOND;
...
async function findOrCreateGraveyardNote(app) {
  ...
  if (existing?.uuid) return { uuid: existing.uuid };
...
function graveyardContentFromRows(rows) {
  ...
  const tableLines = rows.map((row) => {
    ...
    return tableRowFromCells([row.dateKey, row.uuids.join(','), encodedTaskEntries]);
  }).join('\n');
...
// ----------------------------------------------------------------------------------------------
// @desc Rehydrate the task-to-note metadata column, which holds a URI-encoded JSON array of task entries.
//   Falls back to uuid-only entries when the column is absent (notes written before it existed) or unreadable.
// @param {string} cell - Raw metadata cell value.
// @param {object} params - { dateKey, uuids } of the row being parsed, for the fallback and for logging.
// @returns {Array<{noteName: string|null, noteUUID: string|null, uuid: string}>}
function taskEntriesFromCell(cell, { dateKey, uuids }) {
  const uuidOnlyEntries = uuids.map(uuid => ({ noteName: null, noteUUID: null, uuid }));
  if (!cell) {
    logIfEnabled(`[Graveyard] taskEntriesFromCell: no taskEntries column for dateKey=${ JSON.stringify(dateKey) },`,
      'falling back to uuid-only entries');
    return uuidOnlyEntries;
  }
  try {
    const parsedEntries = JSON.parse(decodeURIComponent(cell));
    if (!Array.isArray(parsedEntries)) {
      logIfEnabled('[Graveyard] taskEntriesFromCell: column was not an array after parse, value:', parsedEntries);
      return uuidOnlyEntries;
    }
    const namedEntries = parsedEntries.filter(entry => entry?.uuid);
    return namedEntries.map(entry => ({ noteName: entry.noteName || null, noteUUID: entry.noteUUID || null,
      uuid: entry.uuid }));
  } catch (err) {
    logIfEnabled('[Graveyard] failed to parse cached task note metadata:', err, '— raw cell value:',
      cell?.slice(0, 120));
    return uuidOnlyEntries;
  }
}
...
// Prompt: "keep a table of which task maps to which note when we are extracting the tasks"
// [Claude claude-opus-5[1m]] Task: read the cache table through the shared markdown-table helper
function rowsFromGraveyardContent(content) {
  const rows = [];
  const parsedRows = tableRowsFromMarkdown(content, { columns: CACHE_COLUMNS,
    requiredCellCount: CACHE_ROW_MIN_CELL_COUNT });
  for (const parsedRow of parsedRows) {
    const dateKey = parsedRow.dateKey;
    // Amplenote has been seen rewriting header cells, which can defeat the label match; a row whose first cell
    // is not a date key is never a data row anyway.
    if (!CACHE_ROW_DATE_KEY_PATTERN.test(dateKey)) continue;
    const uuids = parsedRow.uuids.split(',').map(s => s.trim()).filter(Boolean);
    if (uuids.length === 0) continue;
    rows.push({ dateKey, taskEntries: taskEntriesFromCell(parsedRow.taskEntries, { dateKey, uuids }), uuids });
  }
  return rows;
}
...
// ----------------------------------------------------------------------------------------------
// @desc Decide whether a task is old enough to be a graveyard candidate (created at least a month ago).
//   Tasks with an unknown creation timestamp are treated as ineligible, since the graveyard is meant
//   to surface aged, neglected tasks rather than anything recently created.
// @param {number} cutoffMillis - Newest creation timestamp still considered "old enough."
// @param {object} task - Task object with createdAt/created timestamp.
// @returns {boolean} True when the task was created on or before the cutoff.
function taskIsOldEnoughForGraveyard(cutoffMillis, task) {
  const createdAtMillis = taskCreatedAtMillis(task);
...
//   Tasks younger than the minimum age (one month) are excluded before ranking so the widget only
//   ever surfaces genuinely aged tasks.
...
// [OpenAI gpt-5.5] Task: rank graveyard candidates by value and proximity to nine months old
// Prompt: "Rewrite heuristic for evaluating @lib/dashboard/graveyard.js tasks..."
// [Claude claude-opus-4-8] Task: exclude tasks younger than one month before ranking
function rankedGraveyardCandidatesFromTasks(count, tasks) {
  if (tasks.length === 0) return [];
  const now = new Date();
  const targetMillis = targetGraveyardCreatedAtMillisFromDate(now);
  const minAgeCutoffMillis = now.getTime() - GRAVEYARD_MIN_TASK_AGE_MS;
  const eligibleTasks = tasks.filter(task => taskIsOldEnoughForGraveyard(minAgeCutoffMillis, task));
  if (eligibleTasks.length === 0) return [];
  const scored = eligibleTasks.map((task, graveyardHeuristicRankIndex) => ({
    ...task,
...
function taskCreatedAtMillis(task) {
  return millisFromDateInput(task?.createdAt ?? task?.created);
}
...
export async function appendTaskToRetiredNote(app, task) {
  ...
  const existingRetiredNote = await app.findNote({ name: noteName, tags: [DASHBOARD_NOTE_TAG] }).catch(() => null);
  let noteHandle = existingRetiredNote?.uuid ? { uuid: existingRetiredNote.uuid } : null;
  if (!noteHandle?.uuid) {
    const uuid = await app.createNote(noteName, [DASHBOARD_NOTE_TAG], { archive: true });
