...
// @returns {string} Note name, e.g. "Graveyard tasks of May 2026".
...
export function graveyardNoteNameFromDate(date) {
  ...
  return `Graveyard tasks of ${ MONTHS[d.getMonth()] } ${ d.getFullYear() }`;
}
...
async function discoverCandidateTasks(app, count, excludedUuids, taskDomainUUID) {
  ...
  const noteHandles = await app.filterNotes(filterOptions).catch(() => []);
