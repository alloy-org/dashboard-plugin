...
| `lib/plan-wizard/prospect-evidence.js` | Implemented | Important tasks from the past three months, completions from the past month, other recently created tasks, and the notes that work happened in, alongside the chosen intents and the rejected history. No note-authorship filter and no note view counts: the plugin API exposes neither, so domain scoping and open-task load stand in. |
| `lib/plan-wizard/prospect-discovery.js` | Implemented, on demand | Proposes ActionProspects from at least two supporting tasks, tied to chosen intents, favoring automation and upstream fixes. Identity is derived from the summary text, so a re-proposed idea reuses the record the user already judged. Continuous background harvesting remains. |
...
Save the answers and show a saved state. “Find my projects” is now the continuation: it saves the intents, moves
to the projects page, and runs discovery there. Discovery stays an explicit action rather than a mount-time
effect, and the projects page states which discovery state it is in, so the UI still never implies projects have
been generated when none were.

## Following files, after the first slice

All files in this section remain unimplemented. Project discovery has since landed and moved to the table above;
what is left of that milestone is continuous background harvesting and `ProspectTask` proposals.
